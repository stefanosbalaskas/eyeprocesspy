...
target_x = np.repeat([0.2, 0.5, 0.8], 10)
target_y = np.repeat([0.2, 0.5, 0.8], 10)
calibration = pd.DataFrame(
    {
        "target_x": target_x,
        "target_y": target_y,
        "gaze_x": target_x + rng.normal(0, 0.01, len(target_x)),
        "gaze_y": target_y + rng.normal(0, 0.01, len(target_y)),
    }
)
...
samples = pd.DataFrame(
    {
        ...
        "gaze_x": np.clip(0.5 + rng.normal(0, 0.08, 60), 0, 1),
        "gaze_y": np.clip(0.5 + rng.normal(0, 0.08, 60), 0, 1),
