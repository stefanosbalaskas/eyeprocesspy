...
def diagnostic_model(features: pd.DataFrame, assigned: pd.DataFrame, spec) -> pd.DataFrame:
    ...
    if pid == "dilate_1_px":
        return pd.DataFrame([{
            "term": "condition", "estimate": 0.30, "SE": 0.20,
            "CI_low": -0.09, "CI_high": 0.69, "p_value": 0.13,
            "model_converged": False, "N": 4,
        }])
    return pd.DataFrame([{
        "term": "condition", "estimate": 0.42, "SE": 0.12,
        "CI_low": 0.18, "CI_high": 0.66, "p_value": 0.01,
        "model_converged": True, "N": 4,
    }])
...
def run(output_dir: Path) -> dict[str, Path]:
    ...
    ax = ep.plot_aoi_perturbations(
        result, perturbation_id="dilate_1_px",
        data=observations, x_col="x", y_col="y",
    )
