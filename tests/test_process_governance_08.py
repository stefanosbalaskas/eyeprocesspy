    ...
    d = pd.DataFrame(
        {
            ...
            "valid_gaze_prop": np.r_[np.repeat(0.95, 36), np.repeat(0.55, 12)],
            "valid_pupil_prop": rng.uniform(0.75, 0.98, 48),
            "missing_gaze": np.r_[np.repeat(0, 36), np.repeat(1, 12)],
            "missing_pupil": 0,
            "rt_ms": 900,
            "blink_cluster_count": rng.poisson(1, 48),
            "sampling_rate_hz": 60,
        }
    )
...
def test_multivariate_anomaly_is_review_oriented():
    ...
    d = pd.DataFrame(
        {
            "person_id": [f"P{i}" for i in range(1, 41)],
            "dwell_ms": np.r_[rng.normal(700, 50, 39), 2000],
            "rt_ms": np.r_[rng.normal(1000, 70, 39), 4000],
            "valid_gaze_prop": np.r_[rng.uniform(0.9, 0.99, 39), 0.5],
        }
    )
    a = ep.audit_process_anomalies(
        d, metrics=["dwell_ms", "rt_ms", "valid_gaze_prop"], aggregate=False
    )
...
def test_drift_detects_designed_difficulty_change():
    d = pd.MultiIndex.from_product(
        [["i1", "i2", "i3"], range(1, 5)], names=["item_id", "deployment_batch"]
    ...
    d["irt_difficulty"] = np.r_[
        np.repeat(0.0, 3), np.repeat(0.05, 3), np.repeat(0.10, 3), [0.8, 0.1, 0.1]
    ]
    d["irt_discrimination"] = 1.0
    d["dwell_ms"] = 800.0
    d["valid_gaze_prop"] = 0.95
    a = ep.audit_process_drift(
        d, metrics=["irt_difficulty", "irt_discrimination", "dwell_ms", "valid_gaze_prop"]
    )
...
def test_grouped_process_apis_reject_missing_identifiers():
    d = pd.DataFrame({"person_id": ["P1", None], "valid_gaze_prop": [0.95, 0.9]})
    with pytest.raises(
        EyeProcessValidationError, match="Grouping columns must not contain missing values"
    ):
...
def synthetic_window_data():
    ...
    d["pupil_bc"] = np.sin(d.time_ms / 600) + rng.normal(0, 0.05, len(d))
    d["x"] = rng.normal(500, 30, len(d))
    d["y"] = rng.normal(400, 30, len(d))
    d["aoi"] = np.resize(["target", "text", "button"], len(d))
    d["valid_gaze_prop"] = 0.95
    d["valid_pupil_prop"] = 0.95
    d["blink"] = False
    d["trackloss"] = False
    return d
...
def test_process_windows_produce_temporal_features():
    ...
    x = ep.extract_process_windows(
        d, spec=ep.process_window_spec(1000, 500, 0, 3000), pupil="pupil_bc"
    )
...
def test_pupil_activity_features_finite_on_oscillation():
    t = np.arange(0, 5000 + 1e-9, 1000 / 60)
    y = np.sin(2 * np.pi * 0.3 * t / 1000) + 0.2 * np.sin(2 * np.pi * 1.2 * t / 1000)
    lo = ep.pupil_band_power(y, 60, 0.05, 0.5)
    hi = ep.pupil_band_power(y, 60, 0.5, 4)
...
def test_pupil_event_deconvolution_positive_effect():
    ...
    t = np.arange(0, 3000.1, 20)
    k = ep.pupil_response_kernel(t - 500)
    d = pd.DataFrame(
        {
            "person_id": "P1",
            "trial_id": "T1",
            "time_ms": t,
            "pupil_bc": 0.8 * k + rng.normal(0, 0.02, len(t)),
            "event_ms": 500,
        }
    )
...
def test_base_signal_filter_auditable_and_backends_explicit():
    ...
    y = np.sin(np.linspace(0, 6, 101)) + rng.normal(0, 0.1, 101)
    y[49] += 3
    ...
    with pytest.raises(EyeProcessBackendError):
        ep.filter_eye_signal(y, method="robfilter")
...
def test_pupil_confound_lm_reference_adjusts_values():
    ...
    d = pd.DataFrame(
        {
            "person_id": np.repeat([f"P{i}" for i in range(1, 11)], 6),
            "item_id": np.tile([f"I{i}" for i in range(1, 7)], 10),
            "pupil_peak": rng.normal(size=n),
            "screen_luminance": np.resize([80, 160], n),
            "trial_sequence": np.resize([1, 2, 3], n),
        }
    )
    ...
    with pytest.raises(EyeProcessBackendError):
        ep.fit_pupil_confound_model(d, engine="mgcv")
    ...
    d = pd.DataFrame(
        {
            ...
            "trial_id": pd.Series(dtype=str),
            "time_ms": pd.Series(dtype=float),
            "pupil_bc": pd.Series(dtype=float),
            "aoi": pd.Series(dtype=str),
        }
    )
    with pytest.raises(EyeProcessValidationError, match="at least one row"):
        ep.pupil_frequency_features(d)
    with pytest.raises(EyeProcessValidationError, match="at least one row"):
        ep.aoi_trajectory_features(d)


def test_broader_contract_smoke():
    d = synthetic_window_data()
    x = ep.extract_process_windows(d, spec=ep.process_window_spec(1000, 500, 0, 3000))
    b = ep.bind_process_windows(x, x)
    assert len(b.data) == 2 * len(x.data)
    sens = ep.audit_process_window_sensitivity(
        d, widths_ms=[500, 1000], steps_ms=[250], spec=ep.process_window_spec(1000, 500, 0, 3000)
    )
    assert len(sens.table) == 2
    ...
    assert len(freq.features) == 4
    stab = ep.audit_pupil_frequency_stability(d, windows_ms=[500, 1000])
    assert stab.eyeprocess_class == "eye_pupil_frequency_stability"
...
def test_governance_08_plot_counterparts_have_data_layers():
    ...
    ax = ep.plot_eye_process_windows(w)
    assert hasattr(ax, "eyeprocess_plot_data")
    plt.close(ax.figure)
    sens = ep.audit_process_window_sensitivity(
        d, widths_ms=[500, 1000], steps_ms=[250], spec=ep.process_window_spec(1000, 500, 0, 3000)
    )
    ax = ep.plot_process_window_sensitivity(sens)
    assert len(ax.eyeprocess_plot_data) == 2
    plt.close(ax.figure)
    freq = ep.pupil_frequency_features(d)
    for fun in [
        ep.plot_eye_pupil_frequency_features,
        ep.plot_pupil_band_power,
        ep.plot_pupil_activity_windows,
    ]:
        ax = fun(freq)
        assert hasattr(ax, "eyeprocess_plot_data")
        plt.close(ax.figure)
    t = np.arange(0, 2000, 1000 / 60)
    y = np.sin(2 * np.pi * 0.3 * t / 1000)
    ax = ep.plot_pupil_spectrum(y, 60)
