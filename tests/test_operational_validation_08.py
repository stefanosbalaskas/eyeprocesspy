...
def test_validation_evidence_bundle_produces_report_and_manifest(tmp_path: Path):
    ...
    p = ep.write_validation_report(b, tmp_path / "report.txt", include_session=False)
...
def test_preaction_and_decision_proxy_features_generated():
    d = synthetic_process_data()
    p = ep.preaction_process_features(d, windows_ms=[500, 1000])
    q = ep.addm_glam_proxy_features(d)
...
def test_feature_stability_assigns_conservative_families():
    d = pd.MultiIndex.from_product(
        [["pupil_mean", "valid_gaze", "aoi_entropy"], range(1, 5)], names=["feature", "split"]
    ).to_frame(index=False)
    d["importance"] = np.linspace(0.1, 0.9, len(d))
...
def test_operational_plot_counterparts_have_data_layers():
    ...
    p = ep.preaction_process_features(d, windows_ms=[500, 1000])
    q = ep.addm_glam_proxy_features(d)
    b = ep.collect_validation_evidence(recovery=pd.DataFrame({"bias": [0.1]}), model_name="demo")
    s = ep.score_response_stream(object(), [1, 0, 1], observed_order=[1, 2, 3])
    stability = ep.process_feature_stability(
        pd.DataFrame(
            {
                "feature": ["pupil_mean", "valid_gaze"] * 2,
                "split": [1, 1, 2, 2],
                "importance": [0.8, 0.4, 0.7, 0.5],
            }
        ),
        top_n=1,
    )
    for fun, obj in [
        (ep.plot_eye_preaction_process_features, p),
        (ep.plot_eye_decision_process_proxy, q),
        (ep.plot_eye_validation_bundle, b),
        (ep.plot_eye_streaming_score, s),
    ]:
        ax = fun(obj)
        assert hasattr(ax, "eyeprocess_plot_data")
        plt.close(ax.figure)
    ax = ep.plot_process_feature_stability(stability, stability="top_n_selection_rate")
