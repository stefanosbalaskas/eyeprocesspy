...
def _reliability_data(seed: int = 9) -> pd.DataFrame:
    ...
    for i, person in enumerate(people):
        for session in (1, 2):
            rows.append(
                {"person_id": person, "session": session, "value": u[i] + rng.normal(scale=0.2)}
            )
...
def _calibration_data(seed: int = 9) -> pd.DataFrame:
    rng = np.random.default_rng(seed)
    tx = np.repeat([0.2, 0.5, 0.8], 10)
    ty = np.repeat([0.2, 0.5, 0.8], 10)
    return pd.DataFrame(
        {
            "target_x": tx,
            "target_y": ty,
            "gaze_x": tx + rng.normal(0, 0.01, 30),
            "gaze_y": ty + rng.normal(0, 0.01, 30),
        }
    )
...
def test_registry_search_coverage_lineage_and_units():
    reg = ep.register_process_measure(
        ...
        guardrail="Not a psychological construct.",
    )
    ...
    lineage = ep.process_measure_lineage(
        "custom_signal", ["gaze_x", "gaze_y", "gaze_x"], ["center", "aggregate"], "trial"
    )
    ...
    assert set(ep.process_measure_guardrails(reg).columns) == {
        "name",
        "channel",
        "interpretation",
        "guardrail",
...
def test_reliability_contracts_match_frozen_r_tests_and_smoke():
    ...
    boot = ep.bootstrap_process_reliability(
        d, "person_id", "session", "value", replications=20, seed=4
    )
...
def test_split_half_reliability_handles_nonconsecutive_index_and_random_replications():
    ...
    for person in range(1, 16):
        ...
        for trial in range(1, 9):
            rows.append(
                {"person": person, "trial": trial, "measure": trait + rng.normal(scale=0.25)}
            )
    d = pd.DataFrame(rows)
    ...
    rnd = ep.split_half_process_reliability(
        d, "person", "trial", "measure", split="random", repetitions=5, seed=2
    )
    assert len(rnd) == 5
...
def test_calibration_quality_contracts_match_frozen_r_tests():
    ...
    g = pd.DataFrame(
        {
            ...
            "gaze_x": rng.normal(size=100),
            "gaze_y": rng.normal(size=100),
            "valid": pd.Series([True] * 100, dtype="boolean"),
        }
    )
    ...
    aois = pd.DataFrame(
        {"aoi": ["A"], "x_min": [0.0], "x_max": [1.0], "y_min": [0.0], "y_max": [1.0]}
    )
    bd = ep.fixation_boundary_uncertainty(
        pd.DataFrame({"gaze_x": [0.5, np.nan], "gaze_y": [0.5, 0.2]}), aois
    )
...
def test_calibration_metrics_irregularity_drift_and_reporting():
    ...
    t = pd.DataFrame(
        {
            "timestamp_ms": [0, 10, 20, 31, 40],
            "gaze_x": [0, 0.1, 0.2, 0.3, 0.4],
            "gaze_y": [0, 0, 0.1, 0.1, 0.2],
        }
    )
    ef = ep.effective_sampling_frequency(t)
    assert np.isfinite(ef.effective_hz.iloc[0])
    ir = ep.audit_sampling_irregularity(t, cv_threshold=0.01)
...
def test_probabilistic_aoi_uncertainty_preserves_missing_draw_semantics():
    draws = pd.DataFrame(
        {
            "sample_id": [1, 1, 2, 2],
            "gaze_x": [0.5, 0.6, np.nan, np.nan],
            "gaze_y": [0.5, 0.6, 0.2, np.nan],
        }
    )
    aois = pd.DataFrame(
        {"aoi": ["A", "B"], "x_min": [0, 0.6], "x_max": [0.59, 1], "y_min": [0, 0], "y_max": [1, 1]}
    )
    p = ep.aoi_membership_probability(draws, aois)
    assert p.loc[(p.sample_id == 1) & (p.aoi == "A"), "probability"].iloc[0] == pytest.approx(0.5)
    assert p.loc[p.sample_id == 2, "probability"].isna().all()

    model = ep.calibration_error_model(_calibration_data())
    gaze = pd.DataFrame({"gaze_x": [0.25, 0.75], "gaze_y": [0.5, 0.5]})
    pa = ep.probabilistic_aoi_assignment(gaze, aois, model, draws=100, seed=2, min_probability=0.4)
...
def test_process_quality_plot_counterparts_expose_scientific_plot_data():
    ...
    g = pd.DataFrame(
        {
            ...
            "gaze_x": np.linspace(0, 1, 10),
            "gaze_y": np.linspace(0, 1, 10),
        }
    )
    ...
    aois = pd.DataFrame(
        {"aoi": ["A", "B"], "x_min": [0, 0.5], "x_max": [0.49, 1], "y_min": [0, 0], "y_max": [1, 1]}
    )
    pa = ep.probabilistic_aoi_assignment(
        g[["gaze_x", "gaze_y"]].iloc[:3], aois, model, draws=40, seed=1, min_probability=0.2
    )
