...
def test_schema_exact_table_order_and_count():
    ...
    assert ep.canonical_table_names() == [
        "recordings",
        "streams",
        "gaze_samples",
        "eye_samples",
    ...
    assert ep.schema_table("recordings")[:4] == [
        "recording_id",
        "participant_id",
        "session_id",
        "vendor",
...
def test_orphan_recording_and_stop_contract():
    ...
    with pytest.raises(ep.EyeProcessValidationError):
        ep.validate_eye_dataset(x, stop_on_error=True)


def test_coordinate_space_defaults_exact():
    s = ep.new_coordinate_space(
        "display", space_type="display_pixels_top_left", width=1920, height=1080
    )
...
def test_sampling_rate_r_algorithm():
    assert ep.estimate_sampling_rate([0, 0.01, 0.02, 0.03]) == pytest.approx(100)
...
def test_normalize_timebase_per_recording():
    g = pd.DataFrame(
        {
            ...
            "sample_id": ["a", "b", "c", "d"],
            "timestamp_native": [1000, 1010, 50, 60],
        }
    )
    ...
    assert y["gaze_samples"]["timestamp_seconds"].tolist() == pytest.approx([0, 0.01, 0, 0.01])
...
def test_coordinate_conversion_copy_and_overwrite():
    ...
    spaces = pd.concat(
        [
            ep.new_coordinate_space("norm"),
            ep.new_coordinate_space("px", "display_pixels_top_left", width=1000, height=500),
        ],
        ignore_index=True,
    )
    gaze = pd.DataFrame(
    ...
    x = ep.new_eye_dataset(
        recordings=rec, coordinate_spaces=spaces, gaze_samples=gaze, validate=False
    )
    y = ep.convert_coordinates(x, "norm", "px")
    row = y["gaze_samples"][y["gaze_samples"]["coordinate_space_id"] == "px"].iloc[0]
    assert (
        row.gaze_x == pytest.approx(500)
        and row.gaze_y == pytest.approx(125)
        and row.sample_id == "s1_px"
    )
