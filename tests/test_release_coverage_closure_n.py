...
def _fitter(simulation, **kwargs):
    return {"estimate": float(np.mean(simulation["values"])), "se": 0.25, "converged": True}
...
def test_validation_orchestration_fingerprint_path_and_missing_resume(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        vo.inspect, "getsource", lambda f: (_ for _ in ()).throw(OSError("no source"))
    )
    fp = vo._function_fingerprint(lambda x: x)
    assert fp.startswith("fn-")
    ...
    run = vo.run_validation_jobs(
        manifest, _simulator, _fitter, _extractor, _truth, tmp_path / "run"
    )
    ...
    def fake_run(plan_arg, **kwargs):
        ...
        return vo.EyeValidationRun(results=[])
...
def test_requested_api_and_multimodal_invalid_paths():
    frame = ra._extract_samples({"samples": pd.DataFrame({"x": [1]})})
    assert frame.x.iloc[0] == 1
    with pytest.raises(ep.EyeProcessValidationError):
        ra._extract_samples(object())
    with pytest.raises(ep.EyeProcessValidationError, match="eye_multimodal_measurement"):
        mm.audit_multimodal_measurement(object())


def test_gazepoint_invalid_frequency_and_read_oserror(monkeypatch, tmp_path):
    info = gp._gp_time_info(pd.DataFrame({"TIMETICK(F=.)": [1.0]}))
    assert np.isnan(info["tick_frequency"])
    p = tmp_path / "plain.csv"
    p.write_text("x", encoding="utf-8")
    real = Path.read_text
    monkeypatch.setattr(
        Path, "read_text", lambda self, *a, **k: (_ for _ in ()).throw(OSError("blocked"))
    )
    assert gp._gp_is_summary_report(p) is False
...
def test_semantic_coercion_and_nonlinear_correlated_branch():
    ...
    source = pd.DataFrame({"x": [1.0, 2.0, 3.0, 4.0]})
    roundtrip = pd.DataFrame({"x": [1.0, 2.2, 3.7, 5.6]})
    spec = sv.semantic_fidelity_spec(correlation_floor=0.9, coordinate_tolerance=1e-12)
...
def test_validation_release_report_auto_completion(monkeypatch, tmp_path):
    collection = vo.EyeValidationCollection(
        plan=None,
        paths=[],
        results=[],
        jobs=pd.DataFrame(),
    ...
    sentinel = object()
    monkeypatch.setattr(voc, "audit_validation_completion", lambda x: sentinel)
...
def test_bids_defensive_unmapped_pupil_guard(monkeypatch, tmp_path):
    ...
    def map_once(self, arg, *args, **kwargs):
        if self.name == "sample_id":
            calls["sample"] += 1
            if calls["sample"] == 2:
                return pd.Series([np.nan] * len(self), index=self.index, name=self.name)
...
def test_reproducibility_relative_version_import_failure(monkeypatch, tmp_path):
    ...
    def blocked(name, globals=None, locals=None, fromlist=(), level=0):
        fl = fromlist or ()
        if name == "" and level == 1 and "__version__" in fl:
            raise ImportError("blocked version")
        return real(name, globals, locals, fromlist, level)

    monkeypatch.setattr(builtins, "__import__", blocked)
    m = br.package_reproducibility_manifest([p], include_session=True)
    assert not any(s.startswith("eyeprocesspy ") for s in m.session)


def test_truth_map_unique_series_line():
    assert ve._truth_map(pd.Series([1.0], index=["a"])) == {"a": 1.0}
