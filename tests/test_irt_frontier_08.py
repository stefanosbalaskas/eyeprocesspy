...
def test_visual_context_registry_identifies_shared_contexts():
    ...
    X = pd.DataFrame(np.random.default_rng(6).binomial(1, 0.6, size=(200, 8)), columns=m.item_id)
...
def test_multiblock_mapping_has_reference_fallback():
    ...
    d = pd.DataFrame(
        {
            "person_id": [f"P{i}" for i in range(1, 31)],
            "theta": rng.normal(size=30),
            "accuracy": rng.random(30),
            "dwell": rng.normal(700, 70, 30),
            "entropy": rng.random(30),
            "pupil": rng.normal(size=30),
            "validity": rng.uniform(0.8, 1, 30),
        }
    )
    b = ep.process_feature_blocks(
        d,
        {
...
def test_process_profile_reference_is_explicitly_descriptive():
    ...
    d = pd.DataFrame(
        {
            "person_id": [f"P{i}" for i in range(1, 51)],
...
def test_external_validity_and_item_seed_reference_algorithms():
    ...
    d = pd.DataFrame(
        {
            "criterion": rng.normal(size=n),
            "base": rng.normal(size=n),
            "dwell": rng.normal(size=n),
            "pupil": rng.normal(size=n),
        }
    )
    ...
    item = pd.DataFrame(
        {
            "irt_difficulty": rng.normal(size=24),
            "irt_discrimination": rng.uniform(0.5, 2, 24),
            "words": rng.uniform(20, 100, 24),
            "visual": rng.normal(size=24),
        }
    )
    ...
    assert {"predicted_pre_pilot_difficulty", "predicted_pre_pilot_discrimination"}.issubset(
        pred.columns
    )
...
def test_frontier_estimators_remain_gated_without_external_engines():
    X = np.random.default_rng(9).binomial(1, 0.5, size=(20, 5))
    ...
    assert (
        ep.audit_frontier_model_contract(
            k2, {"recovery": 1, "misspec": 1, "benchmark": 1}
        ).status.iloc[0]
        == "candidate_for_validation_review"
    )
...
def test_structured_unstructured_contract_preserves_fold_locality():
    ...
    built = ep.prepare_structured_unstructured_process_features(
        d,
        fold="fold",
        builder=lambda train_s, train_u, test_s, test_u, fold_value: {
            "fold": fold_value,
            "n_train": len(train_s),
        },
    )
...
def test_latent_class_process_alignment_and_imputation_contracts():
    ...
    cls = pd.DataFrame(
        {"person_id": [f"P{i}" for i in range(1, 21)], "class": ["A"] * 10 + ["B"] * 10}
    )
    proc = pd.DataFrame(
        {
            "person_id": np.repeat(cls.person_id, 2),
            "dwell": rng.normal(size=40),
            "pupil": rng.normal(size=40),
        }
    )
    ...
    imp = ep.biometric_imputation_sensitivity(
        pd.DataFrame({"a": [1, np.nan, 3], "b": [np.nan, 2, 3]}), ["a", "b"], methods=[]
    )
    assert imp.eyeprocess_class == "eye_biometric_imputation_sensitivity"
    assert len(imp.missingness) == 2 and len(imp.results) == 0
    assert (
        ep.audit_biometric_imputation(
            pd.DataFrame({"a": [1, np.nan], "b": [2, 3]}), ["a", "b"], methods=[]
        ).eyeprocess_class
...
def test_bayesian_and_3pl_diagnostics_helpers_preserve_review_semantics():
    ...
    dash = EyeResult(
        {"posterior": posterior, "loo_table": pd.DataFrame()},
    ...
    items = pd.DataFrame(
        {
            ...
            "lower_asymptote": [0.1, 0.15, 0.2, 0.3, 0.4],
            "rt_ms": [100, 200, 300, 400, 500],
            "ttff_ms": [50, 60, 70, 80, 90],
            "a": [1, 1.1, 0.9, 1.2, 1.3],
            "b": [-1, -0.5, 0, 0.5, 1],
        }
    )
    audit = EyeResult(
        {
            ...
            "alignment": pd.DataFrame({"feature": ["rt_ms"], "correlation": [0.2]}),
            "process_features": ["rt_ms", "ttff_ms"],
        },
...
def test_08_plot_counterparts_return_axes():
    ...
    ax = ep.plot_eye_gated_process_model(gated)
    ...
    ax = ep.plot_eye_latent_process_alignment(align)
    assert len(ax.lines) == 2
    plt.close(ax.figure)
    imp = ep.biometric_imputation_sensitivity(
        pd.DataFrame({"a": [1, np.nan], "b": [np.nan, 2]}), ["a", "b"], methods=[]
    )
    ax = ep.plot_eye_biometric_imputation_sensitivity(imp)
    assert len(ax.patches) == 2
    plt.close(ax.figure)
    dash = EyeResult(
        {
            "posterior": pd.DataFrame(
                {"model": ["m1", "m1"], "rhat": [1.0, 1.02], "ess_bulk": [800, 200]}
            ),
            "loo_table": pd.DataFrame(),
        },
        eyeprocess_class="eye_bayesian_process_dashboard",
    )
    ax = ep.plot_eye_bayesian_process_dashboard(dash, type="rhat")
    assert ax is not None
    plt.close(ax.figure)
    items = pd.DataFrame(
        {
    ...
    a3 = EyeResult(
        {"item_parameters": items, "process_features": ["rt_ms"]},
        eyeprocess_class="eye_gaze_anchored_3pl_audit",
    )
    ax = ep.plot_eye_gaze_anchored_3pl_audit(a3)
    assert len(ax.patches) == 2
    plt.close(ax.figure)
...
def test_context_structure_plot_counterparts_return_axes():
    ...
    d = pd.DataFrame(
        {
            "person_id": [f"P{i}" for i in range(30)],
            "a": rng.normal(size=30),
            "b": rng.normal(size=30),
            "c": rng.normal(size=30),
            "criterion": rng.normal(size=30),
        }
    )
    ...
    ax = ep.plot_eye_visual_context_irt(vfit, type="context_registry")
    assert len(ax.patches) == 2
    plt.close(ax.figure)
