...
def test_partition_path_groups_chunks_and_transaction_helpers(tmp_path):
    path = mod._partition_path(tmp_path, "responses", ["participant_id"], [pd.NA], 2, "csv")
    ...
    grouped = mod._partition_groups(pd.DataFrame({"g": ["a", "a", pd.NA], "x": [1, 2, 3]}), ["g"])
...
def test_write_partitioned_guards_selection_and_existing_targets(tmp_path):
    ...
    with pytest.raises(ep.EyeProcessValidationError, match="not empty"):
        ep.write_partitioned_eye_storage(_tables(), target, ep.partition_eye_storage(format="csv"))
...
def test_open_partitioned_storage_guards_and_missing_manifest_columns(tmp_path):
    ...
    root.mkdir()
    (root / "_eyeprocess_storage.json").write_text(json.dumps({"format": "csv"}), encoding="utf-8")
    pd.DataFrame({"table": ["responses"], "relative_path": ["x.csv"]}).to_csv(
        root / "_partitions.csv", index=False
    )
    pd.DataFrame({"transaction_id": ["t"]}).to_csv(root / "_transactions.csv", index=False)
...
def test_ensure_and_query_guard_branches(tmp_path):
    ...
    accepted_set = ep.query_eye_storage(storage, "responses", filters={"score": {0, 1}})
...
def test_transaction_manifest_and_migration_guards(tmp_path):
    ...
    with pytest.raises(ep.EyeProcessValidationError, match="only to schema"):
        ep.migrate_eye_storage_schema(storage, tmp_path / "target", target_version="3.0.0")
    with pytest.raises(ep.EyeProcessBackendError, match="RDS"):
        ep.migrate_eye_storage_schema(storage, tmp_path / "target-rds", format="rds")

    migrated = ep.migrate_eye_storage_schema(storage, tmp_path / "target", format=None)
...
def test_benchmark_argument_guards_string_format_and_temp_directory(tmp_path):
    ...
    result = ep.benchmark_eye_storage(_tables(), formats="csv", repetitions=1, directory=tmp_path)
