...
def test_foundation_invalid_regex_own_handler_and_empty_visit_group(monkeypatch):
    ...
    monkeypatch.setattr(
        StringMethods, "contains", lambda *a, **k: (_ for _ in ()).throw(re.error("bad"))
    )
...
def test_preprocess_scalar_multikey_and_zero_transition_total(monkeypatch):
    ...
    x = ep.new_eye_dataset(validate=False)
    x["gaze_samples"] = ep.standardize_eye_table(d, "gaze_samples")
    real_groupby = pd.DataFrame.groupby
    ...
    monkeypatch.undo()
    monkeypatch.setattr(
        pf, "scanpath_sequence", lambda *a, **k: pd.DataFrame({"sequence": ["A > B"]})
    )
...
def test_adapter_non_tie_auto_read(tmp_path):
    ...
    try:
        ad.register_eye_adapter(
            name,
            detect=lambda path, **kw: 0.99,
            read=lambda path, **kw: ep.new_eye_dataset(validate=False),
            priority=9999,
            overwrite=True,
        )
        out = ad.read_eye_export(p, vendor="auto", confidence_threshold=0.5)
...
def test_coordinate_gaze_only_and_fixation_no_reverse():
    ...
    x["episodes"] = ep.standardize_eye_table(
        pd.DataFrame(
            {
                "episode_id": ["f"],
                "recording_id": ["R1"],
    ...
    return EyeResult(
        {
            ...
            "states": ["A", "B"],
            "data": pd.DataFrame({"state_probability": [1.0, 1.0, 1.0, 1.0]}),
        },
...
def test_dynamic_control_directions_and_unnamed_signature_guard():
    ...
    with pytest.raises(ep.EyeProcessValidationError, match="Unnamed strategy signatures"):
        di.theory_strategy_spec({"a": [1.0, 2.0], "b": [1.0]}, feature_columns=["x", "y"])
...
def test_evidence_graph_remaining_paths(monkeypatch):
    ...
    ax = eg._graph_plot(graph.nodes, bad_edges, "x")
    _close(ax)
    monkeypatch.setattr(eg, "cross_recurrence", lambda *a, **k: {})
    monkeypatch.setattr(eg, "recurrence_features", lambda r: pd.DataFrame({"rr": [0.1]}))
    with pytest.raises(ep.EyeProcessValidationError, match="covariates must align"):
        eg.crossmodal_recurrence_model(
            [1], [1], outcome=[1.0, 2.0], covariates=pd.DataFrame({"z": [1.0, 2.0, 3.0]})
        )
    trace = eg.trace_item_decision(graph, "yes")
    ax = eg.plot_eye_decision_trace(trace)
    _close(ax)
    model = eg.crossmodal_recurrence_model([1], [1])
    ax = eg.plot_eye_crossmodal_recurrence_model(model, type="coefficients")
    _close(ax)


def test_frontier_fold_none():
    out = fr.prepare_structured_unstructured_process_features(
        pd.DataFrame({"person_id": ["p"], "item_id": ["i"], "x": [1.0]}), fold=None
    )
...
def test_importer_residual_guards(tmp_path):
    ...
    mapping = {"timestamp": "t", "x": "x", "y": "y"}
    assert imp.validate_eye_mapping(mapping, data=None) is mapping
    data = pd.DataFrame({"ev": [pd.NA], "r": [pd.NA], "score": [np.nan], "rt": [np.nan]})
    rec = pd.Series(["R1"])
    t = pd.Series([0.0])
    assert imp._make_events(data, {"event_name": "ev"}, rec, t, t).empty
    assert imp._make_responses(
        data, {"response": "r", "score": "score", "response_time": "rt"}, rec
    ).empty
...
def test_operational_plot_invalid_objects_and_empty_feature_branch():
    for func in (
        po.plot_eye_streaming_score,
        po.plot_eye_validation_bundle,
        po.plot_eye_preaction_process_features,
        po.plot_eye_decision_process_proxy,
    ):
    ...
    ax = po.plot_eye_preaction_process_features(x)
    _close(ax)
    with pytest.raises(ep.EyeProcessValidationError):
...
def test_process_governance_short_and_wrapper_paths():
    arr = pg._interp([np.nan, 1.0])
    assert np.isnan(arr).all()
    data = pd.DataFrame({"person_id": ["p"], "trial_id": ["t"], "time_ms": [0.0]})
    spec = pg.process_window_spec(start_ms=0, end_ms=100, width_ms=100, step_ms=100, min_samples=2)
    out = pg.extract_process_windows(data, spec=spec)
    assert out.data.empty
    filt = pg.filter_pupil_signal([1, 2, 3, 4, 5], width=3, method="runmed")
    assert filt.method == "runmed"


def test_process_quality_all_nan_aggregate():
    d = pd.DataFrame(
        {"p": ["a", "a", "b", "b"], "t": [1, 2, 1, 2], "m": [np.nan, np.nan, np.nan, np.nan]}
    )
...
def test_pupil_missingness_nonmodel_and_plot_wrappers():
    d = pd.DataFrame({"y": [1.0, 2.0, 3.0], "obs": [True, False, True], "x": [0.0, 1.0, 2.0]})
    out = pm.fit_joint_signal_missingness("y", "obs", x=d, predictors=["x"])
    assert np.isnan(out.shared_correlation)
    sensitivity = pm.process_pattern_mixture([1.0, np.nan, 2.0], delta=[-1, 1])
    tip = pm.sensitivity_mnar_process(sensitivity)
    for func, obj in [
        (pm.plot_eye_mnar_sensitivity, sensitivity),
        (pm.plot_eye_mnar_tipping_point, tip),
    ]:
        ax = func(obj)
        _close(ax)
    obs = pm.fit_process_observation_model(d, [True, False, True], ["x"])
    ax = pm.plot_eye_process_observation_model(obs)
    _close(ax)


def test_timebase_residual_branches():
    assert np.isnan(tb.estimate_sampling_rate([1.0, 1.0, np.nan]))
    x = ep.new_eye_dataset(validate=False)
    with pytest.raises(ValueError, match="origin"):
        tb.normalize_timebase(x, origin="bad")
    out = tb.normalize_timebase(x, origin="absolute")
    assert ep.is_eye_dataset(out)
    audit = tb.audit_timebase(x)
