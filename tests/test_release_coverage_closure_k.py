...
def test_bayesian_3pl_defensive_and_review_paths():
    ...
    dash = EyeResult(
        {"posterior": pd.DataFrame()}, eyeprocess_class="eye_bayesian_process_dashboard"
    )
    ...
    x = EyeResult(
        {"alignment": pd.DataFrame({"x": [1]})}, eyeprocess_class="eye_gaze_anchored_3pl_audit"
    )
    assert not b3.gaze_anchored_3pl_alignment(x).empty
    with pytest.raises(ep.EyeProcessValidationError):
        b3.audit_3pl_process_signatures(object())
    x = EyeResult(
        {"item_parameters": pd.DataFrame({"a": [1, 2]})},
    ...
    x["item_parameters"] = pd.DataFrame(
        {"lower_asymptote": [0.1, 0.9], "rt_ms": [100, 200], "ttff_ms": [20, 30]}
    )
...
def test_irt_validation_guards_and_failure_capture():
    ...
    sbc = iv.run_sbc(
        lambda rng=None: {}, lambda data: None, lambda fit: pd.DataFrame(), replications=1
    )
...
def test_measurement_uncertainty_numpy_simulation_branch():
    base = mq._result(
        ...
        data=np.arange(8.0),
        specification=mq.process_uncertainty_spec(source_sd={"x": 1.0}, draws=3),
        uncertainty=pd.DataFrame({"source": ["x"], "sd": [1.0]}),
        summary=pd.DataFrame({"total_se": [1.0]}),
    )
...
def test_plot_residual_paths():
    ...
    ax = cp.plot_aoi_dwell(x, feature="dwell_time_ms")
    ...
    ax = plm.plot_eye_parameter_recovery(d)
    _close(ax)
    ax = plm.plot_eye_parameter_recovery(pd.DataFrame({"estimate": [np.nan], "truth": [np.nan]}))
    _close(ax)

    sx = EyeResult(
        {
            "history": pd.DataFrame(
                {"step": [1, 2], "theta": [0.1, 0.2], "theta_se": [0.05, np.nan]}
            ),
            "method": "EAP",
        },
        eyeprocess_class="eye_streaming_score",
    )
    ax = po.plot_eye_streaming_score(sx)
    assert len(ax.lines) >= 1
    _close(ax)

    data = pd.DataFrame({"fold": ["a", "b"], "improvement": [0.1, -0.1]})
    ax = ppi.plot_eye_incremental_information_audit(data)
    _close(ax)

    rel = EyeResult(
        {"bland_altman": None, "icc": pd.DataFrame({"icc_a1": [0.5]})},
        eyeprocess_class="eye_process_reliability_profile",
    )
    ax = ppq.plot_eye_process_reliability_profile(rel, type="bland_altman")
    _close(ax)
...
def test_pupil_missingness_validation_and_joint_paths():
    ...
    reg = EyeResult(
        {"registered": np.array([[1.0, 2.0]]), "ids": ["p1"], "id_col": "person_id"},
        eyeprocess_class="eye_pupil_registration",
    )
    with pytest.raises(ep.EyeProcessValidationError):
        pm.decompose_pupil_phase_amplitude(reg)
    d = pd.DataFrame({"y": [1.0, 2.0, 3.0], "x": [0.0, 1.0, 2.0]})
    with pytest.raises(ep.EyeProcessValidationError):
...
def test_evidence_provenance_plot_and_coordinates_aoi_geometry():
    ...
    ax = eg.plot_eye_provenance_comparison(comp)
    _close(ax)

    spaces = pd.DataFrame(
        [
            {
                "coordinate_space_id": "n",
...
def test_engine_adapter_pickle_failure_and_upgrade_scanpath_branches(monkeypatch):
    ...
    old = EyeResult(
        {"model": {"diagnostics": {"ok": 1}}, "spec": {"engine": "z"}},
        eyeprocess_class="eyeprocess_model",
    )
    ...
    ds["episodes"] = ep.standardize_eye_table(
        pd.DataFrame(
            {
                "recording_id": ["r"],
                "trial_id": ["t"],
...
def test_compositional_fallbacks(monkeypatch):
    ...
    comp = pd.DataFrame({"A": [0.4, 0.6], "B": [0.6, 0.4]})
    ...
    ax = ca.plot_aoi_balance_biplot(composition)
    _close(ax)
...
def test_small_remaining_branch_arcs(monkeypatch, tmp_path):
    ...
    x = EyeResult(
        {"item_parameters": pd.DataFrame({"lower_asymptote": [0.1, 0.2]})},
        eyeprocess_class="eye_gaze_anchored_3pl_audit",
    )
    out = b3.audit_3pl_process_signatures(x)
    assert out.fast_rt_review.eq(False).all() and out.fast_ttff_review.eq(False).all()

    z = ca.aoi_balance_coordinates(pd.DataFrame({"A": [0.4], "B": [0.6]}), np.empty((2, 0)))
    assert z.shape == (1, 0)

    old = EyeResult(
        {"engine": "fixed", "spec": {}, "model": {}}, eyeprocess_class="eyeprocess_model"
    )
    assert ea.upgrade_eyeprocess_model(old).engine == "fixed"
    ds = ep.new_eye_dataset(validate=False)
    ds["episodes"] = pd.DataFrame(
        {"recording_id": ["r"], "trial_id": ["t"], "start_time": [0.0], "aoi_id": ["A"]}
    )
    ...
    sbc = iv.run_sbc(
        lambda r: {"data": 1, "truth": {"theta": 0.0}},
        lambda d: object(),
        lambda fit: pd.DataFrame({"other": [0.0]}),
        replications=1,
    )
    assert len(sbc.failures) == 1
    ...
    spec = EyeResult(
        {
            "id": "tmp",
            "status": "reference",
            "fit_fun": lambda data: {"ok": True},
            "simulate_fun": lambda: 1,
            "validate_fun": None,
        },
        eyeprocess_class="eye_irt_model_spec",
    )
    assert pir.fit_irt_model(spec, {}) == {"ok": True}
    assert pir._segment_loglik(None, np.array([1.0, 2.0]), None)[1] == 2

    import matplotlib.pyplot as plt

    _, ax = plt.subplots()
    plm.plot_eye_parameter_recovery(pd.DataFrame({"estimate": [1.0], "truth": [1.0]}), ax=ax)
    _close(ax)
    _, ax = plt.subplots()
    pi.plot_eye_irt_process_alignment(
        {"table": pd.DataFrame({"b": [1.0], "gaze": [1.0]}), "correlations": pd.DataFrame()},
        channel="gaze",
        ax=ax,
    )
    _close(ax)
    legacy = SimpleNamespace(legacy=True, data={"features": pd.DataFrame()}, feature_names=["x"])
    ax = pfp.plot_eye_functional_pupil_irt(legacy)
    _close(ax)

    d = pd.DataFrame({"person_id": ["p1", "p1"], "item_id": ["i1", "i2"], "m": [np.nan, np.nan]})
    try:
        pq.split_half_process_reliability(
            d, "m", person="person_id", item="item_id", aggregate_fun=np.mean
        )
    except Exception:
        pass
