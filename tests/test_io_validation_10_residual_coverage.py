...
def _dataset():
            ...
            [
                {
                    ...
                    "valid_to": 1.0,
                    "frame_id": "F1",
                    "polygon": np.array([[0.1, 0.1], [0.3, 0.1], [0.3, 0.3]]),
                    "visible": True,
                    "coordinate_space_id": x["coordinate_spaces"]["coordinate_space_id"].iloc[0],
                }
            ]
    ...
    return ep.add_provenance(
        x, "import", "dataset", "private source", source_files="C:/private/source.csv"
    )


def _format_result(
    case_id="case", status="pass", imported=True, dataset=None, adapter_issues=None, roundtrip=None
):
    checks = pd.DataFrame(
        [
            ...
            {
                "check": "import",
                "status": "pass" if imported else "fail",
                "value": 1 if imported else 0,
                "message": "ok" if imported else "failed",
            },
        ]
    )
    source = pd.DataFrame(
    ...
    return io.EyeFormatValidation(
        ...
        audits={},
        roundtrip=roundtrip,
        import_error=pd.NA if imported else "failed",
        dataset=dataset,
    )
...
def test_read_corrupt_optional_json_and_schema_edge_paths(tmp_path):
    ...
    assert (
        coverage[(coverage["table"] == "recordings") & coverage["critical"]]["status"]
        .eq("fail")
        .any()
    )
    assert (
        coverage[(coverage["table"] == "gaze_samples") & coverage["critical"]]["status"]
        .eq("fail")
        .any()
    )
...
def test_stability_safe_audit_and_compare_edges():
    ...
    assert io._sorted_table(duplicated, "gaze_samples", ["sample_id", "value"])[
        "value"
    ].tolist() == [1, 2]
...
def test_validate_eye_source_failure_adapter_and_roundtrip_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        io, "inspect_eye_source", lambda p: pd.DataFrame([{"file_name": Path(p).name}])
    )
    monkeypatch.setattr(
        io, "detect_eye_format", lambda p: (_ for _ in ()).throw(RuntimeError("detect"))
    )
    monkeypatch.setattr(
        io, "read_eye_export", lambda *a, **k: (_ for _ in ()).throw(RuntimeError("import"))
    )
    ...
    monkeypatch.setattr(
        io,
        "detect_eye_format",
        lambda p: pd.DataFrame([{"format": "fake", "confidence": 0.1, "priority": 1}]),
    )
    ...
    monkeypatch.setattr(
        io, "roundtrip_eye_dataset", lambda *a, **k: (_ for _ in ()).throw(RuntimeError("rt"))
    )
    ...
    assert (
        result.checks.loc[result.checks["check"] == "format_detection", "status"].iloc[0]
        == "warning"
    )
    ...
    assert (
        warning.checks.loc[warning.checks["check"] == "adapter_specific_validation", "status"].iloc[
            0
        ]
        == "warning"
    )
...
def test_manifest_recycle_flags_validation_and_discovery(tmp_path):
    ...
    assert io._coerce_manifest_flag(pd.Series([True, False], dtype="bool"), "flag").tolist() == [
        True,
        False,
    ]
    flags = io._coerce_manifest_flag(
        pd.Series([pd.NA, "", "na", True, np.bool_(False), 1, 0.0, "yes", "n"], dtype=object),
        "flag",
    )
    ...
    with pytest.raises(io.EyeProcessValidationError, match="non-empty"):
        io._validate_manifest_rows(
            pd.DataFrame({"case_id": [""], "path": ["p"], "vendor": ["auto"]})
        )
...
def test_validate_eye_corpus_expectations_per_case_and_stop(tmp_path, monkeypatch):
    p1 = tmp_path / "ok.csv"
    p2 = tmp_path / "fail.csv"
    p1.write_text("x\n1\n", encoding="utf-8")
    p2.write_text("x\n2\n", encoding="utf-8")
    manifest = io.validation_manifest(
        [p1, p2],
        vendor="generic",
        expected_import=[True, False],
        require_gaze=[False, True],
        run_roundtrip=False,
        case_id=["ok", "expected-fail"],
    )
    calls = []
    ...
    assert (
        corpus.summary.loc[corpus.summary["case_id"] == "expected-fail", "status"].iloc[0] == "pass"
    )
...
def test_anonymization_reports_redaction_and_bundle_variants(tmp_path):
    ...
    rt = io.EyeRoundtripValidation(
        ...
        comparison=pd.DataFrame(
            [{"table": "gaze_samples", "status": "pass", "content_equal": True}]
        ),
    ...
    empty.source = pd.DataFrame()
    empty.detection = pd.DataFrame()
    empty.adapter_issues = pd.DataFrame()
    empty.roundtrip = None
    ...
    assert (
        "## case"
        in Path(io.write_format_validation_report(corpus, tmp_path / "corpus.md")).read_text()
    )
...
def test_remaining_unreadable_vendor_and_roundtrip_own_path(tmp_path, monkeypatch):
    tobii = tmp_path / "bad-tobii.csv"
    tobii.write_text("x\n1\n", encoding="utf-8")
    generic = tmp_path / "bad-generic.csv"
    ...
    eyelink = tmp_path / "cannot-read.asc"
    eyelink.write_text("MSG 1 X\n", encoding="utf-8")
