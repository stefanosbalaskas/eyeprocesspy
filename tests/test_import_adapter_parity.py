...
def test_generic_mapping_produces_canonical_tables_like_r():
    ...
    m = ep.eye_mapping(
        participant="id", recording="rec", timestamp="t", x="gx", y="gy", pupil_left="pupil"
    )
...
def test_generic_inference_and_time_units():
    ...
    np.testing.assert_allclose(x["gaze_samples"].timestamp_seconds, [0, 0.01])
...
def test_generic_events_responses_biometrics():
    ...
    m = ep.eye_mapping(
        ...
        response="answer",
        score="score",
        biometric_channels={"heart_rate": "hr"},
    )
...
def test_adapter_registry_detection_and_read(tmp_path: Path):
    ...
    assert detected.iloc[0]["confidence"] == pytest.approx(0.1)
    ...
    x = ep.read_eye_export(
        p, vendor="generic", mapping=ep.eye_mapping(timestamp="t", x="x", y="y"), quiet=True
    )
    assert ep.is_eye_dataset(x)
...
def test_custom_adapter_tie_priority_and_unregister(tmp_path: Path):
    ...
    ep.register_eye_adapter(
        "unit_b", lambda path, inspect_rows=20: 0.8, reader, priority=7, overwrite=True
    )
    det = ep.detect_eye_format(p, candidates=["unit_a", "unit_b"])
    assert list(det["format"]) == ["unit_b", "unit_a"]
    ep.unregister_eye_adapter("unit_a")
    ep.unregister_eye_adapter("unit_b")
