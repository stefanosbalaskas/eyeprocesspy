...
def test_simulation_and_response_matrix_contracts():
    a = ep.simulate_eye_dataset(n_person=8, n_item=5, sampling_rate=20, trial_duration=0.5, seed=5)
    b = ep.simulate_eye_dataset(n_person=8, n_item=5, sampling_rate=20, trial_duration=0.5, seed=5)
    ...
    y = ep.response_matrix(a)
    rt = ep.response_time_matrix(a)
...
def test_rasch_glm_two_stage_dif_and_model_extraction():
    x = ep.simulate_eye_dataset(
        n_person=20, n_item=6, sampling_rate=10, trial_duration=0.3, samples_per_trial=5, seed=9
    )
    ...
    group_by_person = {
        p: ("A" if i < 10 else "B")
        for i, p in enumerate(sorted(x["responses"].participant_id.unique()))
    }
...
def test_feature_models_process_irt_strategy_and_missingness():
    ...
    assert (
        ep.fit_gaze_informed_irt(x, gaze_features=("gaze_feature",), engine="glm").engine == "glm"
    )
    assert (
        ep.fit_pupil_informed_irt(x, pupil_features=("pupil_feature",), engine="glm").engine
        == "glm"
    )
    assert (
        ep.fit_multimodal_irt(
            x, gaze_features=("gaze_feature",), pupil_features=("pupil_feature",), engine="glm"
        ).engine
        == "glm"
    )
    assert (
        ep.fit_gaze_weighted_choice(x, dwell_features=("gaze_feature",), engine="glm").engine
        == "glm"
    )

    shared = ep.fit_shared_process_factor(
        x, features=("gaze_feature", "pupil_feature"), n_factors=1
    )
    ...
    strategy = ep.fit_strategy_mixture(
        x, features=("gaze_feature", "pupil_feature"), centers=2, seed=1
    )
...
def test_joint_dynamic_diffusion_functional_pupil_and_recovery():
    ...
    joint = ep.fit_joint_process_model(
        x, "score ~ gaze_feature", "log_response_time ~ gaze_feature", engine="separate"
    )
    assert joint.experimental and joint.engine == "separate"
    dyn = ep.fit_dynamic_aoi_model(x, source="samples", smoothing=0.5)
    ...
    recovery = ep.parameter_recovery(
        ...
        estimator=lambda sim: {"estimate": sim["truth"] + 0.1},
    ...
    np.testing.assert_allclose(recovery.bias, 0.1)
...
def test_external_r_engine_paths_are_explicit_gates():
    x = ep.simulate_eye_dataset(
        n_person=8, n_item=5, sampling_rate=10, samples_per_trial=5, seed=77
    )
    for call in [
...
def test_power_process_simulation_small_smoke():
    out = ep.power_process_simulation(n_person=8, n_item=4, effect=0.2, replications=2, seed=10)
    assert len(out) == 1
