...
def test_process_irt_07_frozen_exports_present():
    names = [
        x.strip()
        for x in (Path(__file__).parent / "fixtures" / "process_irt_07_exports.txt")
        .read_text()
        .splitlines()
        if x.strip()
    ]
...
def test_channels_registry_and_gating_contract():
    ...
    custom = ep.irt_model_spec(
        id="custom_validation_model",
        latent="ability",
        channels={"response": ep.irt_response_channel("2pl")},
    )
...
def test_missingness_classification_preserves_process_distinctions():
    d = pd.DataFrame(
        {
            "response": [1, np.nan, np.nan, np.nan, np.nan],
            "reached": [True, False, True, True, True],
            "inspected": [True, False, False, True, True],
            "started": [True, False, False, False, True],
        }
    )
    ...
    d = pd.DataFrame(
        {
            ...
            "item_id": [f"i{i % 4}" for i in range(n)],
        }
    )
    d["gA"] = rng.gamma(2, 1, n)
    d["gB"] = rng.gamma(2, 1, n)
    d["gC"] = rng.gamma(2, 1, n)
    logits = np.column_stack([0.4 * d.gA - 0.2 * d.gB, 0.3 * d.gB, 0.3 * d.gC])
    probs = np.exp(logits - logits.max(axis=1)[:, None])
    probs = probs / probs.sum(axis=1)[:, None]
    cats = np.array(["A", "B", "C"])
    d["response_option"] = [rng.choice(cats, p=p) for p in probs]
    fit = ep.fit_nominal_gaze_irt(d, option_gaze=["gA", "gB", "gC"])
    info = ep.option_process_information(fit)
    mp = ep.distractor_process_map(fit)
...
def test_censored_normal_calibration_and_prediction_dimensions():
    ...
    X = pd.DataFrame(
        {
            "item1": np.clip(0.5 + 0.12 * theta + rng.normal(0, 0.08, 60), 0, 1),
            "item2": np.clip(0.4 + 0.18 * theta + rng.normal(0, 0.10, 60), 0, 1),
        }
    )
...
def test_channel_ablation_is_directional_and_explicit():
    ...
    z = ep.process_channel_ablation(
        d,
        {"a": ["a"], "b": ["b"]},
        lambda data, active_columns, channel_name: len(active_columns),
        baseline=["y"],
    )
...
def test_multiple_response_combinations_and_local_dependence():
    ...
    p = r + rng.normal(scale=0.25, size=(100, 4))
    aud = ep.audit_process_local_dependence(
        pd.DataFrame(r, columns=["i1", "i2", "i3", "i4"]), p, threshold=0.20
    )
...
def test_multiple_response_reference_is_explicitly_non_exact():
    ...
    for pid, t in theta.items():
        for item in [f"i{x}" for x in range(1, 5)]:
            for opt in ["A", "B"]:
                eta = -0.3 + 0.8 * t + (0.25 if opt == "A" else -0.25)
...
def test_changepoints_recalibration_and_manyfacet_contracts():
    ...
    for p in ["p1", "p2"]:
        for order in range(1, 31):
            ...
            rows.append(
                {
                    "participant_id": p,
                    "item_id": f"i{(order - 1) % 5 + 1}",
                    "item_order": order,
                    "response": rng.binomial(1, 0.75 if not shifted else 0.35),
                    "rt": rng.lognormal(1.0 if not shifted else 1.6, 0.15),
                    "fixation_count": rng.poisson(3 if not shifted else 7),
                    "device": "d1" if p == "p1" else "d2",
                }
            )
    d = pd.DataFrame(rows)
    ...
    rr = ep.recalibrate_after_changepoint(
        d,
        lambda x: {"n": len(x)},
        gaze="fixation_count",
        min_segment=5,
    ...
    mf = ep.fit_manyfacet_process_irt(
        d, response="response", process="fixation_count", device="device"
    )
    ef = ep.facet_effects(mf, "process")
    inv = ep.audit_process_measurement_invariance(mf, "process")
...
def test_process_dependent_discrimination_and_generalizability():
    ...
    d = pd.DataFrame(
        {
            "participant_id": [f"p{i % 30}" for i in range(n)],
            "item_id": [f"i{i % 6}" for i in range(n)],
        }
    )
    d["theta"] = rng.normal(size=n)
    d["process"] = np.exp(0.2 * d.theta + rng.normal(scale=0.4, size=n))
    pr = 1 / (1 + np.exp(-(-0.2 + 0.9 * d.theta + 0.25 * d.theta * np.log(d.process))))
    d["response"] = rng.binomial(1, pr)
    fit = ep.process_dependent_discrimination_audit(
        d, "response", "theta", "process", "participant_id", "item_id", nonlinear=False
    )
    ...
    assert math.isclose(
        float(gs.variance_components.proportion.sum()), 1.0, rel_tol=1e-7, abs_tol=1e-7
    )
...
def test_process_irt_07_plots_return_axes_with_data():
    ...
    ab = ep.process_channel_ablation(
        ...
        {"a": ["a"]},
        lambda d, active, name: len(active),
        baseline=["y"],
    )
    ax = ep.plot_eye_process_channel_ablation(ab)
    assert len(ax.gp3_data) == 1
    plt.close(ax.figure)
    rng = np.random.default_rng(2)
    R = pd.DataFrame(rng.normal(size=(40, 3)), columns=["a", "b", "c"])
    ld = ep.audit_process_local_dependence(R)
    ax = ep.plot_eye_process_local_dependence_audit(ld)
    assert len(ax.gp3_data) == 3
    plt.close(ax.figure)
    d = pd.DataFrame(
        {
            "participant_id": [f"p{i % 10}" for i in range(80)],
            "item_id": [f"i{i % 4}" for i in range(80)],
            "theta": rng.normal(size=80),
            "process": rng.lognormal(size=80),
        }
    )
    d["response"] = rng.binomial(1, 1 / (1 + np.exp(-d.theta)))
    pdx = ep.process_dependent_discrimination_audit(
        d, "response", "theta", "process", "participant_id", "item_id", nonlinear=False
    )
    ax = ep.plot_eye_process_dependent_discrimination(pdx)
    assert len(ax.lines) == 3
    plt.close(ax.figure)
    gs = ep.generalizability_process_study(d, "process", ["participant_id", "item_id"])
    ax = ep.plot_eye_process_g_study(gs)
    assert len(ax.gp3_data) >= 3
    plt.close(ax.figure)
