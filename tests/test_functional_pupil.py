                ...
                rows.append(
                    {
                        ...
                        "time_ms": time,
                        "event_time": 1200,
                        "pupil": 3 + 0.02 * s + 0.08 * score + 0.02 * pi,
                        "score": score,
                        "response_time": 1.0 + 0.03 * ji,
                    }
                )
...
def test_basis_grid_and_advanced_validation_contracts():
    ...
    grid = ep.pupil_preprocessing_grid(
        baseline_windows=((-200, 0), (-100, 0)),
        latency_ms=(100, 200),
        basis_df=(4, 6),
        baseline_methods=("subtract", "percent"),
        max_interpolated_fraction=(0.1, 0.2),
    )
...
def test_functional_plots_and_sensitivity_prepare_only():
    ...
    try:
        assert hasattr(ax, "gp3_data") and len(ax.gp3_data) > 0
    finally:
        plt.close(ax.figure)
    diag = ep.functional_pupil_diagnostics(fit)
    ...
    try:
        assert hasattr(ax, "gp3_data") and len(ax.gp3_data) == 2
    finally:
        plt.close(ax.figure)


def test_advanced_simulator_layers_and_guards():
    sim = ep.simulate_advanced_process_data(
        n_person=20,
        n_item=8,
        n_time=12,
        n_states=4,
        ability_speed_correlation=0.35,
        gaze_effect=0.4,
        feature_reliability=0.65,
        missing_process=0.25,
        state_misclassification=0.30,
        pupil_ar1=0.5,
        luminance_effect=0.4,
        dif_effect=0.3,
        local_dependence=0.2,
        seed=205,
    )
    ...
    assert {"ability_speed_correlation", "gaze_effect", "feature_reliability", "pupil_ar1"} <= set(
        sim.truth
    )
...
def test_stan_engine_is_explicit_optional_backend():
    ...
    try:
        ...
        with pytest.raises(ep.EyeProcessBackendError):
            ep.fit_functional_pupil_stan(prepared)
