...
function Resolve-PythonCommand {
    ...
    if ($RequestedVersion) {
        if (Get-Command py -ErrorAction SilentlyContinue) {
            & py "-$RequestedVersion" -c "import sys; assert sys.version_info >= (3,11)" *> $null
            if ($LASTEXITCODE -eq 0) { return @("py", "-$RequestedVersion") }
        }
    ...
    if (Get-Command py -ErrorAction SilentlyContinue) {
        foreach ($version in @("3.14", "3.13", "3.12", "3.11")) {
            & py "-$version" -c "import sys; assert sys.version_info >= (3,11)" *> $null
...
Write-Host "Using: $($python -join ' ')" -ForegroundColor Cyan
& $python[0] $python[1..($python.Count-1)] -m pip install --upgrade pip
& $python[0] $python[1..($python.Count-1)] -m pip install --upgrade $wheel.FullName
...
if ($extras.Count -gt 0) {
    ...
    & $python[0] $python[1..($python.Count-1)] -m pip install --upgrade @extras
}

Write-Host "Verifying eyeprocesspy..." -ForegroundColor Cyan
& $python[0] $python[1..($python.Count-1)] -c "import eyeprocesspy as ep; print('eyeprocesspy', ep.__version__, '| R reference', ep.__r_reference_version__)"
