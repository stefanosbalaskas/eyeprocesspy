...
def _read_delimited(
    path: str | Path, delimiter: str | None = None, encoding: str = "UTF-8", **kwargs: Any
) -> pd.DataFrame:
    p = Path(path)
    if not p.is_file():
...
def _map_column(
    data: pd.DataFrame, mapping: Mapping[str, Any], key: str, default: Any = np.nan
) -> pd.Series:
    col = mapping.get(key)
...
def validate_eye_mapping(
    mapping: Mapping[str, Any], data: pd.DataFrame | None = None, required=("timestamp", "x", "y")
):
...
def infer_eye_mapping(data: pd.DataFrame, vendor: str | None = None):
    ...
    return eye_mapping(
        ...
        recording=pick(
            "recording_id", "recording", "Recording name", "recording id", "Recording UUID"
        ),
        ...
        stimulus=pick(
            "stimulus_id", "stimulus", "MEDIA_ID", "Presented Stimulus name", "world_index"
        ),
...
def _make_recordings(
    ...
    recording.loc[missing] = (
        "rec_" + participant.loc[missing].fillna("<NA>") + "_" + session.loc[missing].fillna("<NA>")
    )
...
def _make_eye_samples(data, mapping, rec_vec, time_native, time_seconds, pupil_unit):
    ...
    for eye in ("left", "right"):
        ...
        x_key = f"{eye}_x"
        y_key = f"{eye}_y"
        valid_key = f"pupil_{eye}_valid"
        gaze_valid_key = f"{eye}_valid"
        ...
        if pvalid.isna().all():
            pvalid = pd.Series(
                np.isfinite(pupil.to_numpy(dtype=float)), index=data.index, dtype="boolean"
            )
        seq = (
            pd.Series(np.arange(1, len(data) + 1), index=data.index)
            .groupby(rec_vec.astype(str), sort=False)
            .cumcount()
            + 1
        )
        rows.append(
            pd.DataFrame(
                {
                    ...
                    "sample_id": rec_vec.astype("string")
                    + f"_eye_{eye}_"
                    + seq.map(lambda z: f"{z:09d}"),
                    ...
                    "gaze_origin_valid": _safe_logical(
                        _map_column(data, mapping, gaze_valid_key, pd.NA)
                    ),
                    "corneal_reflection_x": np.nan,
                    "corneal_reflection_y": np.nan,
                    "detector_method": pd.NA,
    ...
    return pd.DataFrame(
        {
            ...
            "timestamp_native": time_native.iloc[idx].to_numpy(),
            "timestamp_seconds": time_seconds.iloc[idx].to_numpy(),
            "event_type": _map_column(data, mapping, "event_type", "marker")
            .astype("string")
            .iloc[idx]
            .to_numpy(),
            "event_name": event_name.iloc[idx].to_numpy(),
            "event_value": _map_column(data, mapping, "event_value")
            .astype("string")
            .iloc[idx]
            .to_numpy(),
            "duration": np.nan,
...
def _make_responses(data, mapping, rec_vec):
    if not any(k in mapping for k in ("response", "score", "response_time")):
        return empty_eye_table("responses")
    trial = _as_character_id(_map_column(data, mapping, "trial"))
    item = _as_character_id(_map_column(data, mapping, "item"))
    participant = _as_character_id(_map_column(data, mapping, "participant", "P001"))
    response = _map_column(data, mapping, "response").astype("string")
    score = _safe_numeric(_map_column(data, mapping, "score"))
    rt = _safe_numeric(_map_column(data, mapping, "response_time"))
    keep = response.notna() | np.isfinite(score) | np.isfinite(rt)
    if not bool(keep.any()):
        return empty_eye_table("responses")
    tmp = pd.DataFrame(
        {
            "recording_id": rec_vec[keep].astype("string"),
            "participant_id": participant[keep],
            "trial_id": trial[keep],
            "item_id": item[keep],
            "response": response[keep],
            "score": score[keep],
            "response_time": rt[keep],
        }
    )
    tmp = tmp.drop_duplicates(
        subset=["recording_id", "trial_id", "item_id"], keep="last"
    ).reset_index(drop=True)
    tmp["response_id"] = [f"{r}_response_{i:07d}" for i, r in enumerate(tmp.recording_id, 1)]
    tmp["response_timestamp"] = np.nan
    tmp["response_type"] = np.where(
        np.isfinite(pd.to_numeric(tmp.score, errors="coerce")), "scored", "observed"
    )
...
def _make_biometrics(data, mapping, rec_vec, time_native, time_seconds):
    ...
    if not channels:
        return empty_eye_table("biometrics")
    if not isinstance(channels, Mapping):
        raise ValueError(
            "`biometric_channels` must be a named mapping of canonical channels to source columns."
        )
    rows = []
    for channel, col in channels.items():
        ...
        rows.append(
            pd.DataFrame(
                {
                    "recording_id": rec_vec.astype("string"),
                    "stream_id": rec_vec.astype("string") + "_" + str(channel),
                    "timestamp_native": time_native,
                    "timestamp_seconds": time_seconds,
                    "channel": str(channel),
                    "value": value,
                    "unit": _canonical_biometric_unit(channel),
...
def _make_streams(
    recordings, coordinate_space_id, time_unit, eyes=(), pupil_unit=pd.NA, vendor="generic"
):
    base = pd.DataFrame(
        {
            "stream_id": recordings.recording_id.astype("string") + "_gaze",
            "recording_id": recordings.recording_id.astype("string"),
    ...
    for eye in [e for e in ("left", "right") if e in set(map(str, eyes))]:
        d = base.copy()
        d["stream_id"] = d.recording_id.astype("string") + f"_pupil_{eye}"
        d["stream_type"] = f"pupil_{eye}"
        d["value_unit"] = pupil_unit
        d["coordinate_space_id"] = pd.NA
        rows.append(d)
...
def _make_biometric_streams(biometrics, recordings, time_unit, vendor="generic"):
    if biometrics.empty:
        return pd.DataFrame()
    keys = (
        biometrics[["recording_id", "stream_id", "channel", "unit"]]
        .drop_duplicates()
        .reset_index(drop=True)
    )
    rates = []
    for row in keys.itertuples(index=False):
        z = biometrics.loc[
            (biometrics.recording_id == row.recording_id) & (biometrics.stream_id == row.stream_id),
            "timestamp_seconds",
        ]
...
def read_eye_generic(
    ...
    **kwargs,
    ...
    data = (
        path.copy()
        if is_frame
        else _read_delimited(path, delimiter=delimiter, encoding=encoding, **kwargs)
    )
    data = data.reset_index(drop=True)
    if data.empty:
        raise ValueError("Input contains no rows.")
    if mapping is None:
        mapping = infer_eye_mapping(data, vendor=vendor)
    ...
    known = {
        ...
        "user_coordinates_3d",
        "headset_coordinates_3d",
        "gaze_direction_vector",
        "custom",
    }
    coords = new_coordinate_space(
        coord_id,
        space_type=coordinate_space if coordinate_space in known else "custom",
        width=screen_width,
        height=screen_height,
        reference_object="display" if "display" in coordinate_space else pd.NA,
    )
    recordings = _make_recordings(
        data, mapping, vendor, source_label, nominal_sampling_rate, screen_width, screen_height
    )
    ...
    if valid.isna().all():
        valid = pd.Series([True] * len(data), dtype="boolean")
    seq = pd.Series(np.arange(len(data))).groupby(rec_vec.astype(str), sort=False).cumcount() + 1
    gaze = pd.DataFrame(
        {
            "recording_id": rec_vec,
            "stream_id": rec_vec.astype("string") + "_gaze",
            "sample_id": rec_vec.astype("string") + "_sample_" + seq.map(lambda z: f"{z:09d}"),
            "timestamp_native": time_native,
            "timestamp_seconds": time_seconds,
            "gaze_x": _safe_numeric(_map_column(data, mapping, "x")).reset_index(drop=True),
            "gaze_y": _safe_numeric(_map_column(data, mapping, "y")).reset_index(drop=True),
            "gaze_z": _safe_numeric(_map_column(data, mapping, "z")).reset_index(drop=True),
            "azimuth_deg": np.nan,
            "elevation_deg": np.nan,
            "valid": valid,
            "confidence": _safe_numeric(_map_column(data, mapping, "confidence")).reset_index(
                drop=True
            ),
            "fixation_id_source": _as_character_id(
                _map_column(data, mapping, "fixation_id")
            ).reset_index(drop=True),
            "blink_id_source": _as_character_id(_map_column(data, mapping, "blink_id")).reset_index(
                drop=True
            ),
            "trial_id": _as_character_id(_map_column(data, mapping, "trial")).reset_index(
                drop=True
            ),
            "stimulus_id": _as_character_id(_map_column(data, mapping, "stimulus")).reset_index(
                drop=True
            ),
    ...
    streams = _make_streams(
        ...
        coord_id,
        time_unit,
        eyes=pd.unique(eye_samples.eye.dropna()) if not eye_samples.empty else (),
        pupil_unit=pupil_unit,
        vendor=vendor,
    )
    for rid in recordings.recording_id.astype(str):
        ...
        streams.loc[
            (streams.recording_id.astype(str) == rid) & (streams.stream_type == "gaze_combined"),
            "observed_rate_hz",
        ] = rate
    bstreams = _make_biometric_streams(biometrics, recordings, time_unit, vendor)
    if not bstreams.empty:
        streams = pd.concat([streams, bstreams], ignore_index=True, sort=False)
    ...
    out = add_provenance(
        ...
        "import_generic",
        "dataset",
        f"Imported {len(data)} rows; vendor={vendor}.",
        source_files=source_label,
        reversible=bool(keep_raw),
    )
