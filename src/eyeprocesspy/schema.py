    ...
    [
        ...
        (
            ...
            [
                ...
                "gaze_origin_z",
                "gaze_origin_valid",
                "corneal_reflection_x",
                "corneal_reflection_y",
                "detector_method",
                "confidence",
                "trial_id",
                "stimulus_id",
...
def standardize_eye_table(
    data: pd.DataFrame, name: str, keep_extra: bool = True, schema: Mapping | None = None
) -> pd.DataFrame:
...
def validate_eye_table(
    data: pd.DataFrame, name: str, strict: bool = False, schema: Mapping | None = None
) -> pd.DataFrame:
    ...
    for c in missing:
        rows.append(
            dict(
                ...
                table=name,
                field=c,
                message=f"Schema field `{c}` is absent.",
            )
        )
    if strict:
        for c in extra:
            rows.append(
                dict(
                    ...
                    table=name,
                    field=c,
                    message=f"Non-canonical field `{c}` is retained.",
                )
            )
...
def new_coordinate_space(
    ...
    width=np.nan,
    height=np.nan,
    reference_object=pd.NA,
    parent_space_id=pd.NA,
    transform_to_parent=pd.NA,
    ...
    return pd.DataFrame(
        [
            dict(
                ...
                origin=origin or d[0],
                x_unit=x_unit or d[1],
                y_unit=y_unit or d[2],
