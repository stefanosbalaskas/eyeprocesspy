...
def score_partial_response_pattern(
    model: Any, response_pattern: Any, method: str = "MAP", **kwargs: Any
) -> pd.DataFrame:
...
def score_response_stream(
    model: Any,
    response_pattern: Any,
    observed_order: Any = None,
    method: str = "MAP",
    **kwargs: Any,
) -> EyeResult:
    method = str(method).upper()
    ...
    if order.size == 0:
        raise EyeProcessValidationError(
            "No observed responses were supplied for streaming scoring."
        )
    ...
    for k, pos in enumerate(order, start=1):
        ...
        try:
            ...
            if len(fs):
                ...
                theta_se = (
                    float(pd.to_numeric(fs[se_cols[0]], errors="coerce").iloc[0])
                    if se_cols
                    else math.nan
                )
        ...
        rows.append(
            {
                "step": k,
                "item_position": int(pos),
                "latest_response": rp[pos - 1],
                "theta": theta,
                "theta_se": theta_se,
            }
        )
    return _result(
        ...
        caveat=(
            "Streaming scores are demonstrations/operational building blocks. High-stakes use requires "
...
def update_person_score(
    ...
    **kwargs: Any,
    ...
    if np.isnan(value):
        raise EyeProcessValidationError(
            "response must be non-missing when updating a person score."
        )
    rp[pos - 1] = value
    return {
        "pattern": rp,
        "score": score_partial_response_pattern(model, rp, method=method, **kwargs),
    }
...
def collect_validation_evidence(
    *args: Any, model_name: str | None = None, notes: Any = None, **evidence: Any
) -> EyeResult:
    if args:
    ...
    if unknown:
        warnings.warn(
            "Non-standard validation evidence slot(s): " + ", ".join(unknown),
            UserWarning,
            stacklevel=2,
        )
    session = {"python_version": platform.python_version(), "platform": platform.platform()}
    return _result(
        "eye_validation_bundle",
        model_name="unnamed_model" if model_name is None else str(model_name),
...
def validation_report(x: Any, include_session: bool = True) -> list[str]:
    ...
    lines = [
        f"eyeprocess validation report: {x.model_name}",
        "=" * (30 + len(x.model_name)),
        "",
        f"Created: {x.created_at}",
        "",
        "Evidence inventory",
        "------------------",
    ...
    for key, text in labels.items():
        if x.evidence.get(key) is not None:
            lines.append(text)
    if (
        x.evidence.get("external_validation") is not None
        or x.evidence.get("transportability") is not None
    ):
    ...
    if include_session:
        lines += [
            ...
            "Software provenance",
            "-------------------",
            f"Python version: {x.session['python_version']}",
            f"Platform: {x.session['platform']}",
        ]
    return lines
...
def export_validation_bundle(
    x: Any, directory: str | Path, overwrite: bool = False, include_rds: bool = True
) -> EyeResult:
    if getattr(x, "eyeprocess_class", None) != "eye_validation_bundle":
        raise EyeProcessValidationError("x must be eye_validation_bundle.")
    ...
    if directory.exists() and any(directory.iterdir()) and not overwrite:
        raise EyeProcessValidationError(
            "Target directory is not empty; set overwrite=True to continue."
        )
    directory.mkdir(parents=True, exist_ok=True)
    files: dict[str, str] = {}
    manifest = validation_bundle_manifest(x)
    mp = directory / "validation_bundle_manifest.csv"
    manifest.to_csv(mp, index=False)
    files["manifest"] = str(mp)
    ...
    jp = directory / "validation_bundle_manifest.json"
    jp.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    files["json_manifest"] = str(jp)
    if include_rds:
...
def preaction_process_features(
    data: Any,
    ...
    d = _df(data)
    by = list(by)
    _req(d, [*by, time, response_time])
    windows = np.unique(_num(windows_ms))
    windows = windows[np.isfinite(windows) & (windows > 0)]
    if windows.size == 0:
        raise EyeProcessValidationError(
            "windows_ms must contain at least one positive finite window."
        )
    global_aois = []
    if aoi in d:
        global_aois = sorted(
            pd.Series(d[aoi]).dropna().astype(str).loc[lambda s: s.str.len() > 0].unique()
        )
    rows = []
    for idx in _groups(d, by):
        z = d.iloc[idx]
        tt = _num(z[time])
        rtvals = _num(z[response_time])
        fin = rtvals[np.isfinite(rtvals)]
        if not fin.size:
            continue
        rt = float(fin[0])
        rel = tt - rt
        for w in windows:
            keep = np.isfinite(rel) & (rel >= -w) & (rel <= 0)
            zz = z.loc[keep]
            rr = rel[keep]
            if len(zz) < 3:
                continue
            ...
            row = {
                **_group_values(d, idx, by),
                "pre_window_ms": float(w),
                "n_samples": len(zz),
                "aoi_entropy": _entropy_factor(av),
                "aoi_switch_count": _switch_count(av),
                "pupil_mean": _mean(pv),
                "pupil_slope": _slope(pv, rr),
                "blink_prop": _mean(bv),
            }
            for lv in global_aois:
                good = pd.notna(av)
                row[f"aoi_prop__{str(lv).replace(' ', '.')}"] = (
                    float(np.mean(np.asarray(av[good], str) == lv)) if good.any() else math.nan
                )
            rows.append(row)
    return _result(
        "eye_preaction_process_features",
...
def addm_glam_proxy_features(
    data: Any,
    by: Sequence[str] = ("person_id", "trial_id"),
    time: str = "time_ms",
    aoi: str = "aoi",
    target_aoi: str = "target",
    distractor_aoi: str = "distractor",
    action_aoi: str = "button",
) -> EyeResult:
    d = _df(data)
    by = list(by)
    _req(d, [*by, time, aoi])
    ...
    for idx in _groups(d, by):
        z = d.iloc[idx]
        av = z[aoi].astype(str).to_numpy()
        tt = _num(z[time])
        evidence = np.where(av == target_aoi, 1.0, np.where(av == distractor_aoi, -1.0, 0.0))
        action = np.where(
            av == action_aoi, 1.0, np.where(np.isin(av, [target_aoi, distractor_aoi]), -1.0, 0.0)
        )
        tp, dp, ap = _mean(av == target_aoi), _mean(av == distractor_aoi), _mean(av == action_aoi)
        eps = 1e-6
        split = float(np.nanmedian(tt))
        early = evidence[tt < split]
        late = evidence[tt >= split]
        rows.append(
            {
                **_group_values(d, idx, by),
                "target_minus_distractor_prop": tp - dp,
                "evidence_slope": _slope(evidence, tt),
                "action_evidence_slope": _slope(action, tt),
                ...
                "choice_caution_proxy": _switch_count(av) / (abs(_mean(evidence)) + 0.01),
            }
        )
    return _result(
        "eye_decision_process_proxy",
...
def process_feature_family_registry() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "pattern": [
                "transition|switch|entropy|scanpath|sequence",
                "aoi|target|distractor|button|text",
                "pupil|phasic|tonic|ripa|frequency",
                "gaze|fixation|saccade|dwell|ttff",
                "valid|trackloss|missing|confidence|quality",
                "rt|response_time|trial_order",
                "condition|task|stimulus|layout|luminance",
            ],
            "family": [
                "Scanpath organization",
                "AOI attention",
                "Pupil dynamics",
                "Gaze dynamics",
                "Data quality",
...
def assign_process_feature_family(
    feature_names: Any, registry: pd.DataFrame | None = None
) -> np.ndarray:
    ...
    for feature in map(str, np.asarray(feature_names, dtype=object).reshape(-1)):
        hit = [
            i
            for i, pat in enumerate(reg.pattern.astype(str))
            if pd.Series([feature]).str.contains(pat, case=False, regex=True).iloc[0]
        ]
        out.append(str(reg.family.iloc[hit[0]]) if hit else "Other")
    return np.asarray(out, dtype=object)


def process_feature_stability(
    data: Any,
    feature: str = "feature",
    split: str = "split",
    ...
    d = _df(data)
    _req(d, [feature, split, importance])
    top_n = int(top_n)
    if top_n < 1:
        raise EyeProcessValidationError("top_n must be at least 1.")
    q = d[[feature, split, importance]].copy()
    q.columns = ["feature", "split", "importance"]
    q["importance"] = pd.to_numeric(q.importance, errors="coerce")
    q = q.dropna(subset=["feature", "split", "importance"])
    if q.empty:
        raise EyeProcessValidationError("No complete split/importance rows are available.")
    ...
    for f in pd.unique(q.feature):
        ...
        for z in groups:
            selected.append(
                f in z.sort_values("importance", ascending=False).head(top_n).feature.to_numpy()
            )
        rows.append(
            {
                "feature": f,
                "top_n_selection_rate": float(np.mean(selected)),
    ...
    return out.sort_values(
        ["top_n_selection_rate", "mean_importance"], ascending=False, kind="stable"
