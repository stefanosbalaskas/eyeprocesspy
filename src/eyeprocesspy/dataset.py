...
from .exceptions import EyeProcessSchemaError, EyeProcessValidationError
from .schema import (
    SCHEMA_VERSION,
...
def _now_utc() -> str:
    return datetime.now(UTC).astimezone(UTC).strftime("%Y-%m-%d %H:%M:%S %Z")
...
class EyeDataset(dict):
    ...
    def __repr__(self):
        rec = len(self.get("recordings", []))
        streams = len(self.get("streams", []))
        gaze = len(self.get("gaze_samples", []))
...
def new_eye_dataset(
    ...
    out = EyeDataset(
        ...
        vendor_metadata={} if vendor_metadata is None else vendor_metadata,
        schema_version=schema_version,
    )
    if validate:
        out.validation = validate_eye_dataset(out, strict=False)
    return out
...
def _assert_eye_dataset(x):
    if not is_eye_dataset(x):
        raise TypeError("`x` must be an EyeDataset.")
...
def validate_eye_dataset(x, strict=False, stop_on_error=False) -> pd.DataFrame:
    ...
    issues = (
        ...
        if issue_frames
        else pd.DataFrame(columns=["severity", "code", "table", "field", "message"])
    )
    rows = []
    primary = {
    ...
    for n, keys in primary.items():
        d = x[n]
        if d.empty or not all(k in d.columns for k in keys):
            continue
        ...
        if bool(missing.any()):
            rows.append(
                dict(
                    ...
                    table=n,
                    field="+".join(keys),
                    message=f"Missing values found in primary key `{' + '.join(keys)}`.",
                )
            )
        if bool((~missing).any()) and d.loc[~missing, keys].astype(str).duplicated().any():
            rows.append(
                dict(
                    severity="error",
                    ...
                    table=n,
                    field="+".join(keys),
                    message=f"Duplicate values found for primary key `{' + '.join(keys)}`.",
                )
            )
        ...
        if not d.empty and "recording_id" in d and recording_ids:
            ...
            if orphan:
                rows.append(
                    dict(
                        severity="error",
                        code="orphan_recording_id",
                        table=n,
                        field="recording_id",
                        message=f"Unknown recording id(s): {', '.join(orphan)}.",
                    )
                )
        ...
        if not d.empty and "stream_id" in d:
            ...
            if orphan:
                rows.append(
                    dict(
                        severity="error",
                        code="orphan_stream_id",
                        table=n,
                        field="stream_id",
                        message=f"Unknown stream id(s): {', '.join(orphan)}.",
                    )
                )
        ...
        if not d.empty and "coordinate_space_id" in d:
            ...
            if orphan:
                rows.append(
                    dict(
                        severity="error",
                        code="orphan_coordinate_space",
                        table=n,
                        field="coordinate_space_id",
                        message=f"Unknown coordinate space(s): {', '.join(orphan)}.",
                    )
                )
        ...
        if bool(bad.any()):
            rows.append(
                dict(
                    ...
                    message="Non-finite normalized timestamps detected.",
                )
            )
    if not x["intervals"].empty:
        s = pd.to_numeric(x["intervals"]["start_time"], errors="coerce")
        e = pd.to_numeric(x["intervals"]["end_time"], errors="coerce")
        ...
        if bool(bad.any()):
            rows.append(
                dict(
                    ...
                    table="intervals",
                    field="end_time",
                    message="At least one interval ends before it starts.",
                )
            )
    if rows:
        issues = pd.concat([issues, pd.DataFrame(rows)], ignore_index=True)
    ...
    if stop_on_error and bool((issues["severity"] == "error").any()):
        raise EyeProcessValidationError(
            f"`eye_dataset` validation failed with {int((issues['severity'] == 'error').sum())} error(s)."
        )
    return issues
...
def get_eye_table(x, table):
    ...
    if table not in x:
        raise EyeProcessSchemaError(f"Unknown component `{table}`.")
    return x[table]


def set_eye_table(x, table, value, validate=True):
    _assert_eye_dataset(x)
    if table not in canonical_table_names():
        raise EyeProcessSchemaError(f"Unknown canonical table `{table}`.")
    out = x.copy()
    out[table] = standardize_eye_table(value, table, keep_extra=True)
...
def add_provenance(
    ...
    _assert_eye_dataset(x)
    out = x.copy()
    if isinstance(source_files, (list, tuple, set)):
        source_string = "|".join(map(str, source_files))
    elif pd.isna(source_files):
        source_string = pd.NA
    else:
        source_string = str(source_files)
    ...
    if isinstance(file_hashes, (list, tuple, set)):
        hash_string = "|".join("NA" if h is None else str(h) for h in file_hashes)
    elif file_hashes is None:
        hash_string = ""
    else:
        hash_string = str(file_hashes)
    if isinstance(warnings, (list, tuple, set)):
        warn_string = " | ".join(map(str, warnings))
    elif pd.isna(warnings):
        warn_string = "NA"
    else:
        warn_string = str(warnings)
    row = pd.DataFrame(
        [
            dict(
                provenance_id=_next_id("prov"),
                timestamp=_now_utc(),
                action=str(action),
                component=component,
                details=details,
                source_files=source_string,
                file_hashes=hash_string,
                software=str(software),
                software_version=software_version or __import__("eyeprocesspy").__version__,
                reversible=bool(reversible),
                warnings=warn_string,
            )
        ]
    )
    out["provenance"] = standardize_eye_table(
        pd.concat([out["provenance"], row], ignore_index=True, sort=False), "provenance"
    )
    return out
...
def provenance_manifest(x):
    ...
    return {
        ...
        "created": _now_utc(),
        "sources": list(pd.unique(p["source_files"].dropna())),
        "file_hashes": list(pd.unique(p["file_hashes"].dropna())),
        "actions": p.copy(),
        "coordinate_spaces": x["coordinate_spaces"].copy(),
        "streams": x["streams"].copy(),
...
def compact_eye_dataset(x, drop_raw=False, drop_empty=False):
    ...
    if drop_raw:
        out.raw = []
    if drop_empty:
        out.empty_components = [n for n in canonical_table_names() if out[n].empty]
    return add_provenance(
        out,
        "compact_dataset",
        "dataset",
        f"drop_raw={str(bool(drop_raw)).upper()};drop_empty={str(bool(drop_empty)).upper()}",
        reversible=not drop_raw,
    )
