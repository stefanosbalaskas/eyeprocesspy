...
def _prepare_coordinates(
    ...
    *,
    ...
    gx, gy = _convert_xy(
        _numeric(d[x]), _numeric(d[y]), unit=unit, output_unit=out_unit, geometry=geometry
    )
...
def compute_gaze_accuracy(
    ...
    rows = []
    for g, z in _groups(d, keys):
        gx, gy, tx, ty, out_unit = _prepare_coordinates(
            z,
            x=x,
            y=y,
            target_x=target_x,
        ...
        rows.append(
            {
                **_header(keys, g),
                "n_accuracy_samples": int(ok.sum()),
                "n_accuracy_targets": int(n_targets),
    ...
    return _attach_provenance(
        out,
        {
            "function": "compute_gaze_accuracy",
            "input_unit": unit,
...
def compute_rms_s2s(
    ...
    rows = []
    for g, z in _groups(d, keys):
        gx, gy, _, _, out_unit = _prepare_coordinates(
            z, x=x, y=y, unit=unit, output_unit=output_unit, geometry=geometry
        )
        finite_pair = (
            np.isfinite(gx[:-1]) & np.isfinite(gy[:-1]) & np.isfinite(gx[1:]) & np.isfinite(gy[1:])
            if len(z) > 1
        ...
        rows.append(
            {
                **_header(keys, g),
                "n_steps": int(vals.size),
                "precision_rms_s2s": float(np.sqrt(np.mean(vals**2))) if vals.size else math.nan,
                "median_s2s": float(np.median(vals)) if vals.size else math.nan,
                "p95_s2s": float(np.quantile(vals, 0.95)) if vals.size else math.nan,
                "dimension": dimension,
                "unit": _unit_linear(out_unit),
    ...
    return _attach_provenance(
        pd.DataFrame(rows),
        {
            "function": "compute_rms_s2s",
            "missing_gap_policy": "never_bridge",
            "max_gap_ms": max_gap_ms,
        },
    )
...
def compute_gaze_sd_precision(
    ...
    for g, z in _groups(d, keys):
        ...
        xx, yy = gx[ok], gy[ok]
        sx = float(np.std(xx, ddof=0)) if xx.size else math.nan
        sy = float(np.std(yy, ddof=0)) if yy.size else math.nan
        rows.append(
            {
                **_header(keys, g),
                "n_precision_samples": int(ok.sum()),
                "precision_sd_x": sx,
                "precision_sd_y": sy,
                "precision_sd": float(math.hypot(sx, sy))
...
def compute_bcea(
    ...
    for g, z in _groups(d, keys):
        ...
        rows.append(
            {
                **_header(keys, g),
                "n_bcea_samples": int(ok.sum()),
                "bcea": area,
                "bcea_probability": p,
                "sd_x": sx,
                "sd_y": sy,
                "correlation_xy": rho,
                "unit": _unit_area(out_unit),
            }
        )
...
def compute_gaze_precision(data: Any, **kwargs: Any) -> dict[str, pd.DataFrame]:
    ...
    probability = kwargs.pop("probability", 0.68)
    rms_keys = {
        k: v
    ...
    spatial_keys = {
        k: v for k, v in kwargs.items() if k in {"x", "y", "by", "unit", "output_unit", "geometry"}
    }
    return {
        "rms_s2s": compute_rms_s2s(data, **rms_keys),
        "sd": compute_gaze_sd_precision(data, **spatial_keys),
        "bcea": compute_bcea(data, probability=probability, **spatial_keys),
    }
...
def estimate_sampling_interval(
    ...
    d = _df(data)
    keys = _by_list(by)
    _require(d, [time, *keys])
    if time_unit not in _TIME_SCALES:
        raise ValueError("time_unit must be one of s, ms, us, ns")
    rows = []
    for g, z in _groups(d, keys):
        t = _numeric(z[time]) * _TIME_SCALES[time_unit]
        finite = t[np.isfinite(t)]
        dt = np.diff(finite) * 1000 if finite.size > 1 else np.array([], float)
        positive = dt[dt > 0]
        rows.append(
            {
                **_header(keys, g),
                "n_observed_timestamps": int(finite.size),
                "n_intervals": int(positive.size),
                "median_interval_ms": float(np.median(positive)) if positive.size else math.nan,
                "mean_interval_ms": float(np.mean(positive)) if positive.size else math.nan,
                "min_interval_ms": float(np.min(positive)) if positive.size else math.nan,
                "max_interval_ms": float(np.max(positive)) if positive.size else math.nan,
                "duplicate_timestamp_count": int(np.sum(dt == 0)),
                "non_monotonic_timestamp_count": int(np.sum(dt < 0)),
            }
        )
    return _attach_provenance(
        pd.DataFrame(rows), {"function": "estimate_sampling_interval", "time_unit": time_unit}
    )


def estimate_sampling_jitter(
    data: Any,
    time: str = "timestamp_ms",
    by: str | Sequence[str] | None = None,
    time_unit: str = "ms",
    ...
    for g, z in _groups(d, keys):
        ...
        rows.append(
            {
                **_header(keys, g),
                "sampling_jitter_ms": float(np.std(pos - med, ddof=1))
                if pos.size > 1
                else math.nan,
                "sampling_jitter_mad_ms": float(np.median(np.abs(pos - med)))
                if pos.size
                else math.nan,
    ...
    return _attach_provenance(
        pd.DataFrame(rows),
        {
            "function": "estimate_sampling_jitter",
            "definition": "SD of positive inter-sample intervals around their median",
        },
    )
...
def estimate_effective_sampling_rate(
    ...
    dropped_interval_factor: float = 1.5,
    x: str | None = None,
    y: str | None = None,
    valid: str | None = None,
) -> pd.DataFrame:
    d = _df(data)
    keys = _by_list(by)
    _require(d, [time, x, y, valid, *keys])
    if (x is None) != (y is None):
        raise ValueError("x and y must either both be supplied or both be omitted")
    if valid is not None and x is None:
        raise ValueError("x and y are required when valid is supplied")
    if time_unit not in _TIME_SCALES:
        raise ValueError("time_unit must be one of s, ms, us, ns")
    if nominal_sampling_hz is not None and (
        not math.isfinite(float(nominal_sampling_hz)) or float(nominal_sampling_hz) <= 0
    ):
        raise ValueError("nominal_sampling_hz must be positive")
    if not math.isfinite(float(dropped_interval_factor)) or float(dropped_interval_factor) <= 1:
        raise ValueError("dropped_interval_factor must be > 1")
    rows = []
    for g, z in _groups(d, keys):
        t = _numeric(z[time]) * _TIME_SCALES[time_unit]
        finite_t = np.isfinite(t)
        finite = t[finite_t]
        dt = np.diff(finite) if finite.size > 1 else np.array([], float)
        pos = dt[dt > 0]
        med = float(np.median(pos)) if pos.size else math.nan
        span = float(finite[-1] - finite[0]) if finite.size > 1 else math.nan
        duration = (
            span + med
            if math.isfinite(span) and math.isfinite(med) and med > 0
            else (
                1 / float(nominal_sampling_hz)
                if finite.size == 1 and nominal_sampling_hz is not None
                else math.nan
            )
        )
        if x is not None and y is not None:
            ...
            effective_count = int(finite.size)
            count_rule = "finite timestamps"
        hz = (
            float(effective_count / duration)
        ...
        rows.append(
            {
                **_header(keys, g),
                "observed_sample_count": int(finite.size),
                ...
                "dropped_interval_count": dropped,
                "nominal_sampling_hz": nominal_sampling_hz,
                "effective_count_rule": count_rule,
            }
        )
    return _attach_provenance(
        pd.DataFrame(rows),
        {
            "function": "estimate_effective_sampling_rate",
...
def _valid_masks(
    d: pd.DataFrame, x: str, y: str, valid: str | None
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    gx, gy = _numeric(d[x]), _numeric(d[y])
    missing = ~np.isfinite(gx) | ~np.isfinite(gy)
    if valid is None:
        explicit = np.ones(len(d), dtype=bool)
    else:
        s = d[valid]
        if pd.api.types.is_bool_dtype(s.dtype):
            explicit = s.fillna(False).to_numpy(bool)
        else:
            vals = pd.to_numeric(s, errors="coerce").to_numpy(float)
            explicit = np.isfinite(vals) & (vals > 0)
    good = (~missing) & explicit
    invalid = (~missing) & (~explicit)
    return good, invalid, missing


def compute_valid_sample_fraction(
    data: Any,
    x: str = "gaze_x",
    y: str = "gaze_y",
    valid: str | None = None,
    ...
    d = _df(data)
    keys = _by_list(by)
    _require(d, [x, y, valid, *keys])
    rows = []
    for g, z in _groups(d, keys):
        good, invalid, missing = _valid_masks(z, x, y, valid)
        n = len(z)
        rows.append(
            {
                **_header(keys, g),
                "n_samples": n,
                "valid_sample_fraction": float(good.mean()) if n else math.nan,
                "invalid_sample_fraction": float(invalid.mean()) if n else math.nan,
                "missing_sample_fraction": float(missing.mean()) if n else math.nan,
            }
        )
    return _attach_provenance(
        pd.DataFrame(rows),
        {"function": "compute_valid_sample_fraction", "validity_rule": valid or "finite x and y"},
    )
    ...
    for g, z in _groups(d, keys):
        ...
        for i, flag in enumerate(lost):
            ...
            if start is not None and (not flag or i == n - 1):
                ...
                start = None
        longest_samples = max((b - a + 1 for a, b in runs), default=0)
        longest_ms = math.nan
        if time is not None and runs:
            ...
            pos = np.diff(t)
            typical = (
                float(np.median(pos[np.isfinite(pos) & (pos > 0)]))
                if np.any(np.isfinite(pos) & (pos > 0))
                else 0.0
            )
            for a, b in runs:
                if np.isfinite(t[a]) and np.isfinite(t[b]):
                    durations.append(max(0.0, float(t[b] - t[a])) + typical)
        ...
        row = {
            **_header(keys, g),
            ...
            "invalid_sample_fraction": float(invalid.mean()) if n else math.nan,
            "missing_sample_fraction": float(missing.mean()) if n else math.nan,
            "data_loss_fraction": float(lost.mean()) if n else math.nan,
            "missing_run_count": len(runs),
            "longest_missing_run_samples": longest_samples,
            "longest_missing_run_ms": longest_ms,
        }
        if missing_reason is not None:
            reasons = z.loc[lost, missing_reason].fillna("unknown").astype(str).str.lower()
            denom = max(1, int(lost.sum()))
            for reason, count in reasons.value_counts().sort_index().items():
                row[f"missing_reason_{reason}_fraction"] = float(count / denom)
        rows.append(row)
    return _attach_provenance(
        pd.DataFrame(rows),
        {
            "function": "compute_gaze_data_loss",
            "loss_rule": "missing coordinates or explicit invalidity",
        },
    )
...
def summarise_spatial_quality(data: Any, **kwargs: Any) -> pd.DataFrame:
    keys = _by_list(kwargs.get("by"))
    acc = compute_gaze_accuracy(
        data,
        **{
            k: v
            for k, v in kwargs.items()
            if k in {"x", "y", "target_x", "target_y", "by", "unit", "output_unit", "geometry"}
        },
    )
    rms = compute_rms_s2s(
        data,
    ...
    sd = compute_gaze_sd_precision(
        data,
        **{
            k: v
            for k, v in kwargs.items()
            if k in {"x", "y", "by", "unit", "output_unit", "geometry"}
        },
    )
    bcea = compute_bcea(
        data,
        probability=kwargs.get("probability", 0.68),
        **{
            k: v
            for k, v in kwargs.items()
            if k in {"x", "y", "by", "unit", "output_unit", "geometry"}
...
def summarise_sampling_quality(data: Any, **kwargs: Any) -> pd.DataFrame:
    keys = _by_list(kwargs.get("by"))
    inter = estimate_sampling_interval(
        data, **{k: v for k, v in kwargs.items() if k in {"time", "by", "time_unit"}}
    )
    jit = estimate_sampling_jitter(
        data, **{k: v for k, v in kwargs.items() if k in {"time", "by", "time_unit"}}
    )
    eff = estimate_effective_sampling_rate(
        data,
        **{
            k: v
            for k, v in kwargs.items()
...
def _validate_thresholds(thresholds: Mapping[str, Any] | None, columns: Sequence[str]) -> None:
    ...
    if unknown:
        raise ValueError(
            f"threshold metrics are not present in the quality report: {', '.join(unknown)}"
        )
    for metric, rule in thresholds.items():
        if isinstance(rule, Mapping):
            ...
            if bad or not rule:
                raise ValueError(
                    f"threshold rule for {metric} must contain only 'min' and/or 'max'"
                )
...
def _threshold_flags(row: pd.Series, thresholds: Mapping[str, Any] | None) -> list[str]:
    ...
    if not thresholds:
        return flags
    for metric, rule in thresholds.items():
        if not pd.notna(row[metric]):
            continue
        value = float(row[metric])
        if isinstance(rule, Mapping):
            if "max" in rule and value > float(rule["max"]):
                flags.append(f"{metric}>max")
            if "min" in rule and value < float(rule["min"]):
                flags.append(f"{metric}<min")
        else:
            if value > float(rule):
                flags.append(f"{metric}>max")
    return flags
...
def create_gaze_quality_report(
    ...
    y: str = "gaze_y",
    time: str = "timestamp_ms",
    target_x: str | None = "target_x",
    target_y: str | None = "target_y",
    valid: str | None = None,
    missing_reason: str | None = None,
    by: str | Sequence[str] | None = None,
    unit: str = "degrees",
    ...
    preprocessing_spec: Any = None,
    event_detector: Any = None,
    aoi_specification: Any = None,
    ...
    has_targets = (
        ...
        and target_x in d.columns
        and target_y in d.columns
    )
    validation = validate_gaze_quality_inputs(
        d,
        x=x,
        y=y,
        time=time,
        target_x=target_x if has_targets else None,
        target_y=target_y if has_targets else None,
        by=keys,
        unit=unit,
        time_unit=time_unit,
        unit_column=resolved_unit_column,
    )
    if has_targets:
        spatial = summarise_spatial_quality(
            d,
            x=x,
            y=y,
            time=time,
        ...
        sd = compute_gaze_sd_precision(
            d, x=x, y=y, by=keys, unit=unit, output_unit=output_unit, geometry=geometry
        )
        bc = compute_bcea(
            d,
            x=x,
    ...
    sampling = summarise_sampling_quality(
        d,
        time=time,
        by=keys,
        time_unit=time_unit,
    ...
    issue_map = {
        _group_token([item.get(k) for k in keys]): item["issues"]
        for item in validation["group_issues"]
    }
    flags = []
    for _, row in report.iterrows():
        rf = []
        key = _group_token([row.get(k) for k in keys])
        rf.extend(issue_map.get(key, []))
        if (
            "n_accuracy_targets" in row
            and pd.notna(row.get("n_accuracy_targets"))
            and float(row["n_accuracy_targets"]) > 1
        ):
            rf.append("mixed_accuracy_targets")
        if (
            "n_bcea_samples" in row
            and pd.notna(row.get("n_bcea_samples"))
            and float(row["n_bcea_samples"]) < 2
        ):
            rf.append("insufficient_bcea_samples")
        if "n_steps" in row and pd.notna(row.get("n_steps")) and float(row["n_steps"]) < 1:
            rf.append("insufficient_rms_pairs")
        if (
            "valid_sample_fraction" in row
            and pd.notna(row.get("valid_sample_fraction"))
            and float(row["valid_sample_fraction"]) <= 0
        ):
            rf.append("no_valid_gaze_samples")
        rf.extend(_threshold_flags(row, thresholds))
        flags.append(sorted(set(rf)))
    ...
    provenance = {
        ...
        "quality_rules": quality_rules if quality_rules is not None else thresholds,
...
def _compare_reports(data: Any, factor: str, **kwargs: Any) -> pd.DataFrame:
    if factor not in _df(data).columns:
        raise ValueError(f"data is missing required column: {factor}")
    by = _by_list(kwargs.pop("by", None))
    by = list(dict.fromkeys([*by, factor]))
    return create_gaze_quality_report(data, by=by, **kwargs)


def compare_gaze_quality_sessions(
    data: Any, session: str = "session_id", **kwargs: Any
) -> pd.DataFrame:
    return _compare_reports(data, session, **kwargs)


def compare_gaze_quality_conditions(
    data: Any, condition: str = "condition", **kwargs: Any
) -> pd.DataFrame:
...
def plot_gaze_accuracy(report: Any, metric: str = "accuracy_mean", ax: Any = None):
    ...
    _require(d, [metric])
    ax = _ax(ax)
    ax.plot(np.arange(len(d)), pd.to_numeric(d[metric], errors="coerce"), marker="o")
    ax.set(ylabel=metric, xlabel="analysis unit", title="Gaze accuracy")
...
def plot_gaze_precision(report: Any, metric: str = "precision_rms_s2s", ax: Any = None):
    ...
    ax = _ax(ax)
    ax.plot(np.arange(len(d)), pd.to_numeric(d[metric], errors="coerce"), marker="o")
    ax.set(ylabel=metric, xlabel="analysis unit", title="Gaze precision")
    ax.eyeprocess_plot_data = d
    return ax
...
def plot_bcea(report: Any, ax: Any = None):
    ...
    _require(d, ["bcea"])
    ax = _ax(ax)
    ax.bar(np.arange(len(d)), pd.to_numeric(d.bcea, errors="coerce"))
    ax.set(ylabel="BCEA", xlabel="analysis unit", title="Bivariate contour ellipse area")
    ax.eyeprocess_plot_data = d
    return ax


def plot_sampling_intervals(
    data: Any, time: str = "timestamp_ms", time_unit: str = "ms", ax: Any = None
):
    d = _df(data)
    _require(d, [time])
    if time_unit not in _TIME_SCALES:
        raise ValueError("time_unit must be one of s, ms, us, ns")
    t = _numeric(d[time]) * _TIME_SCALES[time_unit]
    dt = np.diff(t) * 1000
    ax = _ax(ax)
    ax.plot(np.arange(1, len(dt) + 1), dt, marker=".")
    ax.set(ylabel="interval (ms)", xlabel="interval", title="Sampling intervals")
    ax.eyeprocess_plot_data = pd.DataFrame({"interval_ms": dt})
    return ax
...
def plot_gaze_quality_dashboard(report: Any):
    ...
    metrics = [
        ...
        ("precision_rms_s2s", "RMS-S2S precision"),
        ("bcea", "BCEA"),
        ("valid_sample_fraction", "Valid fraction"),
    ]
    for ax, (m, title) in zip(axes.flat, metrics, strict=True):
        if m in d:
            ax.bar(np.arange(len(d)), pd.to_numeric(d[m], errors="coerce"))
            ax.set_title(title)
            ax.set_xlabel("analysis unit")
        else:
...
def report_gaze_quality(report: Any, digits: int = 3) -> str:
    ...
    d = _df(report)
    if d.empty:
        return "No gaze-quality rows were available."
    cols = [
        c
        for c in [
            "accuracy_mean",
    ...
    for c in cols:
        ...
        if len(v):
            parts.append(
                f"{c}: mean {v.mean():.{digits}f}, range {v.min():.{digits}f}–{v.max():.{digits}f}"
            )
    n_review = (
        int(pd.Series(d.get("review_required", False)).fillna(False).astype(bool).sum())
        if len(d)
        else 0
    )
    return (
        "; ".join(parts)
        + f". Review required for {n_review}/{len(d)} analysis units. Thresholds, when supplied, are study-specific review rules and never trigger automatic exclusion."
    )


def simulate_gaze_quality_calibration(
    seed: int = 20260918, samples_per_target: int = 18, nominal_sampling_hz: float = 60.0
) -> pd.DataFrame:
    ...
    try:
        ...
        raise ValueError(
            "seed, samples_per_target, and nominal_sampling_hz must be numeric"
        ) from exc
    if (
        not math.isfinite(seed_float)
        or not seed_float.is_integer()
        or seed_float < 0
        or seed_float > 2_147_483_647
    ):
    ...
    targets = [(-5, -5), (0, -5), (5, -5), (-5, 0), (0, 0), (5, 0), (-5, 5), (0, 5), (5, 5)]
    specs = {
        "good_accuracy_good_precision": (0.0, 0.0, 0.12),
        "poor_accuracy_good_precision": (0.9, -0.7, 0.12),
        "good_accuracy_poor_precision": (0.0, 0.0, 0.75),
        "poor_accuracy_poor_precision": (0.9, -0.7, 0.75),
        "irregular_sampling": (0.1, -0.1, 0.20),
        "missingness": (0.1, -0.1, 0.20),
    }
    rows = []
    for profile, (bx, by, sd) in specs.items():
        ...
        for tid, (tx, ty) in enumerate(targets, 1):
            for sample in range(samples_per_target):
                ...
                if profile == "irregular_sampling":
                    dt *= max(0.25, 1 + rng.normal(0, 0.35))
                    dt *= 2.5 if sample in {6, 13} else 1
                t += dt
                gx = tx + bx + rng.normal(0, sd)
                gy = ty + by + rng.normal(0, sd)
                valid = True
                reason = None
                if profile == "missingness" and sample in {5, 6, 7, 14}:
                    gx = gy = np.nan
                    valid = False
                    reason = "blink" if sample in {5, 6} else "tracker_invalidity"
                rows.append(
                    {
                        "participant_id": "P001",
                        "session_id": "S001",
