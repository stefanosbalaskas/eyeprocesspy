...
def register_eye_adapter(name, detect, read, validate=None, priority=0, overwrite=False):
    ...
    if not isinstance(name, str) or not name:
        raise TypeError("`name` must be a single character value.")
    if not callable(detect):
        raise TypeError("`detect` must be a function.")
    if not callable(read):
        raise TypeError("`read` must be a function.")
    if validate is not None and not callable(validate):
        raise TypeError("`validate` must be NULL/None or a function.")
...
def supported_eye_formats():
        ...
        [
            {
                "name": a["name"],
                "priority": a["priority"],
                "has_validator": a["validate"] is not None,
    ...
    return out.sort_values(
        ["priority", "name"], ascending=[False, True], kind="stable"
...
def detect_eye_format(path, inspect_rows=20, candidates=None):
    ...
    adapters = (
        ...
        else {k: v for k, v in _ADAPTERS.items() if k in set(candidates)}
    )
    if not adapters:
        raise ValueError("No eye-data adapters are registered.")
    rows = []
    ...
    return (
        pd.DataFrame(rows)
        .sort_values(
            ["confidence", "priority", "format"], ascending=[False, False, True], kind="stable"
        )
        .reset_index(drop=True)
    )
...
def read_eye_export(path, vendor="auto", confidence_threshold=0.55, **kwargs):
    ...
    if vendor == "auto":
        ...
        if detection.empty or float(detection.iloc[0].confidence) < confidence_threshold:
            raise ValueError(
                f"No adapter reached the required confidence threshold ({confidence_threshold}). Use `read_eye_generic()` with an explicit mapping."
            )
        if len(detection) > 1 and detection.iloc[0].confidence == detection.iloc[1].confidence:
            warnings.warn(
                f"Format detection is tied; selecting `{detection.iloc[0].format}` by adapter priority.",
                RuntimeWarning,
                stacklevel=2,
            )
        vendor = str(detection.iloc[0].format)
    if vendor not in _ADAPTERS:
        raise ValueError(f"Unknown adapter `{vendor}`.")
    out = _ADAPTERS[vendor]["read"](path, **kwargs)
    if isinstance(out, EyeDataset):
        out.format_detection = detection
    return out
...
def read_eye_folder(path, recursive=False, vendor="auto", pattern=None, combine=True, **kwargs):
    ...
    files = [
        q
        for q in (p.rglob("*") if recursive else p.glob("*"))
        if q.is_file() and (pattern is None or re.search(pattern, q.name))
    ]
    if not files:
        raise ValueError(f"No files found in: {path}")
    if not combine:
        return [read_eye_export(f, vendor=vendor, **kwargs) for f in files]
    objs = []
    for f in files:
        try:
            obj = read_eye_export(f, vendor=vendor, **kwargs)
        except Exception:
            continue
        if is_eye_dataset(obj):
            objs.append(obj)
    if not objs:
        raise ValueError("No files in the folder could be imported.")
...
def remap_recording_ids(x, mapping):
    ...
    return add_provenance(
        out, "remap_recording_ids", "dataset", ";".join(f"{a}->{b}" for a, b in mapping.items())
    )
...
def combine_eye_datasets(*xs, resolve_ids=True):
    if len(xs) == 1 and isinstance(xs[0], (list, tuple)) and not is_eye_dataset(xs[0]):
        xs = tuple(xs[0])
    if not xs or not all(is_eye_dataset(x) for x in xs):
    ...
    if resolve_ids:
        ...
        for i, x in enumerate(xs, 1):
            ...
            if duplicates:
                xs[i - 1] = remap_recording_ids(x, {v: f"{v}_set{i}" for v in duplicates})
    ...
    for name in canonical_table_names():
        ...
        tables[name] = (
            ...
            if nonempty
            else (frames[0].copy() if frames else pd.DataFrame())
        )
    identity = {
    ...
    for name, key in identity.items():
        if not tables[name].empty and key in tables[name].columns:
            tables[name] = (
                tables[name].drop_duplicates(subset=[key], keep="first").reset_index(drop=True)
            )
    raw = []
    metadata = {}
    for x in xs:
        if isinstance(x.raw, dict):
            raw.extend(x.raw.items())
        elif isinstance(x.raw, list):
            raw.extend(x.raw)
        if isinstance(x.vendor_metadata, dict):
            metadata.update(x.vendor_metadata)
...
def _detect_generic_delimited(path, inspect_rows=20):
    p = Path(path)
    if p.is_dir() or p.suffix.lower() not in {".csv", ".tsv", ".txt", ".asc"}:
        return 0.0
    try:
        d = _read_delimited(path, nrows=min(2, int(inspect_rows)))
    except Exception:
        return 0.0
...
def _register_builtin_adapters():
    ...
    register_eye_adapter(
        "generic", _detect_generic_delimited, read_eye_generic, priority=1, overwrite=True
    )
