...
def _hmm_fb(
    x: np.ndarray, pi: np.ndarray, A: np.ndarray, mu: np.ndarray, sd: np.ndarray
    ...
    for k in range(K):
        ...
        logB[:, k] = -0.5 * np.sum(
            ((x - mu[k]) / s) ** 2 + 2 * np.log(s) + math.log(2 * math.pi), axis=1
        )
...
def fit_process_hmm_irt(
    data: Any,
    sequence_id: str = "trial_id",
    order: str = "timestamp",
    process_features: Sequence[str] = ("x", "y"),
    ...
    for _ in range(int(max_iter)):
        ...
        second_num = np.zeros_like(mu)
        mu_den = np.zeros(n_states)
        ll_total = 0.0
    ...
    if y.dropna().isin([0, 1]).all() and y.notna().sum() >= 2:
        ...
        Xr, names = _dummy_design(
            summary_data.dropna(subset=[response]), continuous=occ_cols, categorical=[person, item]
        )
        response_fit = _logistic_fit(
            Xr, summary_data.dropna(subset=[response])[response].to_numpy(float)
        )
    ...
    return _result(
        ...
        means=mu,
        sds=sds,
        posterior_state=gamma_all,
        state=state,
        row_data=d,
        occupancy=occupancy,
        summary_data=summary_data,
        response_model=response_fit,
        logLik=history[-1] if history else math.nan,
        logLik_history=np.asarray(history),
        scaling={"center": center, "scale": scale},
...
def process_state_transition_summary(object: Any) -> pd.DataFrame:
    ...
    rows = [
        {"from_state": i + 1, "to_state": j + 1, "probability": float(A[i, j])}
...
def fit_cognitive_diagnosis_process(
    response_matrix: Any,
    q_matrix: Any,
    ...
    **kwargs: Any,
    ...
    if engine == "GDINA":
        raise EyeProcessBackendError(
            "The exact R GDINA engine is not a Python core dependency; supply external_engine for a validated cognitive-diagnosis backend."
        )
    ...
    if process_data is not None and process_features:
        ...
        process_summary = pd.DataFrame(
            {"person_id": agg[person_id].to_numpy(), "process_surrogate": pc}
        )
    return _result(
        "eye_cognitive_diagnosis_process",
        response_model=response_fit,
        process_summary=process_summary,
        process_mastery_correlation=None,
        q_matrix=Q,
        status="experimental-integration",
    )


def fit_latent_class_process_irt(
    data: Any,
    response: str = "response",
    ...
    centers, labels = kmeans2(
        Z, int(n_classes), iter=25, minit="++", seed=np.random.default_rng(seed)
    )
    ...
    if ok.any() and pd.to_numeric(d.loc[ok, response], errors="coerce").isin([0, 1]).all():
        ...
        response_fit = _logistic_fit(Xr, pd.to_numeric(d.loc[ok, response]).to_numpy(float))
        response_fit["design_names"] = names
    return _result(
        "eye_latent_class_process_irt",
        response_model=response_fit,
        class_=pd.Categorical(d[".process_class"]),
...
def fit_crossclassified_process_irt(
    data: Any,
    outcome: str,
    person: str = "participant_id",
    item: str = "item_id",
    ...
    family: str = "gaussian",
    fixed: Sequence[str] | str | None = None,
) -> EyeResult:
    if family not in {"gaussian", "binomial", "poisson", "negative_binomial"}:
        raise EyeProcessValidationError(
            "family must be gaussian, binomial, poisson, or negative_binomial."
        )
    ...
    return _result(
        "eye_crossclassified_process_irt",
        model=fit,
        family=family,
        person=person,
        item=item,
...
def fit_latent_space_irt(
    ...
    raise EyeProcessBackendError(
        "Latent-space IRT parity requires the R LSMjml engine or a separately validated Python backend; no silent approximation is used."
    )
...
def process_residual_map(object: Any, entity: str = "both") -> pd.DataFrame:
    ...
    p = pd.DataFrame(np.asarray(object.person_coordinates))
    p["entity_id"] = list(getattr(object, "person_ids", range(1, len(p) + 1)))
    p["entity_type"] = "person"
    i = pd.DataFrame(np.asarray(object.item_coordinates))
    i["entity_id"] = list(getattr(object, "item_ids", range(1, len(i) + 1)))
    i["entity_type"] = "item"
    if entity == "person":
        return p
    if entity == "item":
...
def validate_latent_space_process_similarity(
    object: Any, process_matrix: Any, entity: str = "person"
) -> EyeResult:
    if getattr(object, "eyeprocess_class", None) != "eye_latent_space_irt":
        raise EyeProcessValidationError("object must be an eye_latent_space_irt.")
    coord = np.asarray(
        object.person_coordinates if entity == "person" else object.item_coordinates, float
    )
    P = np.asarray(process_matrix, float)
    if P.ndim != 2 or len(P) != len(coord):
        raise EyeProcessValidationError(
            "process_matrix rows must align with selected latent-space entities."
        )
    C, _, _ = _zscore(coord)
    Z, _, _ = _zscore(P)
    ...
    return _result(
        "eye_latent_space_process_validation",
        entity=entity,
...
def equate_irt_scales(
    reference: Any,
    new: Any,
    method: str = "stocking-lord",
    theta_grid: Sequence[float] | None = None,
) -> EyeResult:
    ref, nw = _as_df(reference, "reference"), _as_df(new, "new")
    _req_cols(ref, ["a", "b"], "reference")
    _req_cols(nw, ["a", "b"], "new")
    if len(ref) != len(nw):
        raise EyeProcessValidationError(
            "Reference and new anchor sets must contain the same number of items."
        )
    if method not in {"stocking-lord", "haebara", "mean-sigma", "mean-mean"}:
        raise EyeProcessValidationError("Unsupported linking method.")
    ra, rb = ref.a.to_numpy(float), ref.b.to_numpy(float)
    na, nb = nw.a.to_numpy(float), nw.b.to_numpy(float)
    if method == "mean-sigma":
        A = np.std(rb, ddof=1) / np.std(nb, ddof=1)
        B = np.mean(rb) - A * np.mean(nb)
    elif method == "mean-mean":
        A = np.mean(na) / np.mean(ra)
        B = np.mean(rb) - A * np.mean(nb)
    else:
        ...
        def obj(par: np.ndarray) -> float:
            ...
            return float(
                np.sum((pref.sum(1) - pnew.sum(1)) ** 2)
                if method == "stocking-lord"
                else np.sum((pref - pnew) ** 2)
            )
...
def process_person_fit(
    object: Any,
    data: Any = None,
    person: str | None = None,
    response_weight: float = 1,
    rt_weight: float = 1,
    process_weight: float = 1,
) -> pd.DataFrame:
    ...
    if data is None:
        raise EyeProcessValidationError("Supply the data used for the fitted reference model.")
    cols = object.columns
    person = person or cols["person"]
    d = _df(data, list(cols.values())).dropna(subset=list(cols.values())).copy()
    y = pd.to_numeric(d[cols["response"]]).to_numpy(float)
    rt = pd.to_numeric(d[cols["rt"]]).to_numpy(float)
    g = pd.to_numeric(d[cols["gaze"]]).to_numpy(float)
    ...
    if len(p) != len(d):
        p = expit(X @ np.asarray(object.response_model.coefficients, float))
    r_resp = (y - p) / np.sqrt(np.maximum(p * (1 - p), 1e-6))
    rt_pred = np.asarray(object.rt_model.fitted, float)
    rr = np.log(rt) - rt_pred
    s = np.std(rr, ddof=1)
    r_rt = (rr - np.mean(rr)) / (s if np.isfinite(s) and s > 0 else 1)
    gaze_pred = np.expm1(np.asarray(object.gaze_model.fitted, float))
    r_gaze = (g - gaze_pred) / np.sqrt(np.maximum(gaze_pred, 1e-6))
    den = response_weight + rt_weight + process_weight
    row_score = np.sqrt(
        (response_weight * r_resp**2 + rt_weight * r_rt**2 + process_weight * r_gaze**2) / den
    )
    work = pd.DataFrame(
        {
            "participant_id": d[person].astype(str),
            "response_r": r_resp,
            "rt_r": r_rt,
            "process_r": r_gaze,
            "score": row_score,
    ...
    out = (
        work.groupby("participant_id", sort=False)
        .agg(
            n=("score", "size"),
            response_rms=("response_r", lambda x: float(np.sqrt(np.mean(x * x)))),
            rt_rms=("rt_r", lambda x: float(np.sqrt(np.mean(x * x)))),
            process_rms=("process_r", lambda x: float(np.sqrt(np.mean(x * x)))),
            combined_rms=("score", lambda x: float(np.sqrt(np.mean(x * x)))),
        )
        .reset_index()
    )
...
def process_dif_nuisance_surrogate(
    data: Any,
    process_features: Sequence[str],
    person: str = "participant_id",
    aggregate: bool = True,
    ...
    feats = list(process_features)
    d = _df(data, [person, *feats])
    z = (
        d.groupby(person, sort=False)[feats].mean().reset_index()
        if aggregate
        else d[[person, *feats]].copy()
    )
    Z, _, _ = _zscore(z[feats].to_numpy(float))
    pc = (
        Z[:, 0]
        if Z.shape[1] == 1
        else np.linalg.svd(Z, full_matrices=False)[0][:, 0]
        * np.linalg.svd(Z, full_matrices=False)[1][0]
    )
    return pd.DataFrame({"person_id": z[person].to_numpy(), "process_nuisance": pc})


def audit_process_adjusted_dif(
    data: Any,
    response: str = "response",
    ...
    if not ability or not group:
        raise EyeProcessValidationError("ability and group must name columns.")
    d = _df(data, [response, ability, group, item, person, *process_features]).copy()
    sur = process_dif_nuisance_surrogate(d, process_features, person=person, aggregate=True).rename(
        columns={"person_id": person}
    )
    d = d.merge(sur, on=person, how="left", sort=False)
    # Fixed-effect Python diagnostic matching the R nuisance-control intent.
    X0, n0 = _dummy_design(d, continuous=[ability], categorical=[group, item])
    X1, n1 = _dummy_design(d, continuous=[ability, "process_nuisance"], categorical=[group, item])
    y = pd.to_numeric(d[response], errors="coerce").to_numpy(float)
    m0 = _logistic_fit(X0, y)
    m1 = _logistic_fit(X1, y)
    # Report coefficients that are common after process adjustment; interaction-specific R coefficients are backend-dependent.
    c0 = dict(zip(n0, np.asarray(m0.coefficients, float)))
    c1 = dict(zip(n1, np.asarray(m1.coefficients, float)))
    names = sorted(set(c0) & set(c1))
    tab = pd.DataFrame(
        {"term": names, "unadjusted": [c0[n] for n in names], "adjusted": [c1[n] for n in names]}
    )
    tab["absolute_reduction"] = np.abs(tab.unadjusted) - np.abs(tab.adjusted)
    return _result(
        "eye_process_adjusted_dif",
        unadjusted_model=m0,
        adjusted_model=m1,
        coefficients=tab,
        surrogate=sur,
...
def _seqs(sequence: Any, separator: str) -> list[list[str]]:
    if isinstance(sequence, str):
        return [sequence.split(separator)]
    if (
        isinstance(sequence, (list, tuple))
        and sequence
        and isinstance(sequence[0], (list, tuple, np.ndarray, pd.Series))
    ):
        return [[str(x) for x in s] for s in sequence]
    return [[str(x) for x in sequence]]


def process_ngram_features(
    sequence: Any, n: Sequence[int] = (1, 2, 3), separator: str = ">"
) -> np.ndarray:
...
def process_sequence_embedding(
    sequence: Any, n: Sequence[int] = (1, 2, 3), dimensions: int = 5
) -> np.ndarray:
    X = process_ngram_features(sequence, n=n)
    if X.shape[1] == 0:
        raise EyeProcessValidationError("No n-grams could be constructed.")
    tf = X / np.maximum(X.sum(axis=1, keepdims=True), 1)
    idf = np.log((len(X) + 1) / (np.sum(X > 0, axis=0) + 1)) + 1
    Z = tf * idf
    u, s, _ = np.linalg.svd(Z, full_matrices=False)
...
def fit_response_process_embedding_irt(
    ...
    item: str = "item_id",
    dimensions: int = 5,
    n: Sequence[int] = (1, 2, 3),
) -> EyeResult:
    d = _df(data, [response, person, item]).copy()
    emb = process_sequence_embedding(sequences, n=n, dimensions=dimensions)
    if len(emb) != len(d):
        raise EyeProcessValidationError("sequences must align row-for-row with data.")
    dd = d.copy()
    emb_cols = [f"process_embedding_{i + 1}" for i in range(emb.shape[1])]
    for i, c in enumerate(emb_cols):
        dd[c] = emb[:, i]
    X, names = _dummy_design(dd, continuous=emb_cols, categorical=[person, item])
    fit = _logistic_fit(X, pd.to_numeric(dd[response]).to_numpy(float))
    fit["design_names"] = names
    return _result(
        "eye_response_process_embedding_irt",
        model=fit,
        embedding=emb,
        data=dd,
...
def fit_gpirt(
    ...
    **kwargs: Any,
    ...
    X = np.asarray(response_matrix, float)
    if X.ndim != 2:
        raise EyeProcessValidationError("response_matrix must be two-dimensional.")
    if engine == "external":
        if not callable(external_engine):
            raise EyeProcessBackendError("Supply a validated GPIRT fitter through external_engine.")
        return _result(
            "eye_gpirt",
            model=external_engine(response_matrix=X, **kwargs),
            engine="external",
            exact_gpirt=True,
            status="experimental-external",
        )
    if engine != "spline_reference":
        raise EyeProcessValidationError("engine must be spline_reference or external.")
    p = np.nanmean(X, axis=1)
    nn = np.sum(np.isfinite(X), axis=1)
    adj = (p * nn + 0.5) / (nn + 1)
    theta = np.log(np.clip(adj, 1e-5, 1 - 1e-5) / (1 - np.clip(adj, 1e-5, 1 - 1e-5)))
    models = []
    for j in range(X.shape[1]):
        ok = np.isfinite(X[:, j])
        D = _poly_design(theta[ok], degree=max(2, min(5, int(spline_df) - 1)))
        fit = _logistic_fit(D, X[ok, j])
        fit["theta_degree"] = D.shape[1] - 1
        models.append(fit)
    ...
    return _result(
        ...
        exact_gpirt=False,
        status="experimental-model-criticism",
        algorithmic_parity=False,
        note="Polynomial-flexible IRFs are a Python shape-audit surrogate; not a Gaussian-process IRT estimator.",
    )


def compare_parametric_nonparametric_irf(
    response_matrix: Any, gpirt_object: Any = None, theta_grid: Sequence[float] | None = None
) -> pd.DataFrame:
    X = np.asarray(response_matrix, float)
    obj = fit_gpirt(X) if gpirt_object is None else gpirt_object
    if getattr(obj, "eyeprocess_class", None) != "eye_gpirt" or obj.engine != "spline_reference":
        raise EyeProcessValidationError(
    ...
    for j in range(X.shape[1]):
        ...
        y = X[ok, j]
        lin = _logistic_fit(np.column_stack([np.ones(len(th)), th]), y)
        pp = expit(
            np.column_stack([np.ones(len(grid)), grid]) @ np.asarray(lin.coefficients, float)
        )
        deg = int(obj.models[j].theta_degree)
        pf = expit(_poly_design(grid, deg) @ np.asarray(obj.models[j].coefficients, float))
        rows.extend(
            {
                "item": obj.item_names[j],
                "theta": float(t),
                "parametric": float(a),
                "flexible": float(b),
                "absolute_difference": float(abs(a - b)),
            }
            for t, a, b in zip(grid, pp, pf)
        )
    return _tag(pd.DataFrame(rows), "eye_irf_comparison")


def audit_irf_shape(
    comparison: Any, mean_absolute_threshold: float = 0.05, max_absolute_threshold: float = 0.15
) -> pd.DataFrame:
    d = _as_df(comparison, "comparison")
    _req_cols(d, ["item", "absolute_difference"], "comparison")
    out = (
        d.groupby("item", sort=False)
        .absolute_difference.agg(mean_absolute_difference="mean", max_absolute_difference="max")
        .reset_index()
    )
    out["flag"] = (out.mean_absolute_difference > mean_absolute_threshold) | (
        out.max_absolute_difference > max_absolute_threshold
    )
    return out
...
def _external_gate(
    ...
    data_kw: str,
    data: Any,
    external_engine: Callable[..., Any] | None,
    kwargs: dict[str, Any],
    cls: str,
) -> EyeResult:
    if not callable(external_engine):
        raise EyeProcessBackendError(message)
    return _result(
        cls,
        model=external_engine(**{data_kw: data}, **kwargs),
        engine="external",
        status="experimental-gated",
    )


def fit_dynamic_gpirt(
    data: Any, external_engine: Callable[..., Any] | None = None, **kwargs: Any
) -> EyeResult:
    return _external_gate(
        "fit_dynamic_gpirt",
        "Dynamic GPIRT is experimental; supply a validated external_engine.",
        "data",
        data,
        external_engine,
        kwargs,
...
def fit_continuous_time_irt(
    data: Any, external_engine: Callable[..., Any] | None = None, **kwargs: Any
) -> EyeResult:
    return _external_gate(
        "fit_continuous_time_irt",
        "Continuous-time IRT requires a validated external_engine.",
        "data",
        data,
        external_engine,
        kwargs,
        "eye_continuous_time_irt",
    )


def fit_flow_mirt(
    response_matrix: Any, external_engine: Callable[..., Any] | None = None, **kwargs: Any
) -> EyeResult:
    return _external_gate(
        "fit_flow_mirt",
        "Flow-MIRT remains experimental; supply a validated external_engine.",
        "response_matrix",
        response_matrix,
        external_engine,
        kwargs,
...
def fit_variational_irt(
    response_matrix: Any, external_engine: Callable[..., Any] | None = None, **kwargs: Any
) -> EyeResult:
    return _external_gate(
        "fit_variational_irt",
        "Supply a validated variational IRT external_engine; no silent approximation is used.",
        "response_matrix",
        response_matrix,
        external_engine,
        kwargs,
...
def latent_trait_trajectory(time: Any, theta: Any, spar: float | None = None) -> EyeResult:
    ...
    if ok.sum() < 4:
        raise EyeProcessValidationError("At least four finite time/theta pairs are required.")
    ord_ = np.argsort(t[ok])
    tx = t[ok][ord_]
    ty = th[ok][ord_]
...
def predict_theta_at_time(object: Any, time: Any) -> pd.DataFrame:
    if getattr(object, "eyeprocess_class", None) != "eye_latent_trait_trajectory":
        raise EyeProcessValidationError("object must come from latent_trait_trajectory().")
    t = np.asarray(time, float)
    return pd.DataFrame({"x": t, "y": object.model(t)})


def process_item_information(
    theta: Any,
    a: Any,
    b: Any,
    process_information: Any = 0,
    rt_information: Any = 0,
    ...
    aa = np.atleast_1d(np.asarray(a, float))
    bb = np.atleast_1d(np.asarray(b, float))
    if len(aa) != len(bb):
        raise EyeProcessValidationError("a and b must have equal length.")
    p = expit(th[:, None] * aa[None, :] - (aa * bb)[None, :])
    resp = p * (1 - p) * (aa[None, :] ** 2)
    if isinstance(weights, Mapping):
        w = {
            "response": float(weights.get("response", 0)),
            "rt": float(weights.get("rt", 0)),
            "process": float(weights.get("process", 0)),
        }
    else:
        z = list(weights)
        w = {
            "response": z[0] if len(z) > 0 else 0,
            "rt": z[1] if len(z) > 1 else 0,
            "process": z[2] if len(z) > 2 else 0,
        }
    proc = np.resize(np.asarray(process_information, float), len(aa))
    rti = np.resize(np.asarray(rt_information, float), len(aa))
    et = np.resize(np.asarray(expected_time, float), len(aa))
    utility = (
        w["response"] * resp + (w["rt"] * rti + w["process"] * proc - burden_weight * et)[None, :]
    )
    return _result(
        "eye_process_item_information", theta=th, response_information=resp, utility=utility
    )


def expected_process_information(info: Any, theta_weights: Any = None) -> np.ndarray:
    if getattr(info, "eyeprocess_class", None) != "eye_process_item_information":
        raise EyeProcessValidationError("info must come from process_item_information().")
    w = (
        np.repeat(1 / len(info.theta), len(info.theta))
...
def select_next_item_process(
    ...
    b = _as_df(item_bank, "item_bank")
    _req_cols(b, ["item_id", "a", "b"], "item_bank")
    avail = b[~b.item_id.astype(str).isin([str(x) for x in used])].copy()
    if avail.empty:
        raise EyeProcessValidationError("No unused items remain.")
    pi = avail["process_information"].to_numpy(float) if "process_information" in avail else 0
    ri = avail["rt_information"].to_numpy(float) if "rt_information" in avail else 0
    et = avail["expected_time"].to_numpy(float) if "expected_time" in avail else 0
    info = process_item_information([theta], avail.a, avail.b, pi, ri, weights, et, burden_weight)
    u = np.asarray(info.utility)[0]
    j = int(np.nanargmax(u))
    return _result(
        "eye_process_item_selection",
        item_id=avail.iloc[j].item_id,
        utility=float(u[j]),
        row=avail.iloc[[j]].copy(),
        all_utilities=pd.DataFrame({"item_id": avail.item_id.to_numpy(), "utility": u}),
    )


def simulate_process_cat(
    item_bank: Any,
    true_theta: float = 0,
    n_items: int = 10,
    weights: Mapping[str, float] | Sequence[float] = {"response": 1, "rt": 0, "process": 0},
    ...
    for step in range(1, min(int(n_items), len(bank)) + 1):
        ...
        it = sel.row.iloc[0]
        p = float(expit(float(it.a) * (true_theta - float(it.b))))
        y = int(rng.binomial(1, p))
        used.append(str(it.item_id))
        responses.append(y)
        grid = np.linspace(-4, 4, 321)
        sub = bank.set_index(bank.item_id.astype(str)).loc[used]
        aa = sub.a.to_numpy(float)
        bb = sub.b.to_numpy(float)
        yy = np.asarray(responses, float)
        probs = expit(grid[:, None] * aa[None, :] - (aa * bb)[None, :])
        ll = np.sum(
            yy * np.log(np.clip(probs, _EPS, 1)) + (1 - yy) * np.log(np.clip(1 - probs, _EPS, 1)),
            axis=1,
        )
        theta_hat = float(grid[int(np.argmax(ll))])
        rows.append(
            {
                "step": step,
                "item_id": it.item_id,
                "response": y,
                "selection_utility": sel.utility,
                "utility": sel.utility,
                "theta_hat": theta_hat,
...
def _upgrade_registry() -> None:
    _register_builtins()
    mapping = {
        "process_hmm": fit_process_hmm_irt,
        "latent_space_process": fit_latent_space_irt,
        "gpirt_shape_audit": fit_gpirt,
