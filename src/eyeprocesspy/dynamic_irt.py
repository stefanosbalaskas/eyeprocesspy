...
def dynamic_irtree_spec(
    ...
    if person_effect not in {"none", "fixed", "random"} or item_effect not in {
        "none",
        "fixed",
        "random",
    }:
        raise EyeProcessValidationError(
            "person_effect and item_effect must be none, fixed, or random."
        )
    ...
    if any(int(v) < 1 for v in controls) or int(parallel_chains) > int(chains):
        raise EyeProcessValidationError(
            "Stan iteration, chain, and tree-depth controls must be positive and parallel_chains <= chains."
        )
    ...
    if any((v is None or str(v) == "") for v in cc + tp + inter):
        raise EyeProcessValidationError(
            "Predictor and interaction names must be non-missing and non-empty."
        )
    return _result(
...
def _long_to_transitions(
    d: pd.DataFrame, person: str, item: str, trial: str, state: str, time: str | None
) -> pd.DataFrame:
    ...
    **kwargs: Any,
    ...
    for c in ("participant_id", "item_id", "trial_id"):
        if d[c].isna().any() or d[c].astype(str).eq("").any():
            raise EyeProcessValidationError(
                "Participant, item, and trial identifiers must be non-missing and non-empty."
            )
    ...
    if spec.missing_state == "drop":
        ...
        if spec.missing_state == "marginalize" and spec.hidden_states < 2:
            raise EyeProcessValidationError(
                "missing_state = 'marginalize' requires the hidden-state Stan engine."
            )
    if d.empty:
    ...
    levels = (
        ...
        else list(dict.fromkeys(pd.concat([d["from_state"], d["to_state"]]).astype(str)))
    )
    if len(levels) < 2:
    ...
    if spec.uncertain_state_probability:
        if spec.uncertain_state_probability not in d:
            raise EyeProcessValidationError(
                f"Uncertain-state probability column is absent: {spec.uncertain_state_probability}"
            )
        p = pd.to_numeric(d[spec.uncertain_state_probability], errors="coerce").to_numpy(float)
        if np.any(~np.isfinite(p)) or np.any((p <= 0) | (p > 1)):
            raise EyeProcessValidationError(
                "Recorded-state probabilities must be finite values in (0, 1]."
            )
        d["state_probability"] = p
    else:
        d["state_probability"] = np.where(
            (spec.missing_state == "marginalize") & (d["to_state"].astype(str) == "<UNKNOWN>"),
            1 / len(levels),
            1.0,
        )
...
def structural_transition_mask(
    ...
    if (~mask.any(axis=1)).any():
        raise EyeProcessValidationError(
            "Every source state must permit at least one destination state."
        )
...
def dynamic_transition_design(
    data: Any, spec: EyeResult | None = None, formula: Any = None
) -> EyeResult:
    spec = dynamic_irtree_spec() if spec is None else spec
    d = (
        data.copy()
        if isinstance(data, pd.DataFrame)
        and data.attrs.get("eyeprocess_class") == "eye_dynamic_transition_data"
        else prepare_dynamic_irtree_data(data, spec)
    )
    ...
    if spec.include_time_gap:
        blocks.append(
            pd.DataFrame(
                {
                    "log1p(time_gap)": np.log1p(
                        pd.to_numeric(d["time_gap"], errors="coerce").to_numpy(float)
                    )
                }
            )
        )
    if (
        spec.include_response
        and "score" in d
        and np.isfinite(pd.to_numeric(d["score"], errors="coerce")).any()
    ):
        blocks.append(
            pd.DataFrame({"score": pd.to_numeric(d["score"], errors="coerce").to_numpy(float)})
        )
    ...
    return _result(
        "eye_transition_design",
        data=d,
        formula=formula,
        X=Xdf,
        y=d["to_index"].to_numpy(int),
        states=levels,
        allowed=allowed_arr,
        transition_mask=mask,
        scaling=pd.DataFrame(scaling_rows),
        participants=list(dict.fromkeys(d["participant_id"].astype(str))),
        items=list(dict.fromkeys(d["item_id"].astype(str))),
        trials=list(dict.fromkeys(d["trial_index"].astype(int))),
        spec=spec,
    )
...
def fit_multinomial_transition(
    ...
    X = (
        design.X.to_numpy(float)
        if isinstance(design.X, pd.DataFrame)
        else np.asarray(design.X, float)
    )
    ...
    if np.any(~allowed[np.arange(len(y)), y]):
        raise EyeProcessValidationError(
            "Observed transitions violate the declared structural-transition mask."
        )
    ...
    return _result(
        ...
        coefficient_matrix=pd.DataFrame(B, index=terms, columns=states),
        standard_error_matrix=pd.DataFrame(SE, index=terms, columns=states),
        probabilities=p_df,
        fitted_state=pd.Categorical(
            [states[i] for i in np.argmax(probs, axis=1)], categories=states, ordered=True
        ),
        log_likelihood=float(-fit.fun + 0.5 * ridge * np.sum(fit.x**2)),
        convergence=0 if fit.success else 1,
        message=str(fit.message),
        iterations={"function": fit.nfev, "gradient": getattr(fit, "njev", None)},
        reference_state=reference_state,
        ridge=float(ridge),
        optimizer=fit,
    )
...
def fit_dynamic_irtree_stan(
    ...
    **kwargs: Any,
    ...
    if spec.hidden_states >= 2:
        ...
        emission = (
            np.asarray(spec.misclassification_matrix, float)
            if spec.misclassification_matrix is not None
            else np.full((H, Kobs), 1 / Kobs)
        )
        ...
        if emission.shape != (H, Kobs) or np.any(emission < 0) or np.any(~np.isfinite(emission)):
            raise EyeProcessValidationError(
                "Misclassification matrix must have hidden-state rows and observed-state columns."
            )
        emission = emission / emission.sum(axis=1, keepdims=True)
        hidden_labels = [f"state{i + 1}" for i in range(H)]
        hmask = structural_transition_mask(
            hidden_labels, spec.hidden_structural_zeros, spec.hidden_allowed_transitions
        ).to_numpy(int)
        stan_data = dict(
            N=len(d),
            S=len(groups),
            ...
            Kobs=Kobs,
            D=design.X.shape[1],
            X=design.X.to_numpy(float)[order],
            observed=np.asarray(design.y, int)[order],
            observation_certainty=np.asarray(d["state_probability"], float)[order],
            sequence_start=starts.astype(int),
            sequence_length=lengths,
            allowed_hidden=hmask,
            emission_prior=emission,
        )
        ...
        stan_data = dict(
            ...
            K=len(design.states),
            D=design.X.shape[1],
            P=len(design.participants),
            J=len(design["items"]),
            X=design.X.to_numpy(float),
            y=np.asarray(design.y, int),
            person=np.asarray(design.data["person_index"], int),
            item=np.asarray(design.data["item_index"], int),
            from_state=np.asarray(design.data["from_index"], int),
            allowed=np.asarray(design.allowed, int),
            observation_weight=np.asarray(design.data["state_probability"], float),
            use_person_re=int(spec.person_effect == "random"),
            use_item_re=int(spec.item_effect == "random"),
        )
        filename, hidden, order = (
            "dynamic_irtree_observed.stan",
            False,
            np.arange(design.X.shape[0]),
        )
    try:
    ...
    return _result(
        ...
        hidden=hidden,
        sequence_order=np.asarray(order, int),
    )
...
def decode_dynamic_states(object: EyeResult, method: str = "mode") -> pd.DataFrame:
    ...
    if getattr(model, "eyeprocess_class", None) == "eye_multinomial_transition":
        ...
        return pd.DataFrame(
            {"transition": np.arange(1, len(p) + 1), "decoded_state": [p.columns[i] for i in idx]}
        )
    raise EyeProcessModelError(
        "State decoding is unavailable for this engine or requires a fitted hidden-state Stan posterior."
    )


def dynamic_posterior_predictive_check(
    object: EyeResult, draws: int = 200, seed: int = 1
) -> EyeResult:
    ...
    summary = pd.DataFrame(
        {
            "state": states,
            "observed": obs_freq,
            "replicated_mean": repl.mean(0),
            "replicated_lower": np.quantile(repl, 0.025, axis=0),
            "replicated_upper": np.quantile(repl, 0.975, axis=0),
...
def transition_residual_diagnostics(object: EyeResult, type: str = "pearson") -> EyeResult:
    ...
    if getattr(object.model, "eyeprocess_class", None) != "eye_multinomial_transition":
        raise EyeProcessModelError(
            "Residual diagnostics currently require baseline/multinomial probabilities; Stan fits use posterior predictive checks."
        )
    ...
    rows = pd.DataFrame(
        {
            "transition": np.arange(1, len(d) + 1),
            "observed_state": d["to_state"].astype(str),
            "fitted_probability": op,
            "residual": residual,
            "participant_id": d["participant_id"].astype(str),
            "item_id": d["item_id"].astype(str),
            "trial_id": d["trial_id"].astype(str),
...
def compare_dynamic_transition_models(
    *models: Any, criterion: str = "AIC", **named: Any
) -> pd.DataFrame:
    if (
        len(models) == 1
        and isinstance(models[0], Mapping)
        and all(
            getattr(v, "eyeprocess_class", None) == "eye_dynamic_irtree" for v in models[0].values()
        )
    ):
    ...
    for name, obj in pairs:
        ...
        if getattr(obj.model, "eyeprocess_class", None) == "eye_multinomial_transition":
            ...
            k = int(
                (
                    (np.isfinite(co["estimate"])) & (co["destination"] != obj.model.reference_state)
                ).sum()
            )
            p = obj.model.probabilities.to_numpy(float)
            y = np.asarray(obj.model.design.y, int) - 1
            acc = float(np.mean(np.argmax(p, axis=1) == y))
            ls = float(np.mean(np.log(np.maximum(p[np.arange(n), y], 1e-12))))
        rows.append(
            {
                "model": name,
                ...
                "BIC": -2 * ll + math.log(n) * k if np.isfinite(ll) else np.nan,
                "log_score": ls,
                "accuracy": acc,
            }
        )
    ...
    rank[order[np.isfinite(vals[order])]] = np.arange(1, np.isfinite(vals).sum() + 1)
    out["rank"] = rank
...
def simulate_dynamic_irtree_data(
    n_person: int = 100,
    n_item: int = 20,
    transitions_per_trial: int = 8,
    states: Sequence[str] = ("prompt", "evidence", "options"),
    beta_response: float = 0.5,
    person_sd: float = 0.4,
    item_sd: float = 0.3,
    irregular_time: bool = True,
    state_misclassification: float = 0,
    missing_state: float = 0,
    structural_zeros: Any = None,
    seed: int = 1,
) -> EyeResult:
    ...
    if any(
        (not np.isfinite(v) or v < 0 or v >= 1) for v in (state_misclassification, missing_state)
    ):
        raise EyeProcessValidationError(
            "Misclassification and missing-state rates must lie in [0, 1)."
        )
    if person_sd < 0 or item_sd < 0:
        raise EyeProcessValidationError(
            "Person and item heterogeneity SDs must be finite and non-negative."
        )
    levels = list(dict.fromkeys(map(str, states)))
    K = len(levels)
    mask = structural_transition_mask(levels, structural_zeros).to_numpy(bool)
    rng = np.random.default_rng(seed)
    persons = [f"P{i + 1}" for i in range(n_person)]
    items = [f"I{i + 1}" for i in range(n_item)]
    ...
    base = rng.normal(0, 0.4, (K, K))
    base[np.arange(K), np.arange(K)] += 0.8
    rows = []
    for p in range(n_person):
        for j in range(n_item):
            score = int(rng.random() < expit(theta[p] - difficulty[j]))
            frm = int(rng.integers(K))
            t = 0.0
            for step in range(1, transitions_per_trial + 1):
                eta = base[frm] + pe[p] + ie[j] + beta_response * score * np.linspace(-0.5, 0.5, K)
                eta[~mask[frm]] = -1e12
                prob = np.exp(eta - np.max(eta))
                prob /= prob.sum()
                dest = int(rng.choice(K, p=prob))
                gap = float(rng.exponential(0.5)) if irregular_time else 1.0
                obs = (
                    int(rng.choice([k for k in range(K) if k != dest]))
                    if rng.random() < state_misclassification and K > 1
                    else dest
                )
                label: Any = levels[obs] if rng.random() >= missing_state else np.nan
                rows.append(
                    {
                        "participant_id": persons[p],
                        "item_id": items[j],
                        "trial_id": f"{persons[p]}-{items[j]}",
                        "step": step,
                        "time": t,
                        "time_gap": gap,
                        "from_state": levels[frm],
                        "true_to_state": levels[dest],
                        "to_state": label,
                        "score": score,
                    }
                )
...
def dynamic_irtree_recovery(
    grid: Any = None, replications: int = 20, spec: EyeResult | None = None, base_seed: int = 1
) -> EyeResult:
    if grid is None:
        grid = pd.DataFrame(
            [(a, b) for a in (0, 0.05, 0.15) for b in (0, 0.10)],
            columns=["state_misclassification", "missing_state"],
        )
    grid = _df(grid)
    jobs = []
    jid = 0
    for _, row in grid.iterrows():
        for r in range(1, int(replications) + 1):
            jid += 1
            jobs.append({**row.to_dict(), "replication": r, "seed": int(base_seed) + jid - 1})
    plan = _result(
        "eye_validation_job_plan",
        plan_id=f"dynamic_irtree-{base_seed}-{len(jobs)}",
        jobs=pd.DataFrame(jobs),
        model_family="dynamic_irtree",
    )
    return _result(
        "eye_dynamic_recovery",
        plan=plan,
        spec=dynamic_irtree_spec(engine="multinomial") if spec is None else spec,
        purpose="recovery under state misclassification; not automatic construct validation",
    )


def fit_dynamic_irtree(
    x: Any, spec: EyeResult | None = None, min_transitions: int = 10, seed: int = 1, **kwargs: Any
) -> EyeResult:
    spec = dynamic_irtree_spec() if spec is None else spec
    d = prepare_dynamic_irtree_data(x, spec)
    design = dynamic_transition_design(d, spec)
    if spec.engine == "multinomial":
        model = fit_multinomial_transition(
            design,
            ridge=spec.ridge,
            ...
            [
                {
                    ...
                    "optimizer_code": model.convergence,
                    "log_likelihood": model.log_likelihood,
                }
            ]
        )
    elif spec.engine == "stan":
        model = fit_dynamic_irtree_stan(design, spec, seed=seed, **kwargs)
        co = model.summary
        ...
        fits = {}
        rows = []
        probs = np.zeros((len(d), len(design.states)))
        X = design.X.to_numpy(float)
        for k, state in enumerate(design.states):
            y = (np.asarray(design.y) == k + 1).astype(float)
            if y.sum() < min_transitions or (1 - y).sum() < min_transitions:
                continue
            ...
            f = minimize(obj, np.zeros(X.shape[1]), method="BFGS")
            fits[state] = f
            probs[:, k] = expit(X @ f.x)
            for term, est in zip(design.X.columns, f.x):
                rows.append({"destination": state, "term": term, "estimate": est})
        if not fits:
            raise EyeProcessModelError("No destination state had sufficient transition support.")
        probs = probs / np.maximum(probs.sum(1, keepdims=True), _EPS)
        model = _result(
            "eye_multinomial_transition",
            design=design,
            probabilities=pd.DataFrame(probs, columns=design.states),
            fitted_state=pd.Categorical(
                [design.states[i] for i in np.argmax(probs, 1)], categories=design.states
            ),
            coefficients=pd.DataFrame(rows),
            reference_state=design.states[-1],
            log_likelihood=np.nan,
            convergence=0,
        )
        co = pd.DataFrame(rows)
        diag = pd.DataFrame([{"converged": True, "models": len(fits)}])
    return _result(
        "eye_dynamic_irtree",
        spec=spec,
        ...
        model=model,
        fits={} if spec.engine != "baseline" else fits,
        coefficients=co,
        fit_warnings={},
        diagnostics=diag,
...
def theory_strategy_spec(
    ...
    item_availability: Any = None,
    engine: str = "em",
    multiple_starts: int = 10,
    anchor_strength: float = 3,
    chains: int = 4,
    parallel_chains: int | None = None,
    iter_warmup: int = 1000,
    iter_sampling: int = 1000,
    ...
    if engine not in {"em", "stan"}:
        raise EyeProcessValidationError("engine must be em or stan.")
    if strategies is None and prototypes is not None:
        strategies = prototypes
    ...
    if legacy:
        ...
        if (
            ...
            or proto.shape[0] < 2
            or proto.shape[1] < 1
            or np.any(~np.isfinite(proto))
        ):
            raise EyeProcessValidationError(
                "prototypes must contain at least two strategies and one finite numeric feature."
            )
        if isinstance(strategies, pd.DataFrame):
            names = list(map(str, strategies.index))
            cols = list(map(str, strategies.columns))
        else:
            names = [f"strategy_{i + 1}" for i in range(proto.shape[0])]
            cols = list(feature_columns or [f"feature_{j + 1}" for j in range(proto.shape[1])])
        fs = (
            np.ones(proto.shape[1])
            if feature_sd is None
            else np.broadcast_to(np.asarray(feature_sd, float), (proto.shape[1],)).copy()
        )
        pr = np.ones(proto.shape[0]) / proto.shape[0] if prior is None else np.asarray(prior, float)
        pr = pr / pr.sum()
    else:
        if not isinstance(strategies, Mapping) or len(strategies) < 2:
            raise EyeProcessValidationError(
                "strategies must be a uniquely named list of theory-defined signatures or a prototype matrix."
            )
        names = list(map(str, strategies.keys()))
        if any(not n for n in names) or len(set(names)) != len(names):
            raise EyeProcessValidationError("Strategy names must be unique and non-empty.")
        if feature_columns is None:
            cols = []
            for s in strategies.values():
                if not isinstance(s, Mapping):
                    raise EyeProcessValidationError(
                        "Every strategy signature must be a named numeric vector; use a mapping in Python."
                    )
                for c in s:
                    if str(c) not in cols:
                        cols.append(str(c))
        else:
            cols = list(map(str, feature_columns))
        ...
        for k, s in enumerate(strategies.values()):
            if isinstance(s, Mapping):
                ...
                if unknown:
                    raise EyeProcessValidationError(
                        f"Unknown signature features: {', '.join(sorted(unknown))}."
                    )
                for c, v in s.items():
                    proto[k, cols.index(str(c))] = float(v)
            else:
                arr = np.asarray(s, float)
                if arr.size != len(cols):
                    raise EyeProcessValidationError(
                        "Unnamed strategy signatures must match feature_columns."
                    )
                proto[k] = arr
        fs = np.ones(len(cols))
        pr = np.ones(len(names)) / len(names)
    norms = np.sqrt((proto**2).sum(1))
    if np.any(norms == 0):
        raise EyeProcessValidationError(
            "Every strategy signature must contain at least one non-zero anchor."
        )
    signatures = proto / norms[:, None]
    if int(multiple_starts) < 1 or anchor_strength < 0:
        raise EyeProcessValidationError(
            "multiple_starts must be positive and anchor_strength non-negative."
        )
    if parallel_chains is None:
        parallel_chains = min(4, chains)
    return _result(
        "eye_theory_strategy_spec",
        strategies=names,
        signatures=pd.DataFrame(signatures, index=names, columns=cols),
        prototypes=pd.DataFrame(proto, index=names, columns=cols),
        feature_sd=np.asarray(fs, float),
        prior=np.asarray(pr, float),
        legacy_mode=legacy,
        ...
        engine=engine,
        multiple_starts=int(multiple_starts),
...
def prepare_strategy_mixture_data(
    data: Any, spec: EyeResult, standardize: bool = True
) -> EyeResult:
    d = _df(data)
    req = [spec.participant, spec.item, spec.response, *spec.feature_columns] + (
        ([spec.condition]) if spec.condition else []
    )
    _required(d, req)
    d = d.dropna(subset=[spec.participant, spec.item, spec.response, *spec.feature_columns]).copy()
    if d.empty:
        raise EyeProcessValidationError("No complete strategy-mixture rows remain.")
    y = pd.to_numeric(d[spec.response], errors="coerce").to_numpy(int)
    if not np.isin(y, [0, 1]).all():
        raise EyeProcessValidationError("Strategy-mixture responses must be coded 0/1.")
    ...
    if standardize:
        ...
        X = (X - center) / scale
    plevel = list(dict.fromkeys(d[spec.participant].astype(str)))
    ilevel = list(dict.fromkeys(d[spec.item].astype(str)))
    pidx = np.array([plevel.index(v) + 1 for v in d[spec.participant].astype(str)])
    iidx = np.array([ilevel.index(v) + 1 for v in d[spec.item].astype(str)])
    ...
    cidx = (
        np.array([clevel.index(v) + 1 for v in d[spec.condition].astype(str)])
        if spec.condition
        else np.ones(len(d), int)
    )
    return _result(
        "eye_strategy_data",
        data=d,
        X=X,
        y=y,
        signatures=spec.signatures.to_numpy(float),
        participant=pidx,
        item=iidx,
        condition=cidx,
        participant_levels=plevel,
        item_levels=ilevel,
        condition_levels=clevel,
        availability=availability,
        availability_item=avail_item,
        center=center,
        scale=scale,
        spec=spec,
    )
...
def _strategy_loglik(
    ...
    N, K = prep.X.shape[0], means.shape[0]
    lc = np.full((N, K), -np.inf)
    for k in range(K):
        diff = prep.X - means[k]
        pll = -0.5 * np.sum(diff**2 / variance[k] + np.log(2 * np.pi * variance[k]), axis=1)
        eta = intercept[k] + ability[prep.participant - 1] - difficulty[prep.item - 1]
        p = np.clip(expit(eta), 1e-12, 1 - 1e-12)
        rll = prep.y * np.log(p) + (1 - prep.y) * np.log(1 - p)
        lc[:, k] = np.log(max(mixing[k], 1e-12)) + pll + rll
        lc[prep.availability[:, k] == 0, k] = -np.inf
    normz = logsumexp(lc, axis=1)
    prob = np.exp(lc - normz[:, None])
    return float(normz.sum()), prob


def fit_strategy_mixture_em(
    prepared: EyeResult,
    ...
    if getattr(prepared, "eyeprocess_class", None) != "eye_strategy_data":
        raise EyeProcessValidationError("Expected prepared strategy-mixture data.")
    starts = prepared.spec.multiple_starts if starts is None else int(starts)
    rng = np.random.default_rng(seed)
    K, F = prepared.signatures.shape
    P = len(prepared.participant_levels)
    J = len(prepared.item_levels)
    results = []
    for s in range(starts):
        means = prepared.signatures * prepared.spec.anchor_strength + rng.normal(0, 0.15, (K, F))
        var = np.ones((K, F))
        intercept = rng.normal(0, 0.25, K)
        ability = np.zeros(P)
        ...
        for it in range(1, int(max_iter) + 1):
            ll, prob = _strategy_loglik(
                prepared, means, var, intercept, ability, difficulty, mixing
            )
            history.append(ll)
            w = prob.sum(0)
            mixing = w / w.sum()
            for k in range(K):
                if w[k] <= 1e-8:
                    continue
                empirical = (prepared.X * prob[:, k, None]).sum(0) / w[k]
                means[k] = (
                    empirical * w[k] + prepared.signatures[k] * prepared.spec.anchor_strength
                ) / (w[k] + prepared.spec.anchor_strength)
                diff = prepared.X - means[k]
                var[k] = np.maximum((diff**2 * prob[:, k, None]).sum(0) / w[k], 0.05)
                ...
                ybar = np.clip((prepared.y * prob[:, k]).sum() / w[k], 1e-6, 1 - 1e-6)
                intercept[k] = math.log(ybar / (1 - ybar))
    ...
    scores = np.array([r["log_likelihood"] for r in results])
    best = results[int(np.argmax(scores))]
    best["all_starts"] = pd.DataFrame(
        {
            "start": np.arange(1, starts + 1),
...
def fit_strategy_mixture_stan(
    ...
    **kwargs: Any,
    ...
    data = dict(
        N=len(prepared.y),
        K=prepared.signatures.shape[0],
        F=prepared.X.shape[1],
        P=len(prepared.participant_levels),
        J=len(prepared.item_levels),
        X=prepared.X,
        y=prepared.y,
        person=prepared.participant,
        item=prepared.item,
        available=np.asarray(prepared.availability, int),
        signature=np.asarray(prepared.signatures, float),
        anchor_strength=spec.anchor_strength,
    )
    try:
        fit = cmdstanpy.CmdStanModel(stan_file=_stan_path("theory_strategy_mixture.stan")).sample(
            data=data,
            seed=seed,
            chains=spec.chains,
            parallel_chains=spec.parallel_chains,
        ...
        raise EyeProcessBackendError(f"CmdStan strategy-mixture fitting failed: {exc}") from exc
    return _result(
...
def fit_theory_strategy_irt(
    ...
    **kwargs: Any,
    ...
    if item:
        spec["item"] = item
    prep = prepare_strategy_mixture_data(data, spec)
    model = (
        fit_strategy_mixture_em(prep, seed=seed, **kwargs)
        if spec.engine == "em"
        else fit_strategy_mixture_stan(prep, seed=seed, **kwargs)
    )
    return _result(
...
def strategy_posterior_probabilities(object: EyeResult) -> pd.DataFrame:
    if getattr(object, "eyeprocess_class", None) != "eye_theory_strategy_irt":
        raise EyeProcessValidationError("Expected an eye_theory_strategy_irt object.")
    if getattr(object.model, "eyeprocess_class", None) == "eye_strategy_mixture_em":
        p = np.asarray(object.model.posterior, float)
    else:
        try:
            cols = object.model.fit.draws_pd(vars=["posterior_probability"])
            p = (
                cols.to_numpy(float)
                .mean(0)
                .reshape(len(object.prepared.y), len(object.spec.strategies))
            )
        except Exception as exc:
            raise EyeProcessModelError("Posterior strategy probabilities are unavailable.") from exc
    out = pd.DataFrame(p, columns=object.spec.strategies)
    out["trial"] = np.arange(1, len(out) + 1)
    out["participant_id"] = [
        object.prepared.participant_levels[i - 1] for i in object.prepared.participant
    ]
    out["item_id"] = [object.prepared.item_levels[i - 1] for i in object.prepared.item]
    out["modal_strategy"] = [object.spec.strategies[i] for i in np.argmax(p, 1)]
    out["modal_probability"] = p.max(1)
    out["entropy"] = -np.sum(np.where(p > 0, p * np.log(p), 0), axis=1)
    return out


def strategy_classification_uncertainty(object: EyeResult, threshold: float = 0.70) -> EyeResult:
    if not 0 <= float(threshold) <= 1:
        raise EyeProcessValidationError("threshold must be a single probability in [0, 1].")
    p = strategy_posterior_probabilities(object)
    summary = pd.DataFrame(
        [
            {
                "trials": len(p),
                "mean_entropy": p.entropy.mean(),
                "median_modal_probability": p.modal_probability.median(),
                "uncertain_fraction": float((p.modal_probability < threshold).mean()),
                "threshold": threshold,
            }
        ]
    )
    return _result("eye_strategy_uncertainty", trial=p, summary=summary)


def strategy_label_switching_diagnostics(
    object: EyeResult, tolerance: float = 1e-4
) -> pd.DataFrame:
    if getattr(object.model, "eyeprocess_class", None) == "eye_strategy_mixture_em":
        out = object.model.all_starts.copy()
        best = out.log_likelihood.max()
        out["delta_best"] = best - out.log_likelihood
        out["equivalent_optimum"] = out.delta_best <= tolerance * (1 + abs(best))
        out["label_anchor"] = "theory_signature"
        out.attrs["eyeprocess_class"] = "eye_strategy_label_diagnostics"
        return out
    return pd.DataFrame(
        [
            {
                ...
                "assessed": False,
                "reason": "Label diagnostics are unavailable for this engine.",
            }
        ]
    )


def strategy_aoi_sensitivity(
    datasets: Mapping[str, Any] | Sequence[Any], spec: EyeResult, seed: int = 1, **kwargs: Any
) -> EyeResult:
    if isinstance(datasets, Mapping):
        pairs = list(datasets.items())
    else:
        pairs = [(f"definition{i + 1}", v) for i, v in enumerate(datasets)]
    if not pairs:
        raise EyeProcessValidationError("datasets must be non-empty.")
    fits = {}
    rows = []
    for i, (name, data) in enumerate(pairs):
        fit = fit_theory_strategy_irt(data, spec, seed=seed + i, **kwargs)
        fits[name] = fit
        p = strategy_posterior_probabilities(fit)
        shares = p.modal_strategy.value_counts(normalize=True)
        for strategy in spec.strategies:
            rows.append(
                {
                    "definition": name,
                    "strategy": strategy,
                    "modal_share": float(shares.get(strategy, 0)),
                    "mean_entropy": float(p.entropy.mean()),
                }
            )
    return _result("eye_strategy_aoi_sensitivity", fits=fits, summary=pd.DataFrame(rows), spec=spec)


def validate_strategy_manipulation(
    object: EyeResult, condition: str, expected_strategy: str, minimum_contrast: float = 0
) -> EyeResult:
    p = strategy_posterior_probabilities(object)
    d = object.prepared.data.reset_index(drop=True)
    if condition not in d:
        raise EyeProcessValidationError(f"Condition column is absent: {condition}")
    if expected_strategy not in object.spec.strategies:
        raise EyeProcessValidationError(f"Unknown strategy: {expected_strategy}")
    temp = pd.DataFrame(
        {"condition": d[condition].astype(str), "probability": p[expected_strategy]}
    )
    summ = temp.groupby("condition", sort=False).probability.agg(["mean", "count"]).reset_index()
    contrast = float(summ["mean"].max() - summ["mean"].min()) if len(summ) > 1 else 0.0
    return _result(
        "eye_strategy_manipulation_validation",
        strategy=expected_strategy,
        summary=summ,
        contrast=contrast,
        minimum_contrast=float(minimum_contrast),
        supported=contrast >= minimum_contrast,
    )
...
def compare_strategy_heterogeneity(object: EyeResult) -> pd.DataFrame:
    ...
    for strategy in object.spec.strategies:
        v = p[strategy].to_numpy(float)
        rows.append(
            {
                "strategy": strategy,
                "mean_probability": v.mean(),
                "sd_probability": v.std(ddof=1),
                "modal_share": float((p.modal_strategy == strategy).mean()),
            }
        )
    out = pd.DataFrame(rows)
    out.attrs["eyeprocess_class"] = "eye_strategy_heterogeneity"
    return out


def simulate_strategy_mixture_data(
    n_person: int = 100,
    n_item: int = 20,
    signatures: Any = None,
    trials_per_item: int = 1,
    strategy_prevalence: Any = None,
    feature_sd: float = 0.6,
    seed: int = 1,
) -> pd.DataFrame:
    if signatures is None:
        signatures = np.array([[1, 1], [-1, 0.2]], float)
    if isinstance(signatures, pd.DataFrame):
        names = list(map(str, signatures.index))
        cols = list(map(str, signatures.columns))
        S = signatures.to_numpy(float)
    elif isinstance(signatures, Mapping):
        spec = theory_strategy_spec(signatures)
        names = spec.strategies
        cols = spec.feature_columns
        S = spec.prototypes.to_numpy(float)
    else:
        S = np.asarray(signatures, float)
        names = [f"strategy_{i + 1}" for i in range(S.shape[0])]
        cols = [f"feature_{j + 1}" for j in range(S.shape[1])]
    if n_person < 2 or n_item < 2 or trials_per_item < 1 or S.ndim != 2 or S.shape[0] < 2:
        raise EyeProcessValidationError("Simulation sizes/signatures are invalid.")
    prev = (
        np.ones(S.shape[0]) / S.shape[0]
        if strategy_prevalence is None
        else np.asarray(strategy_prevalence, float)
    )
    prev = prev / prev.sum()
    rng = np.random.default_rng(seed)
    theta = rng.normal(size=n_person)
    diff = rng.normal(size=n_item)
    rows = []
    for p in range(n_person):
        for j in range(n_item):
            for r in range(trials_per_item):
                k = int(rng.choice(S.shape[0], p=prev))
                feat = S[k] + rng.normal(0, feature_sd, S.shape[1])
                y = int(
                    rng.random()
                    < expit(theta[p] - diff[j] + np.linspace(-0.35, 0.35, S.shape[0])[k])
                )
                row = {
                    ...
                    "repetition": r + 1,
                    "score": y,
                    "true_strategy": names[k],
                }
                row.update(dict(zip(cols, feat)))
                rows.append(row)
    out = pd.DataFrame(rows)
    out.attrs["truth"] = {
        "strategy_prevalence": prev,
        "signatures": S,
        "theta": theta,
        "difficulty": diff,
    }
...
def gaze_diffusion_spec(
    response: str = "score",
    response_time: str = "response_time",
    participant: str = "participant_id",
    item: str = "item_id",
    drift_features: Sequence[str] = (),
    boundary_features: Sequence[str] = (),
    nondecision_features: Sequence[str] = (),
    starting_features: Sequence[str] = (),
    censor_column: str | None = None,
    contaminant: bool = True,
    engine: str = "baseline",
    gaze_features: Sequence[str] | None = None,
    chains: int = 4,
    parallel_chains: int | None = None,
    iter_warmup: int = 1000,
    iter_sampling: int = 1000,
    ...
    if engine not in {"baseline", "stan", "ez_regression", "diffIRT", "brms"}:
        raise EyeProcessValidationError("Unknown diffusion engine.")
    drift = list(map(str, drift_features))
    boundary = list(map(str, boundary_features))
    nondec = list(map(str, nondecision_features))
    starting = list(map(str, starting_features))
    if gaze_features is not None and not drift:
        drift = list(map(str, gaze_features))
    allf = drift + boundary + nondec + starting
    if len(set(allf)) != len(allf):
        raise EyeProcessValidationError(
            "A feature may map to only one diffusion parameter in a confirmatory specification."
        )
    if parallel_chains is None:
        parallel_chains = min(4, chains)
    return _result(
        "eye_gaze_diffusion_spec",
        response=response,
        response_time=response_time,
        participant=participant,
        item=item,
        drift_features=drift,
        gaze_features=list(dict.fromkeys(allf)),
        boundary_features=boundary,
        nondecision_features=nondec,
        starting_features=starting,
        censor_column=censor_column,
        contaminant=bool(contaminant),
        engine=engine,
        normalized_engine=engine,
        legacy_mode=engine in {"ez_regression", "diffIRT", "brms"},
        chains=int(chains),
        parallel_chains=int(parallel_chains),
        iter_warmup=int(iter_warmup),
        iter_sampling=int(iter_sampling),
...
def _std_matrix(d: pd.DataFrame, cols: Sequence[str], prefix: str) -> np.ndarray:
    ...
    X = d[list(cols)].apply(pd.to_numeric, errors="coerce").to_numpy(float)
    center = X.mean(0)
    scale = X.std(0, ddof=1)
    scale[(~np.isfinite(scale)) | (scale == 0)] = 1
    z = (X - center) / scale
...
def prepare_gaze_diffusion_data(data: Any, spec: EyeResult, minimum_rt: float = 0.05) -> EyeResult:
    ...
    features = (
        spec.drift_features
        + spec.boundary_features
        + spec.nondecision_features
        + spec.starting_features
    )
    req = [spec.response, spec.response_time, spec.participant, spec.item, *features] + (
        ([spec.censor_column]) if spec.censor_column else []
    )
    _required(d, req)
    d = d.dropna(subset=req).copy()
    rt = pd.to_numeric(d[spec.response_time], errors="coerce").to_numpy(float)
    if len(d) == 0 or np.any(~np.isfinite(rt)) or np.any(rt <= minimum_rt):
        raise EyeProcessValidationError(
            "Response times must be finite seconds greater than minimum_rt."
        )
    if np.median(rt) > 30:
        raise EyeProcessValidationError(
            "Response times appear to be milliseconds; convert them to seconds before diffusion modelling."
        )
    y = pd.to_numeric(d[spec.response], errors="coerce").to_numpy(int)
    if not np.isin(y, [0, 1]).all():
        raise EyeProcessValidationError("Diffusion responses must be coded 0/1.")
    if spec.censor_column:
        cmap = {"observed": 0, "right": 1, "left": 2}
        vals = d[spec.censor_column].astype(str).str.lower()
        censor = vals.map(cmap).to_numpy()
        if pd.isna(censor).any():
            raise EyeProcessValidationError("Censoring values must be observed, right, or left.")
        censor = censor.astype(int)
    else:
        censor = np.zeros(len(d), int)
    pl = list(dict.fromkeys(d[spec.participant].astype(str)))
    il = list(dict.fromkeys(d[spec.item].astype(str)))
    pidx = np.array([pl.index(v) + 1 for v in d[spec.participant].astype(str)])
    iidx = np.array([il.index(v) + 1 for v in d[spec.item].astype(str)])
    return _result(
        "eye_gaze_diffusion_data",
        data=d,
        y=y,
        rt=rt,
        censor=censor,
        participant=pidx,
        item=iidx,
        participant_levels=pl,
        item_levels=il,
        X_drift=_std_matrix(d, spec.drift_features, "drift:"),
        X_boundary=_std_matrix(d, spec.boundary_features, "boundary:"),
        X_nondecision=_std_matrix(d, spec.nondecision_features, "nondecision:"),
        X_starting=_std_matrix(d, spec.starting_features, "starting:"),
        rt_lower=float(minimum_rt),
        rt_upper=float(rt.max() * 1.25),
        minimum_observed_rt=float(rt.min()),
        spec=spec,
    )
...
def _baseline_diffusion(prep: EyeResult) -> EyeResult:
    ...
    features = list(
        dict.fromkeys(
            prep.spec.drift_features
            + prep.spec.boundary_features
            + prep.spec.nondecision_features
            + prep.spec.starting_features
        )
    )
    base = pd.DataFrame({"Intercept": np.ones(len(d))})
    for c in features:
        base[c] = pd.to_numeric(d[c], errors="coerce").to_numpy(float)
    base = pd.concat(
        [
            base,
            pd.get_dummies(
                d[prep.spec.participant].astype(str),
                prefix="participant",
                drop_first=True,
                dtype=float,
            ),
            pd.get_dummies(
                d[prep.spec.item].astype(str), prefix="item", drop_first=True, dtype=float
            ),
        ],
        axis=1,
    )
    X = base.to_numpy(float)
    bacc = _logistic_fit(X, prep.y)
    blog = np.linalg.lstsq(X, np.log(prep.rt), rcond=None)[0]
    return _result(
        "eye_gaze_diffusion_baseline",
        accuracy=_result("eye_logistic_model", terms=list(base.columns), coef=bacc),
        timing=_result("eye_linear_model", terms=list(base.columns), coef=blog),
        prepared=prep,
        item_parameters=pd.DataFrame(
            {
                "item_id": prep.item_levels,
                "difficulty": np.nan,
                "boundary_proxy": np.nan,
                "nondecision_proxy": np.nan,
            }
        ),
    )


def fit_gaze_diffusion_stan(
    prepared: EyeResult,
    seed: int = 1,
    refresh: int = 0,
    output_dir: str | None = None,
    **kwargs: Any,
    ...
    try:
        ...
        if ver is None or tuple(ver) < (2, 38):
            raise EyeProcessBackendError(
                "The censored Wiener engine requires CmdStan 2.38.0 or newer."
            )
    except EyeProcessBackendError:
        raise
    except Exception as exc:
        raise EyeProcessBackendError(
            "CmdStan is not available; install/configure eyeprocesspy[stan]."
        ) from exc
    data = dict(
        N=len(prepared.y),
        P=len(prepared.participant_levels),
        J=len(prepared.item_levels),
        Fd=prepared.X_drift.shape[1],
        Fb=prepared.X_boundary.shape[1],
        Fn=prepared.X_nondecision.shape[1],
        Fs=prepared.X_starting.shape[1],
        Xd=prepared.X_drift,
        Xb=prepared.X_boundary,
        Xn=prepared.X_nondecision,
        Xs=prepared.X_starting,
        y=prepared.y,
        rt=prepared.rt,
        censor=prepared.censor,
        person=prepared.participant,
        item=prepared.item,
        min_rt=prepared.minimum_observed_rt,
        rt_lower=prepared.rt_lower,
        rt_upper=prepared.rt_upper,
        use_contaminant=int(spec.contaminant),
    )
    try:
        fit = cmdstanpy.CmdStanModel(stan_file=_stan_path("gaze_diffusion_irt.stan")).sample(
            data=data,
            seed=seed,
            chains=spec.chains,
            parallel_chains=spec.parallel_chains,
        ...
        raise EyeProcessBackendError(f"CmdStan gaze-diffusion fitting failed: {exc}") from exc
    return _result(
...
def fit_gaze_diffusion_irt(
    data: Any, spec: EyeResult | None = None, seed: int = 1, **kwargs: Any
) -> EyeResult:
    spec = gaze_diffusion_spec() if spec is None else spec
    if spec.legacy_mode:
        raise EyeProcessBackendError(
            f"Legacy R diffusion engine '{spec.engine}' is not silently substituted in Python; use an explicit validated adapter."
        )
    prep = prepare_gaze_diffusion_data(data, spec)
    model = (
        _baseline_diffusion(prep)
        if spec.engine == "baseline"
        else fit_gaze_diffusion_stan(prep, seed=seed, **kwargs)
    )
    return _result(
...
def extract_diffusion_parameters(
    object: EyeResult,
    ...
    if getattr(object.model, "eyeprocess_class", None) == "eye_gaze_diffusion_baseline":
        ...
        t = object.model.timing
        return pd.concat(
            [
                pd.DataFrame({"component": "accuracy", "term": a.terms, "estimate": a.coef}),
                pd.DataFrame(
                    {"component": "log_response_time", "term": t.terms, "estimate": t.coef}
                ),
            ],
            ignore_index=True,
        )
    ...
    vals = (
        ...
        if "variable" in s
        else pd.Series(s.index.astype(str), index=s.index)
    )
...
def diffusion_parameter_diagnostics(
    object: EyeResult, correlation_threshold: float = 0.85
) -> EyeResult:
    if getattr(object.model, "eyeprocess_class", None) != "eye_gaze_diffusion_stan":
        return _result(
            "eye_diffusion_diagnostics",
            engine="baseline",
            sampling=None,
            correlations=pd.DataFrame(),
            flagged=pd.DataFrame(),
            message="Posterior identification diagnostics require the Stan engine.",
        )
    return _result(
        "eye_diffusion_diagnostics",
        engine="stan",
        sampling=object.model.diagnostics,
        correlations=pd.DataFrame(),
        flagged=pd.DataFrame(),
...
def diffusion_posterior_predictive(
    ...
    if getattr(object.model, "eyeprocess_class", None) == "eye_gaze_diffusion_baseline":
        observed = pd.DataFrame(
            [
                {
                    "accuracy": float(np.mean(object.prepared.y)),
                    "mean_rt": float(np.mean(object.prepared.rt)),
                    "median_rt": float(np.median(object.prepared.rt)),
                }
            ]
        )
        return _result(
...
def compare_diffusion_accuracy_rt(object: EyeResult) -> pd.DataFrame:
    if getattr(object, "eyeprocess_class", None) != "eye_gaze_diffusion_irt":
        raise EyeProcessValidationError("Expected a gaze-diffusion fit.")
    return pd.DataFrame(
        {
            "model": ["gaze_diffusion", "separate_accuracy_log_rt"],
            "log_likelihood_proxy": [np.nan, np.nan],
            "parameters": [np.nan, len(extract_diffusion_parameters(object))],
            "purpose": [
                "joint cognitive-process likelihood",
                "descriptive accuracy and timing baseline",
            ],
        }
    )
...
def _wiener_trial(
    ...
    pos = boundary * starting
    limit = max(1, int(math.ceil(max_time / dt)))
    for step in range(1, limit + 1):
        pos += drift * dt + math.sqrt(dt) * rng.normal()
        if pos >= boundary:
            return 1, nondecision + step * dt
        if pos <= 0:
            return 0, nondecision + step * dt
    return int(pos >= boundary / 2), nondecision + max_time


def simulate_gaze_diffusion_data(
    n_person: int = 80,
    n_item: int = 20,
    trials_per_item: int = 1,
    gaze_effect: float = 0.35,
    contaminant_fraction: float = 0.02,
    time_step: float = 0.002,
    max_decision_time: float = 10,
    seed: int = 1,
) -> pd.DataFrame:
    if n_person < 2 or n_item < 2 or trials_per_item < 1:
        raise EyeProcessValidationError(
            "Simulation requires at least two persons, two items, and one trial per item."
        )
    if not 0 <= contaminant_fraction < 1:
        raise EyeProcessValidationError("contaminant_fraction must be in [0, 1).")
    rng = np.random.default_rng(seed)
    theta = rng.normal(size=n_person)
    difficulty = rng.normal(size=n_item)
    person_boundary = np.exp(0.15 + rng.normal(0, 0.12, n_person))
    item_nd = 0.18 + expit(rng.normal(-2, 0.3, n_item)) * 0.18
    person_start = expit(rng.normal(0, 0.15, n_person))
    rows = []
    for p in range(n_person):
        for j in range(n_item):
            for rep in range(trials_per_item):
                gaze = float(rng.normal())
                drift = float(theta[p] - difficulty[j] + gaze_effect * gaze)
                boundary = float(person_boundary[p])
                nd = float(item_nd[j])
                starting = float(person_start[p])
                y, rt = _wiener_trial(
                    rng, drift, boundary, nd, starting, time_step, max_decision_time
                )
                contaminant = rng.random() < contaminant_fraction
                if contaminant:
                    rt = float(rng.uniform(max(0.05, nd), max(rt, nd + 0.05)))
...
def diffusion_identification_study(
    conditions: Any = None,
    replications: int = 20,
    ...
    if isinstance(conditions, Mapping):
        ...
        grid = pd.DataFrame(
            [dict(zip(keys, v)) for v in itertools.product(*[conditions[k] for k in keys])]
        )
    else:
        grid = _df(conditions)
    jobs = []
    ...
    for _, r in grid.iterrows():
        for rep in range(1, int(replications) + 1):
            j += 1
            jobs.append({**r.to_dict(), "replication": rep, "seed": base_seed + j - 1})
    plan = _result(
        "eye_validation_job_plan",
        plan_id=f"gaze_diffusion-{base_seed}-{len(jobs)}",
        jobs=pd.DataFrame(jobs),
        model_family="gaze_diffusion",
    )
    return _result(
        "eye_diffusion_identification_study",
        plan=plan,
        spec=gaze_diffusion_spec(drift_features=["gaze_balance"], engine="stan")
        if spec is None
        else spec,
        metadata={
            "purpose": "parameter identification, recovery, censoring, contaminants, and trade-off diagnostics"
        },
    )
