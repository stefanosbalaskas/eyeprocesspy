...
_GP_COLUMNS = {
    ...
    "left_x": ["LPOGX", "left_x"],
    "left_y": ["LPOGY", "left_y"],
    "right_x": ["RPOGX", "right_x"],
    "right_y": ["RPOGY", "right_y"],
    "left_valid": ["LPOGV", "left_valid"],
    "right_valid": ["RPOGV", "right_valid"],
...
def _gp_filename_identity(path, participant_id=None, recording_id=None, session_id="S001"):
    base = Path(path).stem
    stem = re.sub(
        r"(?i)(?:[_ -](?:all[_ -]?gaze|fixations?|user(?:[_ -]?fix)?))$", "", base
    ).strip()
    m = re.match(r"(?i)^user[ _-]*([0-9]+)$", stem)
    inferred = f"User {m.group(1)}" if m else stem
    if not inferred or re.match(r"(?i)^data[_ -]?summary", inferred):
        inferred = "P001"
    ...
    recording = str(
        ...
        else f"rec_{_gp_id_token(participant)}_{_gp_id_token(session)}"
    )
    return {
...
def _gp_tick_origin(data, info):
    tick_col = info["tick_col"]
    freq = info["tick_frequency"]
    if not tick_col or not np.isfinite(freq):
        return np.nan
    tick = _safe_numeric(data[tick_col])
    media = (
        _safe_numeric(data[info["media_time_col"]])
    ...
    i = keep[0]
    return (
        float(tick.iloc[i] - media.iloc[i] * freq)
        if np.isfinite(media.iloc[i])
        else float(tick.iloc[i])
    )
...
def _gp_biometric_mapping(data):
    ...
    return {
        ...
        if (col := _first_existing(nms, cands)) is not None
    }
...
def _gp_mapping(data):
    ...
    return eye_mapping(
        participant=_gp_pick(nms, "participant"),
        recording=_gp_pick(nms, "recording"),
        timestamp=timestamp,
        x=_gp_pick(nms, "x"),
        y=_gp_pick(nms, "y"),
        left_x=_gp_pick(nms, "left_x"),
        left_y=_gp_pick(nms, "left_y"),
        right_x=_gp_pick(nms, "right_x"),
        right_y=_gp_pick(nms, "right_y"),
        gaze_valid=_gp_pick(nms, "valid"),
        left_valid=_gp_pick(nms, "left_valid"),
        right_valid=_gp_pick(nms, "right_valid"),
        pupil_left=_gp_pick(nms, "pupil_left"),
        pupil_right=_gp_pick(nms, "pupil_right"),
        pupil_left_valid=_gp_pick(nms, "pupil_left_valid"),
        pupil_right_valid=_gp_pick(nms, "pupil_right_valid"),
        fixation_id=_gp_pick(nms, "fixation_id"),
        blink_id=_first_existing(nms, ["BKID", "BLINK_ID"]),
        trial=_gp_pick(nms, "trial"),
        stimulus=_gp_pick(nms, "stimulus"),
        event_name=_gp_pick(nms, "marker"),
...
def _gp_is_summary_report(path):
    ...
    if not p.is_file():
        return False
    if re.match(r"(?i)^Data[_ -]?Summary[_ -]?export.*\.csv$", p.name):
        return True
    try:
        lines = p.read_text(encoding="utf-8", errors="replace").splitlines()[:20]
    except OSError:
        return False
    return any(line.strip() == "AOI Summary" for line in lines) and any(
        line.startswith("Gazepoint Analysis") for line in lines
    )
...
def is_gazepoint_export(path, inspect_rows=20):
    ...
    if p.suffix.lower() not in {".csv", ".txt", ".tsv"}:
        return 0.0
    if _gp_is_summary_report(p):
        return 0.99
    filename_signature = bool(
        re.search(r"(?i)(?:_all[_ -]?gaze|_fixations?|[-_]user(?:[-_]fix)?)\.csv$", p.name)
    )
    try:
        header = list(_read_delimited(p, nrows=min(2, int(inspect_rows))).columns)
    except Exception:
        return 0.75 if filename_signature else 0.0
    known = {str(v).upper() for values in _GP_COLUMNS.values() for v in values}
    gp_hits = sum(str(h).upper() in known for h in header)
    sig = any(
        s in {str(h).upper() for h in header}
        for s in ["FPOGX", "BPOGX", "LPOGX", "RPOGX", "FPOGV", "BPOGV", "TIMETICK(F=10000000)"]
    )
    score = min(1.0, gp_hits / 8.0)
    if sig:
        score = max(score, 0.85)
    if filename_signature or "currentaoistatistics" in p.name.lower():
...
def gp_identify_export_type(path):
    ...
    if p.is_dir():
        return "folder"
    if _gp_is_summary_report(p) or "currentaoistatistics" in p.name.lower():
        return "aoi_statistics"
    if re.search(r"(?i)(?:_fixations?|[-_]user[-_]?fix)\.csv$", p.name):
        return "fixations"
    try:
        d = _read_delimited(p, nrows=3)
    except Exception:
        return "unknown"
    nms = {str(n).upper() for n in d.columns}
    if (
        {"FPOGS", "FPOGD"}.issubset(nms)
        and "FPOGID" in d
        and not d["FPOGID"].duplicated().any()
        and not re.search(r"(?i)_all[_ -]?gaze", p.name)
    ):
        return "fixations"
    has_gaze = bool(nms & {"BPOGX", "FPOGX", "LPOGX", "RPOGX"})
    has_bio = any(re.match(r"^(HR|GSR|EDA|DIAL|IBI)", n) for n in nms)
    if has_gaze and has_bio:
        return "combined_biometrics"
    if has_gaze:
        return "gaze"
    return "unknown"
...
def gp_profile_export(path):
    p = Path(path)
    files = [q for q in p.iterdir() if q.is_file()] if p.is_dir() else [p]
    rows = []
    for f in files:
        try:
            d = _read_delimited(f, nrows=10)
            columns = "|".join(map(str, d.columns))
            ncols = d.shape[1]
        except Exception:
            columns = pd.NA
            ncols = pd.NA
    ...
    out.attrs["path_type"] = gp_identify_export_type(path)
    return out
            ...
            rows.append(
                {
                    ...
                    "file": row.file,
                    "message": "File does not strongly match known Gazepoint fields.",
                }
            )
...
def _gp_apply_timebase(out, data, info, origin_tick=None):
    if not info["tick_col"] or not np.isfinite(info["tick_frequency"]):
        return out
    ...
    for table in ["gaze_samples", "eye_samples", "biometrics", "events"]:
        ...
        if not d.empty and "timestamp_native" in d:
            d["timestamp_seconds"] = (
                pd.to_numeric(d.timestamp_native, errors="coerce") - origin
            ) / freq
            out[table] = d
    ...
    if not out["streams"].empty:
        out["streams"].loc[:, "timestamp_unit"] = "ticks"
        out["streams"].loc[:, "source_clock"] = "Gazepoint TIMETICK"
        for idx, row in out["streams"].iterrows():
            if row.stream_type == "gaze_combined":
                z = out["gaze_samples"].loc[
                    out["gaze_samples"].recording_id == row.recording_id, "timestamp_seconds"
                ]
                out["streams"].loc[idx, "observed_rate_hz"] = estimate_sampling_rate(z)
    out.vendor_metadata["gazepoint_timebase"] = {
...
def _gp_apply_biometric_validity(out, data):
    if out["biometrics"].empty:
        return out
    info = _gp_time_info(data)
    validity = {
        "gsr_raw": "GSRV",
        "eda": "GSRV",
        "skin_conductance_level": "GSRV",
        "skin_conductance_response": "GSRV",
        "heart_rate": "HRV",
        "heart_rate_period": "HRV",
        "interbeat_interval": "HRV",
        "engagement_dial": "DIALV",
    }
    units = {
        "gsr_raw": "vendor_raw",
        "eda": "microsiemens",
        ...
        "heart_rate_period": "vendor_units",
        "interbeat_interval": "seconds",
        "engagement_dial": "proportion",
    }
    ...
    for channel in pd.unique(out["biometrics"].channel):
        ...
        if tick is not None and vcol in data.columns:
            ...
            out["biometrics"].loc[idx, "valid"] = [
                lookup.get(v, pd.NA) for v in out["biometrics"].loc[idx, "timestamp_native"]
            ]
        if channel in units:
            out["biometrics"].loc[idx, "unit"] = units[channel]
    if not out["streams"].empty:
        for channel, unit in units.items():
            out["streams"].loc[out["streams"].stream_type.eq(channel), "value_unit"] = unit
    return out


def gp_parse_user_events(x):
    if not is_eye_dataset(x):
        raise TypeError("Expected an `eye_dataset` object.")
    raw = x.raw.get("gazepoint") if isinstance(x.raw, dict) else None
    if raw is None and isinstance(x.raw, dict):
        raw = x.raw.get("generic")
    if not isinstance(raw, pd.DataFrame):
        return x
    col = _first_existing(list(raw.columns), ["USER_DATA", "MARKER", "EVENT"])
    time_col = _gp_pick(raw.columns, "time")
    if not col or not time_col:
        return x
    values = raw[col].astype("string")
    keep = values.notna() & values.ne("")
    if not keep.any():
        return x
    ...
    for j, i in enumerate(np.flatnonzero(keep.to_numpy()), 1):
        t = (
            float(_safe_numeric(raw[time_col]).iloc[i])
            if np.isfinite(_safe_numeric(raw[time_col]).iloc[i])
            else np.nan
        )
        rows.append(
            {
                "event_id": f"{rec_id}_gp_user_{j:07d}",
                "recording_id": rec_id,
                "timestamp_native": t,
                "timestamp_seconds": t,
                "event_type": "user_data",
                "event_name": values.iloc[i],
                "event_value": values.iloc[i],
                "duration": np.nan,
                "source": "Gazepoint USER_DATA",
                "native_record": values.iloc[i],
                "trial_id": pd.NA,
                "stimulus_id": pd.NA,
            }
        )
    ...
    existing = (
        set(
            zip(
                x["events"].recording_id.astype(str),
                x["events"].timestamp_seconds.astype(str),
                x["events"].event_name.astype(str),
            )
        )
        if not x["events"].empty
        else set()
    )
    ev = ev[
        [
            (str(r.recording_id), str(r.timestamp_seconds), str(r.event_name)) not in existing
            for r in ev.itertuples(index=False)
        ]
    ]
    if not ev.empty:
        x["events"] = standardize_eye_table(
            pd.concat([x["events"], ev], ignore_index=True, sort=False), "events"
        )
    return x
...
def gp_parse_media_events(x):
    if not is_eye_dataset(x):
        raise TypeError("Expected an `eye_dataset` object.")
    if x["gaze_samples"].empty or x["gaze_samples"].stimulus_id.isna().all():
        return x
    d = (
        x["gaze_samples"]
        .sort_values(["recording_id", "timestamp_seconds"], kind="stable")
        .reset_index(drop=True)
    )
    changed = (
        d.groupby("recording_id", sort=False).stimulus_id.transform(lambda z: z.ne(z.shift(1)))
        & d.stimulus_id.notna()
    )
    rows = []
...
def read_gazepoint(
    ...
    **kwargs,
    ...
    if typ == "fixations":
        return read_gazepoint_fixations(
            p,
            participant_id=participant_id,
            recording_id=recording_id,
            session_id=session_id,
    ...
    data = _read_delimited(p, **kwargs)
    identity = _gp_filename_identity(p, participant_id, recording_id, session_id)
    info = _gp_time_info(data)
    mapping = _gp_mapping(data)
    if not all(k in mapping for k in ("timestamp", "x", "y")):
        raise ValueError(
            "Gazepoint sample export is missing identifiable time and gaze-coordinate columns."
        )
    pupil_unit = (
        "millimetres"
        if any(c in data.columns for c in ["LPMM", "RPMM", "LPMMV", "RPMMV"])
        else "vendor_units"
    )
    out = read_eye_generic(
        data,
        mapping=mapping,
        time_unit="ticks" if info["tick_col"] else "seconds",
    ...
    out = _gp_apply_timebase(out, data, info)
    out = _gp_apply_biometric_validity(out, data)
    out["recordings"].loc[:, "device_model"] = "Gazepoint"
    out["recordings"].loc[:, "software_name"] = "Gazepoint Analysis"
    out.vendor_metadata["gazepoint"] = {
        "export_type": typ,
    ...
    out = add_provenance(
        out, "import_gazepoint_v7", "dataset", f"Gazepoint export type: {typ}", source_files=str(p)
    )
...
def read_gazepoint_fixations(
    ...
    **kwargs,
    ...
    tick = (
        _safe_numeric(data[info["tick_col"]])
        if info["tick_col"]
        else pd.Series(np.nan, index=data.index)
    )
    origin = (
        _gp_tick_origin(data, info)
        if origin_tick is None or not np.isfinite(origin_tick)
        else origin_tick
    )
    if np.isfinite(origin) and np.isfinite(info["tick_frequency"]):
        end = (tick - origin) / float(info["tick_frequency"])
        start = end - duration_ms / 1000
    else:
        start = _safe_numeric(
            _map_column(data, {"start": pick("FPOGS", "FIXATION_START", "start_time")}, "start")
        )
        end = start + duration_ms / 1000
    fix = _map_column(
        data, {"fixation_id": pick("FPOGID", "FIXATION_ID", "fixation_id")}, "fixation_id"
    ).astype("string")
    missing = fix.isna() | fix.eq("")
    fix = fix.copy()
    fix.loc[missing] = [f"{i:07d}" for i in range(1, int(missing.sum()) + 1)]
    stimulus = _as_character_id(
        _map_column(data, {"stimulus": pick("MEDIA_ID", "MEDIA_NAME", "stimulus_id")}, "stimulus")
    )
    ...
    episode_id = pd.Series(
        [
            f"{identity['recording_id']}_fix_{_gp_id_token(s)}_{_gp_id_token(f)}"
            for s, f in zip(stimulus, fix)
        ]
    )
    if episode_id.duplicated().any():
        episode_id = episode_id + pd.Series([f"_row{i:05d}" for i in range(1, len(episode_id) + 1)])
    episodes = pd.DataFrame(
        {
            ...
            "end_x": np.nan,
            "end_y": np.nan,
            "centroid_x": _safe_numeric(
                _map_column(data, {"x": pick("FPOGX", "FIXATION_X", "x")}, "x")
            ),
            "centroid_y": _safe_numeric(
                _map_column(data, {"y": pick("FPOGY", "FIXATION_Y", "y")}, "y")
            ),
            "amplitude": np.nan,
            ...
            "aoi_id": _gp_aoi_id(stimulus, raw_aoi),
            "timestamp_native": tick,
            "source_fixation_id": fix,
            "source_media_time_start": _safe_numeric(
                _map_column(data, {"start": pick("FPOGS")}, "start")
            ),
            "source_media_time_end": _safe_numeric(data[info["media_time_col"]])
            if info["media_time_col"]
            else np.nan,
        ...
        [
            {
                ...
                "source_file_set": str(Path(path).resolve()),
            }
        ]
    )
    out = new_eye_dataset(
        ...
        coordinate_spaces=new_coordinate_space(
            "coord_display_normalized_top_left", "display_normalized_top_left"
        ),
        raw={"gazepoint_fixations": data.copy()} if keep_raw else {},
        vendor_metadata={
            "gazepoint_fixations": {
                "source_columns": list(map(str, data.columns)),
                "source_identity": identity,
    ...
    out = add_provenance(
        out,
        "import_gazepoint_fixations_v7",
        "episodes",
        f"{len(episodes)} fixations",
...
def gp_pair_exports(path):
    ...
    rows = []
    for f in files:
        typ = gp_identify_export_type(f)
        base = f.stem
        group = re.sub(r"(?i)(?:[_ -](?:all[_ -]?gaze|fixations?|user(?:[_ -]?fix)?))$", "", base)
        if typ == "aoi_statistics":
            group = "gazepoint_data_summary"
...
def gp_match_recordings(path):
    return gp_pair_exports(path)


def gp_match_biometrics(path):
    pairs = gp_pair_exports(path)
    return pairs[pairs.export_type.isin(["combined_biometrics", "gaze"])].reset_index(drop=True)
        ...
        rows.append(
            {
                ...
                "has_gaze": bool(has_gaze),
                "has_fixations": bool(d.export_type.eq("fixations").any()),
                "has_biometrics": bool(d.export_type.eq("combined_biometrics").any()),
                "n_files": len(d),
                "status": "usable" if has_gaze else "incomplete",
            }
        )
    out = pd.DataFrame(rows)
    out.attrs["summary_reports"] = (
        int(pairs.export_type.eq("aoi_statistics").sum()) if not pairs.empty else 0
    )
    return out
...
def read_gazepoint_folder(
    ...
    **kwargs,
    ...
    if not files:
        raise ValueError("No delimited Gazepoint exports found.")
    profile = gp_pair_exports(p)
    objs = []
    user = profile[profile.export_type.isin(["gaze", "combined_biometrics", "fixations"])]
    groups = list(pd.unique(user.group))
    if recording_id is not None and len(groups) > 1:
        raise ValueError(
            "recording_id can only be supplied when a Gazepoint folder contains one recording group; omit it for multi-recording folders so identifiers can be derived from filenames."
        )
    include = set(include)
    for group in groups:
        d = user[user.group.eq(group)]
        gaze = d[d.export_type.isin(["gaze", "combined_biometrics"])]
        if not gaze.empty and bool(include & {"gaze", "biometrics"}):
            gf = Path(gaze.file.iloc[0])
            ident = _gp_filename_identity(gf, participant_id, recording_id, session_id)
            g = read_gazepoint(
                gf,
                participant_id=ident["participant_id"],
            ...
            if "fixations" in include:
                for ff in d.loc[d.export_type.eq("fixations"), "file"]:
                    objs.append(
    ...
    if not objs:
        raise ValueError("No requested Gazepoint export types were found.")
    out = combine_eye_datasets(objs, resolve_ids=False)
    out.vendor_metadata["gazepoint_folder"] = profile
    out = add_provenance(
        out,
        "import_gazepoint_folder_v7",
        "dataset",
        f"Files: {len(files)}; paired groups: {len(groups)}",
...
def read_gazepoint_biometrics(
    ...
    **kwargs,
    ...
    data = _read_delimited(path, **kwargs)
    channels = _gp_biometric_mapping(data)
    if not channels:
        raise ValueError("No recognized Gazepoint biometric channels found.")
    if any(c in data.columns for c in ["BPOGX", "FPOGX", "LPOGX", "RPOGX"]):
        ...
        out["streams"] = out["streams"][out["streams"].stream_type.ne("gaze_combined")].reset_index(
            drop=True
        )
        out = add_provenance(
            out,
            "select_gazepoint_biometrics",
            "biometrics|eye_samples",
            ",".join(channels),
            source_files=str(path),
        )
        out.validation = validate_eye_dataset(out)
    ...
    if time_col is None:
        raise ValueError("Cannot identify a Gazepoint time column.")
    data = data.copy()
    data[".eye_dummy_x"] = np.nan
    data[".eye_dummy_y"] = np.nan
    mapping = eye_mapping(
        timestamp=time_col,
        x=".eye_dummy_x",
        y=".eye_dummy_y",
        pupil_left=_gp_pick(data.columns, "pupil_left"),
        pupil_right=_gp_pick(data.columns, "pupil_right"),
        pupil_left_valid=_gp_pick(data.columns, "pupil_left_valid"),
        pupil_right_valid=_gp_pick(data.columns, "pupil_right_valid"),
        trial=_gp_pick(data.columns, "trial"),
        stimulus=_gp_pick(data.columns, "stimulus"),
    ...
    out = read_eye_generic(
        ...
        if any(c in data.columns for c in ["LPMM", "RPMM"])
        else "vendor_units",
        keep_raw=keep_raw,
        quiet=True,
    )
    ...
    out.vendor_metadata["gazepoint_biometrics"] = {
        "source_columns": list(map(str, data.columns)),
        "channels": channels,
    }
    out = add_provenance(
        out,
        "import_gazepoint_biometrics_v7",
        "biometrics|eye_samples",
        ",".join(channels),
        source_files=str(path),
    )
...
def read_gazepoint_combined(gaze, fixations=None, biometrics=None, **kwargs):
    g = read_gazepoint(gaze, **kwargs)
    xs = [g]
    ident = {
        "participant_id": g["recordings"].participant_id.iloc[0],
        "recording_id": g["recordings"].recording_id.iloc[0],
        "session_id": g["recordings"].session_id.iloc[0],
    }
