...
def _clip_geometry(geometry: pd.DataFrame, width: float, height: float) -> pd.DataFrame:
    ...
    for i, row in out.iterrows():
        if row["shape_type"] == "rectangle":
            ...
            if (
                not out.loc[i, "xmin"] < out.loc[i, "xmax"]
                or not out.loc[i, "ymin"] < out.loc[i, "ymax"]
            ):
            ...
            if _polygon_self_intersects(p) or abs(_signed_polygon_area(p)) <= 1e-12:
                raise EyeProcessValidationError(
                    f"Screen clipping invalidated polygon AOI `{row['aoi_id']}`."
                )
            out.at[i, "polygon"] = p
    return out
...
def _transform_geometry(geometry: pd.DataFrame, spec: Any) -> pd.DataFrame:
    ...
    for i, row in out.iterrows():
        ...
        if op == "jitter":
            tx = (
                float(rng.uniform(-abs(tx if tx else mx), abs(tx if tx else mx)))
                if rng is not None
                else 0.0
            )
            ty = (
                float(rng.uniform(-abs(ty if ty else my), abs(ty if ty else my)))
                if rng is not None
                else 0.0
            )
        if shape == "rectangle":
            xmin, xmax, ymin, ymax = map(
                float, (row["xmin"], row["xmax"], row["ymin"], row["ymax"])
            )
            if op == "dilation":
                xmin -= mx
                xmax += mx
            ...
            if not xmin < xmax or not ymin < ymax:
                raise EyeProcessValidationError(
                    f"Perturbation `{sp['perturbation_id']}` collapsed AOI `{row['aoi_id']}`."
                )
            ...
            if op in {"dilation", "erosion"}:
                ...
                if width <= 0 or height <= 0 or width + 2 * mx <= 0 or height + 2 * my <= 0:
                    raise EyeProcessValidationError(
                        f"Anisotropic expansion collapsed polygon AOI `{row['aoi_id']}`."
                    )
            ...
            if _polygon_self_intersects(poly) or abs(_signed_polygon_area(poly)) <= 1e-12:
                raise EyeProcessValidationError(
                    f"Perturbation `{sp['perturbation_id']}` invalidated polygon AOI `{row['aoi_id']}`."
                )
...
def translate_aoi(
    aois: pd.DataFrame, x: float = 0.0, y: float = 0.0, **kwargs: Any
) -> pd.DataFrame:
    spec = aoi_perturbation_spec(
        "translation", "translate", translation_x=x, translation_y=y, **kwargs
    )
    return _transform_geometry(aois, spec)


def jitter_aoi(
    aois: pd.DataFrame, x: float, y: float | None = None, *, seed: int = 20260918, **kwargs: Any
) -> pd.DataFrame:
    yy = x if y is None else y
    spec = aoi_perturbation_spec(
        "jitter", "jitter", translation_x=x, translation_y=yy, seed=seed, **kwargs
    )
    return _transform_geometry(aois, spec)
...
def create_aoi_perturbation_grid(
    ...
    **geometry_kwargs: Any,
    ...
    def pairs_or_empty(values: Any, name: str) -> Iterable[Sequence[float]]:
        ...
        if isinstance(values, pd.DataFrame):
            ...
            if array.ndim != 2 or array.shape[1] != 2:
                raise EyeProcessValidationError(
                    f"`{name}` data frames must have exactly two columns."
                )
        ...
        if isinstance(values, np.ndarray):
            ...
            if array.ndim == 1:
                if array.shape[0] != 2:
                    raise EyeProcessValidationError(
                        f"`{name}` one-dimensional arrays must have length two."
                    )
                return [array.tolist()]
            if array.ndim == 2 and array.shape[1] == 2:
                return [row.tolist() for row in array]
            raise EyeProcessValidationError(
                f"`{name}` arrays must be length two or have exactly two columns."
            )
    ...
    for value in values_or_empty(dilations):
        specs.append(
            aoi_perturbation_spec(
                f"dilate_{float(value):g}_{unit}",
                "dilation",
                margin_x=value,
                unit=unit,
                **geometry_kwargs,
    ...
    for value in values_or_empty(translations_x):
        specs.append(
            aoi_perturbation_spec(
                f"shift_x_{float(value):g}_{unit}",
                "translate",
                translation_x=value,
                unit=unit,
                **geometry_kwargs,
    ...
    for value in values_or_empty(translations_y):
        specs.append(
            aoi_perturbation_spec(
                f"shift_y_{float(value):g}_{unit}",
                "translate",
                translation_y=value,
                unit=unit,
                **geometry_kwargs,
    ...
    for idx, value in enumerate(values_or_empty(jitters)):
        specs.append(
            aoi_perturbation_spec(
                f"jitter_{float(value):g}_{unit}_{idx + 1}",
                "jitter",
                translation_x=value,
                translation_y=value,
                unit=unit,
    ...
    for values in pairs_or_empty(anisotropic, "anisotropic"):
        ...
        specs.append(
            aoi_perturbation_spec(
                f"anisotropic_{mx:g}_{my:g}_{unit}",
    ...
    if len(set(ids)) != len(ids):
        raise EyeProcessValidationError(
            "Generated perturbation IDs are not unique; use distinct values."
        )
    table = pd.DataFrame(
        [
            {
...
def apply_aoi_perturbation_grid(aois: pd.DataFrame, grid: Any) -> EyeResult:
    specs = (
        ...
        else getattr(grid, "specifications", None)
    )
    if specs is None:
        try:
            ...
            raise EyeProcessValidationError(
                "`grid` must be created by `create_aoi_perturbation_grid()`."
            ) from exc
            ...
            audits.append(
                {
                    ...
                    "status": "completed",
                    "message": None,
                    "geometry_hash": _stable_frame_hash(out),
                }
            )
        except Exception as exc:
            audits.append(
